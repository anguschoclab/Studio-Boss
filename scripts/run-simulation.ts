...
const filmReleases = releasedProjects.filter(p => (p as unknown as { format?: string }).format !== 'tv');
const tvReleases = releasedProjects.filter(p => (p as unknown as { format?: string }).format === 'tv');
...
if (tvReleases.length > 0) {
  ...
  tvReleases.slice(0, 5).forEach((p: unknown) => { const proj = p as { title: string; budget?: number; revenue?: number; tvDetails?: { currentSeason?: number; episodesOrdered?: number } };
    console.log(`    - ${proj.title}: seasons=${proj.tvDetails?.currentSeason ?? '?'}, eps=${proj.tvDetails?.episodesOrdered ?? '?'}, budget=${((proj.budget||0)/1e6).toFixed(0)}M, revenue=${((proj.revenue||0)/1e6).toFixed(0)}M`);
  });
}
...
const tvFormations = shingleEventLog.filter(e => e.kind === 'formed' && (e as unknown as { medium?: string }).medium === 'TV');
const filmFormations = shingleEventLog.filter(e => e.kind === 'formed' && (e as unknown as { medium?: string }).medium !== 'TV');
