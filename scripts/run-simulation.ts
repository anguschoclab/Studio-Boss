...
if (tvReleases.length > 0) {
  ...
  tvReleases.slice(0, 5).forEach(p => {
    console.log(`    - ${p.title}: seasons=${'tvDetails' in p ? p.tvDetails?.currentSeason ?? '?' : '?'}, eps=${'tvDetails' in p ? p.tvDetails?.episodesOrdered ?? '?' : '?'}, budget=$${((p.budget||0)/1e6).toFixed(0)}M, revenue=$${((p.revenue||0)/1e6).toFixed(0)}M`);
  });
}
...
const totalOverhead = shingleEventLog
  ...
  .reduce((s, e) => s + (('overheadPerYear' in e ? e.overheadPerYear : 0) || 0) * (('termYears' in e ? e.termYears : 1) || 1), 0);
