...
const finalPlayerProjects = Object.values(finalState.entities.projects).filter(
  (p) => p.ownerId === "PLAYER"
);
console.log(`\n[DEBUG] Final player projects count: ${finalPlayerProjects.length}`);
finalPlayerProjects.forEach((p) => {
  console.log(
    `  - ${p.title}: state=${p.state}, weeksInPhase=${p.weeksInPhase}, productionWeeks=${p.productionWeeks}`
  );
});
...
console.log("\n--- DETAILED ANALYSIS ---");
console.log(
  `M&A Activity: ${mergers} studios consolidated (from ${initialStudios} to ${finalStudios})`
);
...
console.log(
  `Successful Projects (ROI > 1.0): ${successfulProjects.length} (${((successfulProjects.length / releasedProjects.length) * 100).toFixed(1)}% success rate)`
);
...
if (tvReleases.length > 0) {
  ...
  tvReleases.slice(0, 5).forEach((p) => {
    ...
    if (p.type === "SERIES" && "tvDetails" in p) {
      console.log(
        `    - ${p.title}: seasons=${p.tvDetails?.currentSeason ?? "?"}, eps=${p.tvDetails?.episodesOrdered ?? "?"}, budget=$${((p.budget || 0) / 1e6).toFixed(0)}M, revenue=$${((p.revenue || 0) / 1e6).toFixed(0)}M`
      );
    } else {
      console.log(
        `    - ${p.title}: budget=$${((p.budget || 0) / 1e6).toFixed(0)}M, revenue=$${((p.revenue || 0) / 1e6).toFixed(0)}M`
      );
    }
  });
}
...
const topTalents = [..._talentsForTop]
  .sort((a, b) => (b.prestige || 0) - (a.prestige || 0))
  .slice(0, 10);
console.log("\n--- TOP 10 TALENTS BY PRESTIGE ---");
topTalents.forEach((t, i) => {
  console.log(
    `  ${i + 1}. ${t.name} — prestige ${(t.prestige || 0).toFixed(1)}, role=${t.role}, draw=${(t.draw || 0).toFixed(0)}, momentum=${(t.momentum || 0).toFixed(0)}`
  );
});
...
rivalPerformance.forEach((r, i) => {
  console.log(
    `${i + 1}. ${r.name} - Cash: $${(r.cash / 1000000).toFixed(1)}M, Prestige: ${r.prestige.toFixed(0)}, Archetype: ${r.archetype}${r.isAcquirable ? " (ACQUIRABLE)" : ""}`
  );
});
...
console.log(
  `Avg Prestige: ${(talents.reduce((sum, t) => sum + t.prestige, 0) / talents.length).toFixed(1)}`
);
...
const byDecade: Record<
  number,
  { releases: number; franchises: number; bankruptcies: number; heat: number; shocks: Set<string> }
> = {};
for (let d = 0; d < 5; d++)
  byDecade[d] = { releases: 0, franchises: 0, bankruptcies: 0, heat: 0, shocks: new Set() };
...
for (let d = 0; d < 5; d++) {
  ...
  console.log(
    `Decade ${simYearStart}-${simYearStart + 9}: releases=${byDecade[d].releases}, franchises_born=${byDecade[d].franchises}, heat~${byDecade[d].heat.toFixed(2)}, inflation=${getBudgetInflation(d * 520 + 260).toFixed(2)}x${byDecade[d].shocks.size ? ", shocks: [" + Array.from(byDecade[d].shocks).join("; ") + "]" : ""}`
  );
}
...
[260, 780, 1300, 1820, 2340].forEach((w, i) => {
  console.log(
    `Year ${(i + 1) * 5 - 2} (week ${w}): heat=${getMarketHeat(w).toFixed(2)}, regime=${getMarketRegime(w)}${getActiveShock(w) ? ", shock: " + getActiveShock(w) : ""}`
  );
});
...
for (let d = 0; d < 5; d++) {
  ...
  console.log(
    `Decade ${1975 + d * 10}s: avg=${avgRivals.toFixed(1)} rivals, min=${minR}, max=${maxR}`
  );
}

// New entrants log — classify by rival id prefix in final state
const finalRivals = Object.values(finalState.entities.rivals || {});
const newEntrants = finalRivals.filter((r) =>
  /^(indie-|disruptor-|divest-|upstart-studio-)/.test(r.id)
);
const indies = newEntrants.filter(
  (r) => r.id.startsWith("indie-") || r.id.startsWith("upstart-studio-")
);
...
console.log(
  `Total: ${newEntrants.length} (indies=${indies.length}, disruptors=${disruptors.length}, divestitures=${divests.length})`
);
...
const maCount = newsHistory.filter((n) =>
  /CONSOLIDATION:|VERTICAL INTEGRATION:/.test(n.headline)
).length;
...
consolidationEventLog.forEach((e) => {
  console.log(
    `  ${e.year} (w${e.week}) ${e.motive.toUpperCase()}: ${e.acquirerName} -> ${e.targetName} $${(e.cost / 1e6).toFixed(0)}M`
  );
});
...
if (antitrustEventLog.length === 0) {
  ...
  antitrustEventLog.forEach((e) => {
    console.log(
      `  ${e.year} (w${e.week}): ${e.kind} vs ${e.dominantName} — top1=${(e.topShare * 100).toFixed(1)}%, top3=${(e.top3Share * 100).toFixed(1)}% (${e.note})`
    );
  });
}
...
if (distressEventLog.length === 0) {
  ...
  distressEventLog.forEach((e) => {
    ...
    console.log(
      `  ${e.year} (w${e.week}) stage${e.stage} ${e.kind}: ${e.studioName}${cp}${amt} — ${e.note}`
    );
  });
}
...
for (let d = 0; d < 5; d++) {
  ...
  console.log(
    `Decade ${1975 + d * 10}s: ip-sales=${s1}, liquidations=${s2}, distressed-M&A=${s3}, bankruptcies=${s4}`
  );
}
...
const tvFormations = shingleEventLog.filter(
  (e) => e.kind === "formed" && "medium" in e && e.medium === "TV"
);
const filmFormations = shingleEventLog.filter(
  (e) => e.kind === "formed" && (!("medium" in e) || e.medium !== "TV")
);
...
tvFormations.forEach((e) => {
  ...
  console.log(
    `  ${e.year} (w${e.week}) TV-${e.dealType}: ${e.ownerName}'s ${e.shingleName} @ ${e.studioName}${overhead}${term}`
  );
});
...
if (shingleEventLog.length === 0) {
  ...
  shingleEventLog.forEach((e) => {
    ...
    console.log(
      `  ${e.year} (w${e.week}) ${e.kind.toUpperCase()}: ${e.ownerName}'s ${e.shingleName}${studio} ${e.dealType || ""}${overhead}${term}${extra}`
    );
  });
}
...
for (let d = 0; d < 5; d++) {
  ...
  const formed = shingleEventLog.filter(
    (e) => e.kind === "formed" && e.week >= lo && e.week < hi
  ).length;
  const churned = shingleEventLog.filter(
    (e) => e.kind === "churned" && e.week >= lo && e.week < hi
  ).length;
  const renewed = shingleEventLog.filter(
    (e) => e.kind === "renewed" && e.week >= lo && e.week < hi
  ).length;
  const cancelled = shingleEventLog.filter(
    (e) => e.kind === "cancelled" && e.week >= lo && e.week < hi
  ).length;
  const expired = shingleEventLog.filter(
    (e) => e.kind === "expired" && e.week >= lo && e.week < hi
  ).length;
  console.log(
    `Decade ${1975 + d * 10}s: formed=${formed}, renewed=${renewed}, churned=${churned}, cancelled=${cancelled}, expired=${expired}`
  );
}
...
finalShingles.forEach((s) => {
  byDealType[s.dealType] = (byDealType[s.dealType] || 0) + 1;
});
console.log(`\n--- FINAL ACTIVE SHINGLES: ${finalShingles.length} ---`);
console.log(
  `  FIRST_LOOK=${byDealType.FIRST_LOOK}, OVERALL=${byDealType.OVERALL}, HOUSEKEEPING=${byDealType.HOUSEKEEPING}, POD=${byDealType.POD}`
);
...
const passthroughs = pitchOutcomeLog.filter(
  (p) => p.note && p.note.startsWith("pass-through")
).length;
const passed = pitchOutcomeLog.filter((p) => p.passed).length;
console.log(
  `  accepted=${accepted}, pass-throughs (first-look rejected, picked up by rival)=${passthroughs}, fully-passed=${passed}`
);
