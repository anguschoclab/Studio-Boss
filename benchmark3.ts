...
function optimized3() {
  ...
  for (const t of talentPool) {
    // Only check Set if the talent actually has a tracked role
...
for (let i = 0; i < 100; i++) { original(); optimized3(); }
...
for (let i = 0; i < ITERATIONS; i++) { optimized3(); }
const endOptimized = performance.now();

console.log(`Original: ${(endOriginal - startOriginal).toFixed(2)} ms`);
console.log(`Optimized3: ${(endOptimized - startOptimized).toFixed(2)} ms`);
