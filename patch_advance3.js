...
// 1. replace `awards: [...(state.industry.awards || []), ...newAwards],` with a pre-allocated array.
code = code.replace(
...
// 2. replace `headlines: [...newHeadlines, ...state.industry.headlines].slice(0, 50),` with a pre-allocated array
code = code.replace(
...
// 3. replace `newState.industry.scandals = [...(newState.industry.scandals || []), ...scandalResult.newScandals];`
code = code.replace(
...
// 4. Update the simulateWorld logic to avoid mapping and spreading loops
// Looking specifically for `financeHistory = [...financeHistory, { week: nextWeek, cash: newCash, revenue, costs }];` in resolveFinancials
code = code.replace(
...
console.log('patched weekAdvance.ts - Part 2');
