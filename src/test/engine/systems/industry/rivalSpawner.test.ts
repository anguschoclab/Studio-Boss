...
function makeRivalDict(count: number, overrides?: (i: number) => Partial<any>): Record<string, any> {
...
describe('RivalSpawner', () => {
  ...
  describe('tickHardBankruptcy', () => {
    ...
    it('only processes one bankruptcy per tick', () => {
      const rivals = makeRivalDict(8, () => ({ cash: 50_000_000, strength: 50 }));
      rivals['rival-0'] = createMockRival({ id: 'rival-0', name: 'Bankrupt A', cash: -500_000_000, strength: 10 });
      rivals['rival-1'] = createMockRival({ id: 'rival-1', name: 'Bankrupt B', cash: -500_000_000, strength: 10 });
      const state = createMockGameState({
        week: 50,
        entities: { projects: {}, talents: {}, contracts: {}, rivals } as any,
      });
