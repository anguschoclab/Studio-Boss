...
describe('stats system', () => {
  describe('getFilmStats', () => {
    ...
    it('applies 5.0x risk multiplier for high tier (budget >= 100M)', () => {
    ...
    it('applies 10.0x risk multiplier for blockbuster tier (budget >= 200M)', () => {
      const tier = BUDGET_TIERS.blockbuster;
      const stats = getFilmStats(tier);
      const riskMultiplier = 10.0;
  ...
  describe('getTvStats', () => {
    ...
    it('applies 4.0x scale multiplier for high tier (budget >= 100M)', () => {
      const tier: typeof BUDGET_TIERS.high = { ...BUDGET_TIERS.high, budget: 100_000_000 };
      const stats = getTvStats(tier, format, episodes);
    ...
    it('applies 6.0x scale multiplier for blockbuster tier (budget >= 150M)', () => {
      const tier = BUDGET_TIERS.blockbuster;
      const stats = getTvStats(tier, format, episodes);
  ...
  describe('getUnscriptedStats', () => {
    ...
    it('applies 5.0x scale multiplier for high tier (budget >= 100M) for weekly cost only', () => {
      const tier: typeof BUDGET_TIERS.high = { ...BUDGET_TIERS.high, budget: 100_000_000 };
      const stats = getUnscriptedStats(tier, format, episodes);
    ...
    it('applies 5.0x scale multiplier for blockbuster tier (budget >= 100M)', () => {
      const tier = BUDGET_TIERS.blockbuster;
      const stats = getUnscriptedStats(tier, format, episodes);
