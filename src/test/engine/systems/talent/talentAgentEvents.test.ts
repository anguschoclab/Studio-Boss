...
function makeRelationship(overrides: Partial<TalentAgentRelationship> = {}): TalentAgentRelationship {
  return {
    talentId: 'TAL-1',
    agentId: 'AGT-1',
...
describe('selectAgentForTalent', () => {
  ...
  it('weighted selection: higher prestige agents selected more often', () => {
    ...
    const talent = createMockTalent({ tier: 'B_LIST', agentId: undefined });
    const state = makeState({ agencies: [makeAgency()], agents: [lowPrestigeAgent, highPrestigeAgent] });
