...
describe('tickProductionEnhancementSystem', () => {
  ...
  it('may generate credit scenes for a high-budget FILM in production', () => {
    const project = makeProject({ id: 'PRJ-1', state: 'production', type: 'FILM' as any, budget: 80_000_000 });
  ...
  it('does not generate credit scenes for a SERIES project', () => {
    const project = makeProject({ id: 'PRJ-1', state: 'production', type: 'SERIES' as any, budget: 80_000_000 });
  ...
  it('does not generate credit scenes for a low-budget FILM', () => {
    const project = makeProject({ id: 'PRJ-1', state: 'production', type: 'FILM' as any, budget: 40_000_000 });
  ...
  it('isolates contracts between projects', () => {
    ...
    for (let seed = 1; seed <= 500 && (notesA.length === 0 || notesB.length === 0); seed++) {
      const rng = new RandomGenerator(seed);
      const impacts = tickProductionEnhancementSystem(state, rng);
      notesA = impacts.filter((i: any) => i.type === 'SCREENPLAY_NOTE_CREATED' && i.payload.note.projectId === 'PRJ-A');
      notesB = impacts.filter((i: any) => i.type === 'SCREENPLAY_NOTE_CREATED' && i.payload.note.projectId === 'PRJ-B');
    }
