...
describe('TalentTier string comparisons (cross-system)', () => {
  ...
  describe('GuestStarEngine.generateGuestStarBooking', () => {
    it('charges 500K base cost for A_LIST talent', () => {
      ...
      const contract = createMockContract({ id: 'CON-S1', talentId: 'TAL-CAST', projectId: 'PRJ-S1' });
      const state = createMockGameState({
        entities: {
          ...baseEntities,
    ...
    it('charges 200K base cost for B_LIST talent', () => {
      ...
      const contract = createMockContract({ id: 'CON-S2', talentId: 'TAL-CAST', projectId: 'PRJ-S2' });
      const state = createMockGameState({
        entities: {
          ...baseEntities,
  ...
  describe('BiographyGenerator.generateBiography', () => {
    it('uses "A-list" label for A_LIST talent', () => {
      const talent = createMockTalent({ tier: 'A_LIST', demographics: { age: 35, gender: 'MALE', ethnicity: 'Unknown', country: 'USA' } });
      const state = createMockGameState();
    ...
    it('uses "up-and-coming" label for NEWCOMER talent', () => {
      const talent = createMockTalent({ tier: 'NEWCOMER', demographics: { age: 25, gender: 'FEMALE', ethnicity: 'Unknown', country: 'USA' } });
      const state = createMockGameState();
      const rng = makeMockRng(0.0);
      const bio = generateBiography(talent, state, rng);
    ...
    it('uses "respected" label for B_LIST talent', () => {
      const talent = createMockTalent({ tier: 'B_LIST', demographics: { age: 45, gender: 'MALE', ethnicity: 'Unknown', country: 'USA' } });
      const state = createMockGameState();
      const rng = makeMockRng(0.0);
      const bio = generateBiography(talent, state, rng);
    ...
    it('uses "working" label for C_LIST talent', () => {
      const talent = createMockTalent({ tier: 'C_LIST', demographics: { age: 40, gender: 'FEMALE', ethnicity: 'Unknown', country: 'USA' } });
      const state = createMockGameState();
      const rng = makeMockRng(0.0);
      const bio = generateBiography(talent, state, rng);
  ...
  describe('DeathSystem tier-based grief', () => {
    it('produces higher grief for A_LIST death', () => {
      ...
      const project = createMockProject({ id: 'PRJ-1', talents: ['TAL-DEAD-1', 'TAL-COW-1'] } as any);
      const state = createMockGameState({
        week: 10,
        entities: {
          ...baseEntities,
