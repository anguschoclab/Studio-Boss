...
function makeState(rivals: RivalStudio[]): GameState {
  ...
  return {
    ...
    market: { opportunities: [], trends: [], activeMarketEvents: [], buyers: [], marketingIntensity: 0 },
...
describe('tickRivalProduction', () => {
  it('spawns projects for rivals over time', () => {
    const rival = createMockRival({ id: 'r1', archetype: 'major', cash: 500_000_000, strength: 80 });
  ...
  it('gates spawn rate by cash/strength', () => {
    const flush = createMockRival({ id: 'rich', archetype: 'major', cash: 1_000_000_000, strength: 95 });
    const broke = createMockRival({ id: 'poor', archetype: 'indie', cash: 1_000_000, strength: 20 });
    ...
    const richCount = Object.values(current.entities.projects).filter((p) => p.ownerId === 'rich').length;
    const poorCount = Object.values(current.entities.projects).filter((p) => p.ownerId === 'poor').length;
  ...
  it('is deterministic for a fixed seed', () => {
    const mk = () => makeState([createMockRival({ id: 'r1', archetype: 'major', cash: 500_000_000, strength: 80 })]);
