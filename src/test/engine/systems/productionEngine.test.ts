...
describe("Production Engine (Target A2) - Symmetry", () => {
  ...
  it("should return PROJECT_UPDATED impacts for Player and Rival", () => {
    ...
    const playerImpact = impacts.find(
      (i) => (i as ProjectUpdateImpact).payload.projectId === "player-p1"
    ) as ProjectUpdateImpact | undefined;
    const rivalImpact = impacts.find(
      (i) => (i as ProjectUpdateImpact).payload.projectId === "rival-p1"
    ) as ProjectUpdateImpact | undefined;
