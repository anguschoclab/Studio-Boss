...
describe('Razzies Award System', () => {
  ...
  test('Razzies are only awarded to projects with Budget > 50M and Score < 30', () => {
    ...
    const state = {
      studio: { internal: { projects: [goodProject, cheapFlop, bigFlop], contracts: [] } },
      industry: { talentPool: [] }
  ...
  test('Razzie win triggers Studio Prestige penalty payload and marks cult classic if absurd', () => {
    const absurdFlop = createProject('absurd', 60_000_000, 20, 'a bizarre and absurd mess', 'Action');
    const state = {
      studio: { internal: { projects: [absurdFlop], contracts: [] } },
      industry: { talentPool: [] }
    } as unknown as GameState;
