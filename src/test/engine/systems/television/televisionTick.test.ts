...
describe('Television Tick System (Guild Auditor)', () => {
  ...
  it('cancels a show if it finishes its run with a poor rating', () => {
    ...
    // evaluateRenewal uses (episodesAired || 0) < (episodesOrdered || 1) but tickTelevision passes NEXT aired:
    // const aired = (project.tvDetails.episodesAired || 0) + 1;
    // evaluateRenewal uses project directly without updating episodesAired inside the object.
    // Let's set episodesAired to 10 so that in evaluateRenewal it will evaluate correctly. Wait, tickTelevision uses evaluateRenewal with the old project object.
    // Wait, let's check evaluateRenewal signature: evaluateRenewal(project, nextAverageRating).
    // If project.tvDetails.episodesAired is 9, then 9 < 10, it returns ON_AIR.
