...
describe("Platform Engine (Target B1)", () => {
  ...
  it("should calculate subscriber growth and churn correctly", () => {
    ...
    const buyerImpact = impacts.find(
      (i): i is BuyerUpdateImpact => i.type === "BUYER_UPDATED" && i.payload.buyerId === "s1"
    );
    expect(buyerImpact).toBeDefined();
    expect(buyerImpact?.type).toBe("BUYER_UPDATED");

    const nextSubs = (buyerImpact?.payload.update as Partial<StreamerPlatform> | undefined)
      ?.subscribers;
