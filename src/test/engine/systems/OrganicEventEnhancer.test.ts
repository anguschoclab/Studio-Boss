...
function setupState(
  ...
  return {
    ...
    entities: { ...createMockGameState().entities, talents },
    relationships: { relationships: relMap, ...overrides.relationships },
  };
}

function tryTrigger(fn: (rng: RandomGenerator) => any, maxTries = 200): any | null {
  for (let i = 0; i < maxTries; i++) {
    const r = fn(makeRng(1 + i));
    if (r) return r;
  }
  return null;
}
...
function contractsFor(projectId: string, ...talentIds: string[]): Record<string, any> {
  ...
  talentIds.forEach((tid, i) => {
    out["C" + (i + 1)] = createMockContract({ id: "C" + (i + 1), projectId, talentId: tid });
  });
  return out;
}
...
describe("checkRelationshipCrises", () => {
  ...
  it("returns null when only friendly relationships exist", () => {
    const s = setupState([makeRel({ type: "friend", strength: 80 })], {
      "TAL-1": createMockTalent({ id: "TAL-1", name: "A" }),
      "TAL-2": createMockTalent({ id: "TAL-2", name: "B" }),
    });
    setContracts(s, "PRJ-1", "TAL-1", "TAL-2");
    expect(checkRelationshipCrises(PROJ, s, makeRng())).toBeNull();
  });
  ...
  it("returns null when feud exists but rng does not trigger", () => {
    ...
    let gotNull = false;
    for (let i = 0; i < 200; i++) {
      if (checkRelationshipCrises(PROJ, s, makeRng(1 + i)) === null) {
        gotNull = true;
        break;
      }
    }
  ...
  it("sets severity high when |feud.strength| > 70", () => {
    const s = setupState([makeRel({ type: "enemy", strength: -85 })], {
      "TAL-1": createMockTalent({ id: "TAL-1", name: "A" }),
      "TAL-2": createMockTalent({ id: "TAL-2", name: "B" }),
    });
    setContracts(s, "PRJ-1", "TAL-1", "TAL-2");
  ...
  it("sets severity medium when |feud.strength| <= 70", () => {
    const s = setupState([makeRel({ type: "rival", strength: -40 })], {
      "TAL-1": createMockTalent({ id: "TAL-1", name: "A" }),
      "TAL-2": createMockTalent({ id: "TAL-2", name: "B" }),
    });
    setContracts(s, "PRJ-1", "TAL-1", "TAL-2");
  ...
  it("only considers relationships where BOTH talents are in cast", () => {
    const s = setupState(
      [makeRel({ talentAId: "TAL-1", talentBId: "TAL-3", type: "rival", strength: -50 })],
      {
  ...
  it("returns null when referenced talents do not exist in state", () => {
    ...
    setContracts(s, "PRJ-1", "TAL-1", "TAL-2");
    for (let i = 0; i < 200; i++) {
      expect(checkRelationshipCrises(PROJ, s, makeRng(1 + i))).toBeNull();
    }
  });
});
...
describe("checkCliqueCrises", () => {
  ...
  it("returns null when no cliques have 2+ members on project", () => {
    ...
    s.relationships = {
      cliques: { cliques: { "CLQ-1": makeClique() }, memberCliqueMap: { "TAL-1": ["CLQ-1"] } },
    };
    expect(checkCliqueCrises(PROJ, s, makeRng())).toBeNull();
  });
  ...
  it("returns NEWS_ADDED when toxic clique has 2+ members and rng triggers", () => {
    ...
    s.relationships = {
      cliques: {
        cliques: { "CLQ-1": makeClique({ name: "Bad Blood", reputation: "toxic" }) },
        memberCliqueMap: { "TAL-1": ["CLQ-1"], "TAL-2": ["CLQ-1"], "TAL-3": ["CLQ-1"] },
      },
    };
  ...
  it("returns null when toxic clique has 2+ members but rng does not trigger", () => {
    ...
    s.relationships = {
      cliques: {
        cliques: { "CLQ-1": makeClique({ reputation: "toxic" }) },
        memberCliqueMap: { "TAL-1": ["CLQ-1"], "TAL-2": ["CLQ-1"], "TAL-3": ["CLQ-1"] },
      },
    };
    let gotNull = false;
    for (let i = 0; i < 200; i++) {
      if (checkCliqueCrises(PROJ, s, makeRng(1 + i)) === null) {
        gotNull = true;
        break;
      }
    }
  ...
  it("returns null when clique is non-toxic", () => {
    ...
    s.relationships = {
      cliques: {
        cliques: { "CLQ-1": makeClique({ reputation: "cool" }) },
        memberCliqueMap: { "TAL-1": ["CLQ-1"], "TAL-2": ["CLQ-1"], "TAL-3": ["CLQ-1"] },
      },
    };
    for (let i = 0; i < 200; i++) {
      expect(checkCliqueCrises(PROJ, s, makeRng(1 + i))).toBeNull();
    }
  });
  ...
  it("uses clique name in description when clique exists", () => {
    ...
    s.relationships = {
      cliques: {
        cliques: { "CLQ-1": makeClique({ name: "The Drama Club", reputation: "toxic" }) },
        memberCliqueMap: { "TAL-1": ["CLQ-1"], "TAL-2": ["CLQ-1"], "TAL-3": ["CLQ-1"] },
      },
    };
    const r = tryTrigger((rng) => checkCliqueCrises(PROJ, s, rng));
  ...
  it("does NOT trigger drama when clique does not exist in cliques map", () => {
    ...
    s.relationships = {
      cliques: {
        cliques: {},
        memberCliqueMap: { "TAL-1": ["CLQ-999"], "TAL-2": ["CLQ-999"], "TAL-3": ["CLQ-999"] },
      },
    };
    for (let i = 0; i < 200; i++) {
      expect(checkCliqueCrises(PROJ, s, makeRng(1 + i))).toBeNull();
...
describe("generateRelationshipScandals", () => {
  ...
  it("generates scandal impacts for secret romance with spouse", () => {
    const talents = {
      "TAL-1": { ...createMockTalent({ id: "TAL-1", name: "Alice" }), spouseId: "TAL-3" },
      "TAL-2": createMockTalent({ id: "TAL-2", name: "Bob" }),
      "TAL-3": createMockTalent({ id: "TAL-3", name: "Carol" }),
    };
    const s = setupState(
      [
        makeRel({
  ...
  it("does not generate scandal for secret romance without spouse", () => {
    const talents = {
      "TAL-1": createMockTalent({ id: "TAL-1", name: "Alice" }),
      "TAL-2": createMockTalent({ id: "TAL-2", name: "Bob" }),
    };
    const s = setupState([makeRel({ type: "romantic", isPublic: false, strength: 80 })], talents, {
      week: 10,
    });
    for (let i = 0; i < 200; i++) {
      expect(generateRelationshipScandals(s, makeRng(1 + i))).toEqual([]);
    }
  });
  ...
  it("does not treat secret romance with strength <= 60 as secret romance", () => {
    ...
    const s = setupState([makeRel({ type: "romantic", isPublic: false, strength: 60 })], talents, {
      week: 10,
    });
    for (let i = 0; i < 200; i++) {
      expect(generateRelationshipScandals(s, makeRng(1 + i))).toEqual([]);
    ...
    const s = setupState(
      [
        makeRel({
          ...
          isPublic: true,
          strength: 50,
          history: [{ week: 8, type: "breakup", impact: -20, description: "Split" }],
        }),
      ],
      talents,
      { week: 10 }
    ...
    const s = setupState(
      [
        makeRel({
          ...
          isPublic: true,
          strength: 50,
          history: [{ week: 5, type: "breakup", impact: -20, description: "Split" }],
        }),
      ],
      talents,
      { week: 10 }
    ...
    const s = setupState(
      [
        makeRel({
          ...
          isPublic: true,
          strength: 50,
          history: [{ week: 5, type: "strengthened", impact: 10, description: "Stronger" }],
        }),
      ],
      talents,
      { week: 10 }
  ...
  it("ignores non-romantic relationships", () => {
    ...
    const s = setupState(
      [
        makeRel({ type: "friend", strength: 80, isPublic: false }),
        makeRel({ id: "R2", type: "rival", strength: -80, isPublic: true }),
      ],
      talents,
      { week: 10 }
    );
  ...
  it("processes both secret and public romances in a single call", () => {
    const talents = {
      ...
      "TAL-2": createMockTalent({ id: "TAL-2", name: "Bob" }),
      "TAL-3": createMockTalent({ id: "TAL-3", name: "Carol" }),
      "TAL-4": createMockTalent({ id: "TAL-4", name: "Dave" }),
    };
    const s = setupState(
      [
        makeRel({ type: "romantic", isPublic: false, strength: 80 }),
        makeRel({
          id: "R2",
          talentAId: "TAL-3",
          talentBId: "TAL-4",
          ...
          isPublic: true,
          strength: 50,
          history: [{ week: 9, type: "breakup", impact: -20, description: "Split" }],
        }),
      ],
      talents,
      { week: 10 }
    ...
    const hasBreakupNews = impacts.some(
      (x) => x.type === "NEWS_ADDED" && (x.payload as any).headline.includes("Power Couple Splits")
    );
...
describe("calculateSocialCrisisModifier", () => {
  ...
  it("adds 0.15 per feud relationship", () => {
    const s = setupState(
      [
        makeRel({ type: "rival", strength: -50 }),
        makeRel({ id: "R2", talentAId: "TAL-1", talentBId: "TAL-3", type: "enemy", strength: -60 }),
      ],
      {
        "TAL-1": createMockTalent({ id: "TAL-1" }),
        "TAL-2": createMockTalent({ id: "TAL-2" }),
  ...
  it("adds 0.10 per toxic clique member", () => {
    ...
    s.relationships = {
      ...
      cliques: {
        cliques: { "CLQ-1": makeClique({ reputation: "toxic" }) },
        memberCliqueMap: { "TAL-1": ["CLQ-1"], "TAL-2": ["CLQ-1"] },
      },
    };
  ...
  it("caps at 2.0 maximum", () => {
    ...
    for (let i = 0; i < 10; i++) {
      talents["T" + (i + 2)] = createMockTalent({ id: "T" + (i + 2) });
    }
    const s = setupState(rels, talents);
    const ct: Record<string, any> = {};
    for (let i = 0; i < 11; i++) {
      ct["C" + (i + 1)] = createMockContract({
        id: "C" + (i + 1),
        projectId: "PRJ-1",
        talentId: i === 0 ? "TAL-1" : "T" + (i + 1),
      });
    }
  ...
  it("only counts feuds between cast members", () => {
    const s = setupState(
      [makeRel({ talentAId: "TAL-1", talentBId: "TAL-9", type: "rival", strength: -50 })],
      {
        "TAL-1": createMockTalent({ id: "TAL-1" }),
        "TAL-2": createMockTalent({ id: "TAL-2" }),
        "TAL-9": createMockTalent({ id: "TAL-9" }),
      }
    );
    setContracts(s, "PRJ-1", "TAL-1", "TAL-2");
    expect(calculateSocialCrisisModifier("PRJ-1", s)).toBe(1.0);
