...
describe('FlopMechanics', () => {
  ...
  describe('calculateFlopPenalties', () => {
    ...
    it('calculates correct penalties for MAJOR severity', () => {
      const project = createMockProject({ budget: 1000000 } as unknown as import('../../../../engine/types').Project);
    ...
    it('handles negative budgets mathematically consistently', () => {
      const project = createMockProject({ budget: -1000000, marketingBudget: 0 } as unknown as import('../../../../engine/types').Project);
      const penalties = calculateFlopPenalties(project, FlopSeverity.CATASTROPHIC);
  ...
  describe('applyFlopPenalties', () => {
    ...
    it('applies penalties to RIVAL correctly for CATASTROPHIC flop and triggers restructuring if multiple major flops', () => {
      ...
      const rival = createMockRival({ id: rivalId, name: 'Rival Studio', cash: 1000000, prestige: 50 });
      state.entities.rivals[rivalId] = rival;

      const project = createMockProject({ id: 'p2', title: 'Rival Flop', revenue: 0, budget: 1000000 }); // 0 / 1000000 = 0 -> CATASTROPHIC
      ...
      const cashUpdate = rivalUpdates.find(i => (i.payload as { update?: { cash?: number } }).update?.cash !== undefined);
      ...
      const restructureUpdate = rivalUpdates.find(i => (i.payload as { update?: { strategy?: string } }).update?.strategy === 'prestige_chaser');
