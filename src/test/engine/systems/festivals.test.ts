...
describe('Festivals System', () => {
  ...
  describe('FESTIVAL_BY_BODY', () => {
    ...
    it('returns undefined for unknown body', () => {
      expect(FESTIVAL_BY_BODY.get('Toronto International Film Festival' as AwardBody)).toBeUndefined();
    });
  });
  ...
  it('declines submission when festival body is not in FESTIVALS', () => {
    const impact = submitToFestival(mockState, mockProject.id, 'Toronto International Film Festival' as AwardBody);
    expect(impact).toBeNull();
  });
  ...
  it('filters out rejected submissions older than 12 weeks', () => {
    ...
    expect(impact.newFestivalSubmissions?.some(s => s.id === 'sub-old-rej' && s.status === 'rejected')).toBe(true);
  });
