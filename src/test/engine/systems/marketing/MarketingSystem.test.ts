...
describe("tickMarketing", () => {
  ...
  it("respects diminishing returns after week 4", () => {
    const fresh = marketingProject({
      weeksInPhase: 1,
      marketingCampaign: {
        domesticBudget: 40_000_000,
        foreignBudget: 40_000_000,
        primaryAngle: "SELL_THE_SPECTACLE",
        weeksInMarketing: 1,
      },
    });
    const stale = marketingProject({
      id: "proj-2",
      weeksInPhase: 8,
      marketingCampaign: {
        domesticBudget: 40_000_000,
    ...
    const state = makeState({
      entities: {
        projects: { [fresh.id]: fresh, [stale.id]: stale },
        releasedProjectIds: [],
        talents: {},
        contracts: {},
        rivals: {},
    ...
    const byId = (id: string) =>
      (
        impacts.find((i) => i.type === "PROJECT_UPDATED" && (i.payload as any).projectId === id)!
          .payload as any
      ).update;
  ...
  it("secondary angle contributes to accrual", () => {
    const primaryOnly = marketingProject({
      id: "p1",
      marketingCampaign: {
        domesticBudget: 30_000_000,
        foreignBudget: 30_000_000,
        primaryAngle: "SELL_THE_SPECTACLE",
        weeksInMarketing: 1,
      },
    });
    const withSecondary = marketingProject({
      id: "p2",
      marketingCampaign: {
        domesticBudget: 30_000_000,
        foreignBudget: 30_000_000,
        primaryAngle: "SELL_THE_SPECTACLE",
        secondaryAngle: "SELL_THE_STARS",
        weeksInMarketing: 1,
      },
    });
    ...
    const state = makeState({
      entities: {
        projects: { [primaryOnly.id]: primaryOnly, [withSecondary.id]: withSecondary },
        releasedProjectIds: [],
        talents: {},
        contracts: {},
        rivals: {},
  ...
  it("share-of-voice reduces accrual when rivals outspend", () => {
    ...
    const highIntensity = makeState({
      market: {
        ...
        activeMarketEvents: [],
        buyers: [],
        marketingIntensity: 500_000_000,
      } as any,
      entities: {
        projects: { [project.id]: project },
        releasedProjectIds: [],
    ...
    const lowAware = (low.find((i) => i.type === "PROJECT_UPDATED")!.payload as any).update
      .awareness;
    const highAware = (high.find((i) => i.type === "PROJECT_UPDATED")!.payload as any).update
      .awareness;
  ...
  it("is a no-op when no active campaigns", () => {
    const state = makeState({
      entities: {
        projects: { x: marketingProject({ marketingCampaign: undefined }) },
        releasedProjectIds: [],
        talents: {},
        contracts: {},
        rivals: {},
  ...
  it("is deterministic for a fixed seed", () => {
    ...
    const build = () =>
      makeState({
        entities: {
          projects: { [project.id]: marketingProject() },
          releasedProjectIds: [],
          talents: {},
          contracts: {},
          rivals: {},
