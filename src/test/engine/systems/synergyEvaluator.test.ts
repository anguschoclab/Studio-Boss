...
describe("Synergy Evaluator", () => {
  ...
  const createMockAsset = (
    ...
    syndicationTier: "NONE" | "BRONZE" | "SILVER" | "GOLD" = "NONE"
  ): IPAsset => ({
    id,
  ...
  it("applies Built-in Audience bonus to films if the vault has syndicated TV assets", () => {
    const relatedAssets = [createMockAsset("ip-tv1", "SYNDICATED", "GOLD")];
  ...
  it("applies Multi-Format Prestige bonus if the brand spans Film and TV", () => {
    const assets = [createMockAsset("ip-film1", "NONE"), createMockAsset("ip-tv1", "SYNDICATED")];
