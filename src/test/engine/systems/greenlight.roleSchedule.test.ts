...
function talent(id: string, role: string, draw = 60): Talent {
  return { id, name: id, roles: [role], role: role as any, prestige: 60, fee: 1_000_000, draw, personality: 'Pro', accessLevel: 'outsider' } as any;
}
...
describe('roleCompletenessScore', () => {
  it('returns 100 when director, actor, and writer are filled', () => {
    const contracts = [contract('t1', 'director'), contract('t2', 'actor'), contract('t3', 'writer')];
    const talents = { t1: talent('t1', 'director'), t2: talent('t2', 'actor'), t3: talent('t3', 'writer') };
...
describe('scheduleCertainty', () => {
  ...
  it('is low when budget per production week is small', () => {
    ...
    expect(scheduleCertainty(baseProject({ budget: 5_000_000, productionWeeks: 24 }))).toBeLessThan(40);
  });
});
...
describe('evaluateGreenlight — role completeness + schedule certainty', () => {
  it('includes roleCompleteness and scheduleCertainty in the report', () => {
    ...
    const report = evaluateGreenlight(baseProject(), 100_000_000, [talent('t1', 'actor')], 10, [], contracts, talents);
  ...
  it('penalizes an incomplete role package', () => {
    const fullContracts = [contract('t1', 'actor'), contract('t2', 'director'), contract('t3', 'writer')];
    const fullTalents = { t1: talent('t1', 'actor'), t2: talent('t2', 'director'), t3: talent('t3', 'writer') };
    const full = evaluateGreenlight(
      baseProject(),
      100_000_000,
      [talent('t1', 'actor'), talent('t2', 'director'), talent('t3', 'writer')],
      10, [], fullContracts, fullTalents
    );
    ...
    const partial = evaluateGreenlight(
      ...
      100_000_000,
      [talent('t1', 'actor')],
      10, [], partialContracts, partialTalents
    );
  ...
  it('penalizes a missing key role in an otherwise strong package', () => {
    const fullContracts = [contract('t1', 'actor'), contract('t2', 'director'), contract('t3', 'writer')];
    const fullTalents = { t1: talent('t1', 'actor', 90), t2: talent('t2', 'director', 90), t3: talent('t3', 'writer', 90) };
    const full = evaluateGreenlight(
      baseProject(),
      200_000_000,
  ...
  it('engine report matches the UI derivation in GreenlightQueue', () => {
    ...
    const contracts = [contract('t1', 'director'), contract('t2', 'actor')];
    const talents = { t1: talent('t1', 'director'), t2: talent('t2', 'actor') };
    const report = evaluateGreenlight(baseProject(), 100_000_000, [talent('t1', 'director'), talent('t2', 'actor')], 10, [], contracts, talents);
