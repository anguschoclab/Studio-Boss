...
describe("StreamingViewershipTracker", () => {
  ...
  it("applies decay pattern correctly for binge releases", () => {
    const history: StreamingViewershipHistory = {
      ...
      startWeek: 1,
      entries: [
        { week: 1, viewers: 1000000, hoursWatched: 2500000, completionRate: 70, retention: 1.0 },
      ],
    ...
    const updated = StreamingViewershipTracker.updateViewership(history, 2, project, rng);
  ...
  it("improves completion rate over time due to word-of-mouth", () => {
    ...
    const updated = StreamingViewershipTracker.updateViewership(history, 4, project, rng);
