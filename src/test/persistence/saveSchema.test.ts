...
describe('saveSchema', () => {
  ...
  describe('safeJsonParse', () => {
    ...
    it('strips constructor key from top-level and nested objects', () => {
      const result = safeJsonParse('{"constructor":{"prototype":{"polluted":true}},"nested":{"constructor":{"harm":1}}}') as any;
    ...
    it('strips prototype key from top-level and nested objects', () => {
      const result = safeJsonParse('{"prototype":{"evil":true},"nested":{"prototype":{"evil":2}}}') as any;
    ...
    it('preserves all other keys and values intact', () => {
      const result = safeJsonParse('{"normal":"value","num":42,"bool":true,"nullVal":null,"arr":[1,2,3]}') as any;
  ...
  describe('validateSaveData', () => {
    ...
    it('rejects missing finance.cash', () => {
      const result = validateSaveData({ ...validState, finance: { ...validState.finance, cash: undefined } });
      expect(result.success).toBe(false);
    });

    it('rejects non-number finance.cash (string)', () => {
      const result = validateSaveData({ ...validState, finance: { ...validState.finance, cash: 'rich' } });
      expect(result.success).toBe(false);
    });

    it('rejects missing studio.id', () => {
      const result = validateSaveData({ ...validState, studio: { ...validState.studio, id: undefined } });
      expect(result.success).toBe(false);
    });

    it('rejects missing studio.name', () => {
      const result = validateSaveData({ ...validState, studio: { ...validState.studio, name: undefined } });
      expect(result.success).toBe(false);
    });

    it('rejects missing studio.archetype', () => {
      const result = validateSaveData({ ...validState, studio: { ...validState.studio, archetype: undefined } });
      expect(result.success).toBe(false);
    });

    it('rejects invalid studio.archetype value', () => {
      const result = validateSaveData({ ...validState, studio: { ...validState.studio, archetype: 'blockbuster' } });
      expect(result.success).toBe(false);
    });

    it('rejects missing studio.prestige', () => {
      const result = validateSaveData({ ...validState, studio: { ...validState.studio, prestige: undefined } });
      expect(result.success).toBe(false);
    });
    ...
    it('accepts optional fields (news, ip) when present', () => {
      const result = validateSaveData({ ...validState, news: { headlines: [] }, ip: { vault: [], franchises: {} } });
      expect(result.success).toBe(true);
    });
