...
function makeState(projects: Record<string, any>): GameState {
  return {
    ...
    news: { headlines: [] },
    ip: { vault: [], franchises: {} },
    entities: { projects, talents: {}, contracts: {}, rivals: { [RIVAL_ID]: { id: RIVAL_ID } as any } },
    studio: { id: 'PLR-1', name: 'Test', archetype: 'major', prestige: 50, internal: { projectHistory: [] } },
...
describe('RivalRevenueCalculator', () => {
  it('calculates zero revenue for studio with no projects', () => {
    ...
    const revenue = RivalRevenueCalculator.calculateWeeklyRevenue(rival, 1, MOCK_RNG, makeState({}));
  ...
  it('calculates theatrical revenue with decay', () => {
    ...
    const state = makeState({
      'p1': { id: 'p1', ownerId: RIVAL_ID, state: 'released', distributionStatus: 'theatrical', releaseWeek: 1,
  ...
  it('calculates streaming revenue for streaming projects', () => {
    const rival = { id: RIVAL_ID } as RivalStudio;
    const state = makeState({
      'p1': { id: 'p1', ownerId: RIVAL_ID, state: 'released', distributionStatus: 'streaming', reviewScore: 80, rating: 'TV-MA' } as any
    });
  ...
  it('calculates merch revenue for high-buzz projects', () => {
    const rival = { id: RIVAL_ID } as RivalStudio;
    const state = makeState({
      'p1': { id: 'p1', ownerId: RIVAL_ID, state: 'released', buzz: 85, franchiseId: 'franchise1', rating: 'PG' } as any
    });
    const revenue = RivalRevenueCalculator.calculateWeeklyRevenue(rival, 1, MOCK_RNG, state);
  ...
  it('applies rating premium to streaming revenue', () => {
    ...
    const stateTVMA = makeState({
      'p1': { id: 'p1', ownerId: RIVAL_ID, state: 'released', distributionStatus: 'streaming', reviewScore: 70, rating: 'TV-MA' } as any
    });
    const rivalPG13 = { id: RIVAL_ID } as RivalStudio;
    const statePG13 = makeState({
      'p1': { id: 'p1', ownerId: RIVAL_ID, state: 'released', distributionStatus: 'streaming', reviewScore: 70, rating: 'PG-13' } as any
    });
    const revenueTVMA = RivalRevenueCalculator.calculateWeeklyRevenue(rivalTVMA, 1, MOCK_RNG, stateTVMA);
    const revenuePG13 = RivalRevenueCalculator.calculateWeeklyRevenue(rivalPG13, 1, MOCK_RNG, statePG13);
