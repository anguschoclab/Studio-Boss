...
describe("SyndicationRevenuePanel", () => {
  ...
  it("renders summary cards with correct data", () => {
    ...
    expect(
      screen.getByText((c) => c.includes("TOTAL REVENUE") || c.includes("Total Revenue"))
    ).toBeInTheDocument();
    expect(screen.getByText("$5.0M")).toBeInTheDocument();
    expect(
      screen.getByText((c) => c.includes("ACTIVE DEALS") || c.includes("Active Deals"))
    ).toBeInTheDocument();
    expect(screen.getByText("12")).toBeInTheDocument();
    expect(
      screen.getByText((c) => c.includes("TOP PERFORMING REGION") || c.includes("Top Region"))
    ).toBeInTheDocument();
  ...
  it("shows empty state when no syndication data", () => {
    ...
    expect(
      screen.getByText(
        (c) => (c.includes("NO") && c.includes("SYNDICATION")) || c.includes("No syndication")
      )
    ).toBeInTheDocument();
  });
});
