...
describe("DealsDesk", () => {
  ...
  it("renders main header and empty states", () => {
    ...
    expect(screen.getByText(/0 NETWORK/i)).toBeInTheDocument();
    ...
    vi.mocked(useGameStore).mockImplementation((selector) => {
      const state = {
        gameState: {
          ...
          market: {
            buyers: [
              generateMockBuyer("b1", "Netflix", "streamer", {
                type: "prestige",
                activeUntilWeek: 10,
              }),
  ...
  it("displays fit scores when projects and buyers are present", () => {
    ...
    vi.mocked(useGameStore).mockImplementation((selector) => {
      const state = {
        gameState: {
          ...
          market: {
            buyers: [generateMockBuyer("b1", "Buyer 1", "streamer")],
          },
        },
      };
      return selector(state as any);
    ...
    render(<DealsDesk />);
    expect(screen.getByText("ACQUISITION FIT ANALYSIS", { exact: false })).toBeInTheDocument();
