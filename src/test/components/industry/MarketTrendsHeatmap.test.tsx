...
vi.mock('@/components/shared/Heatmap', () => ({
  Heatmap: ({ data, rows, cols }: any) => (
    <div data-testid="heatmap" data-length={data.length} data-rows={rows.length} data-cols={cols.length}>
      {data.map((d: any) => (
        <div key={d.id} data-testid="heatmap-cell" data-row={d.row} data-col={d.col} data-value={d.value}>
          {d.tooltip}
        </div>
      ))}
    </div>
...
describe('MarketTrendsHeatmap', () => {
  ...
  it('pre-aggregates player project counts by genre', () => {
    ...
    render(<MarketTrendsHeatmap />);
    const actionCells = screen.getAllByTestId('heatmap-cell').filter(c => c.getAttribute('data-row') === 'Action');
    const comedyCells = screen.getAllByTestId('heatmap-cell').filter(c => c.getAttribute('data-row') === 'Comedy');
  ...
  it('pre-aggregates rival project counts by genre', () => {
    ...
    render(<MarketTrendsHeatmap />);
    const horrorCells = screen.getAllByTestId('heatmap-cell').filter(c => c.getAttribute('data-row') === 'Horror');
