...
vi.mock("@/engine/systems/finance", () => ({
  calculateWeeklyCosts: vi.fn((projects: any[]) =>
    projects.reduce((acc: number, p: any) => acc + (p.weeklyCost || 0), 0)
  ),
  calculateWeeklyRevenue: vi.fn((projects: any[]) =>
    projects.reduce((acc: number, p: any) => acc + (p.weeklyRevenue || 0), 0)
  ),
...
describe("FinancePanel Component", () => {
  ...
  it("renders gracefully with null/empty game state", () => {
    ...
    expect(
      screen.getAllByText((content) => content.includes("FISCAL INTELLIGENCE")).length
    ).toBeGreaterThan(0);
  });
