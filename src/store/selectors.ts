...
const DEFAULT_CULTURE = { genrePopularity: {} };
const DEFAULT_FINANCE = { bankBalance: 0, yearToDateRevenue: 0, yearToDateExpenses: 0 };
...
let lastProjectsRef: Record<string, Project> | null = null;
let lastProjectsArray: Project[] = EMPTY_PROJECTS;

export const selectProjects = (state: GameState | null) => {
  if (!state) return EMPTY_PROJECTS;
  const rawProjects = state.studio.internal.projects;
  if (rawProjects === lastProjectsRef) return lastProjectsArray;
  lastProjectsRef = rawProjects;
  lastProjectsArray = Object.values(rawProjects);
...
let lastTalentPoolRef: Record<string, TalentProfile> | null = null;
let lastTalentPoolArray: TalentProfile[] = EMPTY_TALENT;

export const selectTalentPool = (state: GameState | null) => {
  if (!state) return EMPTY_TALENT;
  const rawTalent = state.industry.talentPool;
  if (rawTalent === lastTalentPoolRef) return lastTalentPoolArray;
  lastTalentPoolRef = rawTalent;
  lastTalentPoolArray = Object.values(rawTalent);
...
// --- Epic 4 Selectors ---
export const selectCulture = (state: GameState | null) => state?.culture || DEFAULT_CULTURE;
export const selectFinance = (state: GameState | null) => state?.finance || DEFAULT_FINANCE;
