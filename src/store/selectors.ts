...
export const selectCash = createSelector([selectFinance], (finance): number => finance.cash || 0);
...
export const selectRevenueBreakdown = createSelector([selectFinance], (finance) => {
...
export const selectWeeklyRevenueHistory = createSelector([selectFinance], (finance) =>
  (finance.weeklyHistory || []).map((h) => h.net)
);
...
export const selectRivals = createSelector([selectRivalsRaw], (rivals): RivalStudio[] =>
  Object.values(rivals)
);
...
export const selectReleasedProjects = createSelector([selectProjects], (projects) =>
  projects.filter(
...
export const selectRecoupmentStatus = createSelector([selectProjects], (projects) =>
  projects
...
export const selectStudioSuccess = createSelector([selectReleasedProjects], (released) => {
...
export const selectBoxOfficeData = createSelector([selectReleasedProjects], (released) =>
  released
    .filter((p) => !!p.boxOffice)
    .map((p) => ({
      title: p.title,
      total: p.revenue,
      roi: p.accumulatedCost > 0 ? (p.revenue / p.accumulatedCost) * 100 : 0,
      opening:
        (p.boxOffice?.openingWeekendDomestic || 0) + (p.boxOffice?.openingWeekendForeign || 0),
    }))
);

export const selectProductionSlippage = createSelector([selectProjects], (projects) =>
  projects
    .filter((p) => p.state === "production" && p.weeksInPhase > p.productionWeeks)
    .map((p) => ({
      title: p.title,
...
export const selectRecentEvents = createSelector([selectEventHistory], (history): GameEvent[] =>
...
export const selectTalentSatisfaction = createSelector([selectTalentPool], (talents) => {
  ...
  return {
    ...
    byCategory: ["Actor", "Director", "Writer", "Star"].map((cat) => ({
      category: cat,
      score: avgMood,
    })),
  };
...
export const selectTalentTierDistribution = createSelector([selectTalentPool], (talents) => {
  const list = Object.values(talents);
  const distribution = [
...
export const selectDealStats = createSelector([selectOpportunities], (opps) => {
...
export const selectCrisisRiskLevel = createSelector([selectProjects], (projects) => {
  ...
  return {
    ...
    activeThreats: activeCrises.map((p) => ({
      title: p.title,
      threat: p.activeCrisis!.description,
    })),
  };
});
      ...
      return {
        ...
        roi:
          genreProjects.length > 0
            ? genreProjects.reduce((sum, p) => sum + p.revenue / p.accumulatedCost, 0) /
