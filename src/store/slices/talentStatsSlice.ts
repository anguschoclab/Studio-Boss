...
export interface TalentStatsSlice {
  getTalentFilmography: (
    talentId: TalentId
  ) => import("@/engine/types/talent.types").Talent["filmography"];
...
export const createTalentStatsSlice: StateCreator<GameStore, [], [], TalentStatsSlice> = (
  set,
  get
) => ({
  getTalentFilmography: (talentId) => {
    ...
    return (talent?.filmography ||
      []) as import("@/engine/types/talent.types").Talent["filmography"];
  },
