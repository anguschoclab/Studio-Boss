...
export interface TalentSlice {
  addTalent: (talent: Omit<Partial<TalentProfile>, 'id'>) => string;
...
export const createTalentSlice: StateCreator<GameStore, [], [], TalentSlice> = (set, get) => ({
  ...
  addTalent: (talent) => {
    ...
    set((s) => {
      ...
      const newTalent = { ...talent, id: newId } as any;
      return {
        gameState: {
          ...s.gameState,
          industry: {
            ...
            talentPool: {
              ...s.gameState.industry.talentPool,
              [newId]: newTalent
            }
          }
        }
  ...
  updateTalent: (id, updates) => {
    set((s) => {
      if (!s.gameState || !s.gameState.industry.talentPool[id]) return s;
      return {
        gameState: {
          ...s.gameState,
          industry: {
            ...
            talentPool: {
              ...s.gameState.industry.talentPool,
              [id]: { ...s.gameState.industry.talentPool[id], ...updates }
            }
          }
        }
      };
  ...
  removeTalent: (id) => {
    set((s) => {
      if (!s.gameState || !s.gameState.industry.talentPool[id]) return s;
      const newPool = { ...s.gameState.industry.talentPool };
      delete newPool[id];
      return {
        gameState: {
          ...s.gameState,
  ...
  getTalentFilmography: (talentId) => {
    ...
    const talent = Object.values(s.gameState.industry.talentPool).find(t => t.id === talentId);
