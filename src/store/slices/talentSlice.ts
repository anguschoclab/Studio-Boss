...
export const createTalentSlice: StateCreator<GameStore, [], [], TalentSlice> = (set, get) => ({
  signContract: (talentId, projectId) => {
    set((s) => {
      ...
      const contract: Contract = {
        ...
        creativeControl:
          talent.accessLevel === "dynasty" || talent.prestige > 85 ? true : undefined,
      ...
      const newIndex = addContractToIndex(
        state.entities.contractsByProjectId,
        projectId,
        contract.id
      );
      const newTalentIndex = addContractToTalentIndex(
  ...
  removeTalentFromProject: (talentId, projectId) => {
    set((s) => {
      ...
      if (project.state === "production") {
        penalty = Math.floor(project.budget * 0.2);
      }
  ...
  acquireOpportunity: (oppId) => {
    set((s) => {
      ...
      if (params.format !== "unscripted") {
        const roles: CharacterArchetype[] = [
          "protagonist",
          "antagonist",
          "mentor",
          "love_interest",
        ];
        const activeRoles: CharacterArchetype[] = roles.slice(
          0,
          project.budgetTier === "blockbuster" ? 4 : 3
        );
      ...
      newContracts.forEach((c) => {
        contracts[c.id as ContractId] = c;
      });
  ...
  placeBid: (oppId, amount) => {
    set((s) => {
      ...
      const bidHistory = [
        ...(opp.bidHistory || []),
        { rivalId: studioId, amount, week: state.week },
      ];
