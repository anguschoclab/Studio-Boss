 ...
 */
export const selectCashFlowTrends = (
  ...
  return history.slice(-weeks).map((snapshot) => {
    ...
    return {
      week: snapshot.week,
      revenue:
        snapshot.revenue.theatrical +
        snapshot.revenue.streaming +
        snapshot.revenue.merch +
        snapshot.revenue.passive,
 ...
 */
export const selectBudgetBurnReport = (
  state: GameState | null,
  projectId: string
): BudgetBurnReport | null => {
 ...
 */
export const selectProjectTimelineData = (
  state: GameState | null,
  weeks: number = 12
 ...
 */
export const selectScriptQualityMetrics = (
  state: GameState | null,
  projectId: string
): ScriptQualityData | null => {
  const project = selectProjectsRaw(state)[projectId];
  if (!project) return null;
 ...
 */
export const selectScriptQualityReport = (
  state: GameState | null,
  projectId: string
): ScriptQualityReport | null => {
...
export const selectMarketShareData = (state: GameState | null): MarketSharePoint[] => {
  ...
  return [
    {
      name: "Your Studio",
      share: totalRevenue > 0 ? (myRevenue / totalRevenue) * 100 : 0,
      isPlayer: true,
    },
    ...rivalRevenues.map((r) => ({
      name: r.name,
 ...
 */
export const selectStreamingViewership = (
  state: GameState | null,
  platformName: string
): StreamingViewershipEntry[] => {
  const projects = selectProjects(state).filter(
    (p) => p.distributionStatus === "streaming" && p.buyerId === platformName
  );
...
export const selectStudioHealthMetrics = (state: GameState | null): StudioHealthMetric[] => {
  ...
  const onTrackProjects = activeProjects.filter((p) => {
    const onTime =
      (p.progress || 0) >= ((p.weeksInPhase || 0) / Math.max(p.productionWeeks || 1, 1)) * 100;
    const onBudget = (p.accumulatedCost || 0) <= (p.budget || 0) * 1.1;
    return onTime && onBudget;
  });
  const projectScore =
    activeProjects.length > 0 ? (onTrackProjects.length / activeProjects.length) * 100 : 70;
