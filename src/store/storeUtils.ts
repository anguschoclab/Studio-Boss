...
export function buildProjectAndContracts(
  state: GameState,
  params: CreateProjectParams
): { project: Project; newContracts: Contract[]; talentFees: number } {
  ...
  const { talentFees, newContracts } = prepareTalentAndContracts(
    state,
    params.attachedTalentIds,
    projectId
  );
 ...
 */
export function applyStateImpact(
  state: GameState,
  impacts: StateImpact | StateImpact[]
): GameState {
