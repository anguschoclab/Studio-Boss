  ...
  return (
    <TooltipWrapper tooltip="VIEW STRATEGIC DOSSIER" side="right">
      <div
        ...
        className="w-full text-left p-8 rounded-2xl border border-white/5 bg-white/[0.01] backdrop-blur-3xl hover:bg-white/[0.04] hover:border-primary/40 transition-all duration-700 space-y-8 group relative overflow-hidden cursor-pointer shadow-2xl"
      >
        {/* Visual Accent */}
        <div className={cn(
          ...
          hasUnresolvedCrisis ? "bg-red-400 shadow-[0_0_30px_rgba(244,63,94,0.4)]" : "bg-primary shadow-[0_0_30px_rgba(var(--primary),0.4)]"
        )} />

        {/* Header: Title & Format */}
        <div className="flex items-start justify-between gap-6 relative z-10">
          <div className="min-w-0 space-y-2">
            <h4 className="font-display font-black text-xl text-foreground/90 uppercase tracking-tighter italic truncate group-hover:text-primary transition-all duration-700 leading-none drop-shadow-[0_0_10px_rgba(255,255,255,0.05)]">{project.title}</h4>
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-black text-muted-foreground/20 uppercase tracking-[0.3em] group-hover:text-muted-foreground/60 transition-all duration-700 italic">{project.genre.toUpperCase()}</span>
              <span className="text-[10px] text-muted-foreground/10">•</span>
              <span className="text-[10px] font-black text-muted-foreground/20 uppercase tracking-[0.3em] group-hover:text-muted-foreground/60 transition-all duration-700 italic">{tier.label.toUpperCase()}</span>
            </div>
          </div>
        ...
        <div className="space-y-6 relative z-10">
            ...
            <TooltipWrapper tooltip="MARKET ANTICIPATION INDEX" side="top">
              <div className="space-y-3">
                <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground/30 group-hover:text-muted-foreground/60 transition-all duration-700 italic">
                  <span className="flex items-center gap-2"><Target className="h-3 w-3 group-hover:text-amber-400 transition-colors" strokeWidth={3} /> MARKET BUZZ</span>
                  <span className="text-amber-400 font-display font-black italic tracking-tighter text-sm drop-shadow-[0_0_10px_rgba(251,191,36,0.3)]">{Math.round(project.buzz)}%</span>
                </div>
          ...
          {(project.state === 'development' || project.state === 'production') && (
            <TooltipWrapper tooltip={`CURRENT PHASE: ${project.state.toUpperCase()}`} side="top">
              <div className="space-y-3">
                <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground/30 group-hover:text-muted-foreground/60 transition-all duration-700 italic">
                  <span className="flex items-center gap-2"><Activity className={cn("h-3 w-3 transition-colors", hasUnresolvedCrisis ? "group-hover:text-red-400" : "group-hover:text-primary")} strokeWidth={3} /> {project.state.replace('_', ' ').toUpperCase()}</span>
                  <span className="font-display font-black italic tracking-tighter text-sm">{project.weeksInPhase}/{project.state === 'development' ? project.developmentWeeks : project.productionWeeks}W</span>
                </div>
                <div className="h-2 bg-black/60 rounded-none overflow-hidden border border-white/5 p-[1px]">
                  <div
                    className={cn(
                      "h-full transition-all duration-1000 relative",
                      hasUnresolvedCrisis ? "bg-red-400 shadow-[0_0_20px_rgba(244,63,94,0.4)]" : "bg-primary shadow-[0_0_20px_rgba(var(--primary),0.4)]"
                    )}
            ...
            <div className="space-y-3 border-t border-white/5 pt-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <DistributionBadge status={project.distributionStatus} className="rounded-none shadow-[0_0_15px_rgba(255,255,255,0.05)]" />
                  <span className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/40 truncate max-w-[120px] italic">{buyer.name.toUpperCase()}</span>
                </div>
                {weeklyRevenueForecast > 0 && (
                  <TooltipWrapper tooltip="PROJECTED WEEKLY FISCAL FLOW" side="top">
                    <div className="flex items-center gap-2 bg-emerald-400/5 px-3 py-1.5 border border-emerald-400/20 shadow-[0_0_20px_rgba(16,185,129,0.1)]">
                      ...
                      <span className="text-[11px] font-display font-black italic text-emerald-400 tracking-tighter">{formatMoney(weeklyRevenueForecast).toUpperCase()}/WK</span>
                    </div>
                  </TooltipWrapper>
                )}
              </div>
          ...
          {(project.state === 'released' || project.state === 'archived') && (
            <RecoupmentStatus project={project} className="p-5 bg-black/40 rounded-none border border-white/5" />
          )}
        </div>
