...
import { Plus, LayoutGrid, ListFilter, Search, Clapperboard, Layers } from 'lucide-react';
...
const COLUMNS: { status: ProjectStatus[]; title: string; color: string; description: string }[] = [
  { status: ['development', 'needs_greenlight'], title: 'DEVELOPMENT', color: 'bg-amber-400', description: 'SCRIPTS & CONCEPT' },
  { status: ['pitching'], title: 'PITCHING', color: 'bg-primary', description: 'DISTRIBUTION DEALS' },
  { status: ['production', 'marketing'], title: 'ACTIVE SLATE', color: 'bg-emerald-400', description: 'PRODUCTION & PR' },
  { status: ['released', 'post_release', 'archived'], title: 'CATALOG', color: 'bg-muted-foreground/20', description: 'RELEASE & LEGACY' },
];
    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-1000 h-full flex flex-col">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 bg-white/[0.02] p-10 rounded-none border border-white/5 backdrop-blur-3xl relative overflow-hidden shadow-2xl">
        ...
        <div className="flex items-center gap-6 relative z-10">
          ...
          <Button variant="ghost" size="icon" aria-label="Filter properties" className="h-12 w-12 rounded-none bg-white/5 border border-white/5 text-muted-foreground/40 hover:text-primary hover:border-primary/40 transition-all duration-700">
            ...
            return (
              <div key={col.title} className="flex flex-col h-full space-y-6 group/col">
                ...
                <div className="flex items-center justify-between px-6 py-4 rounded-none bg-white/[0.02] border border-white/5 group-hover/col:bg-white/[0.04] group-hover/col:border-white/10 transition-all duration-700 relative overflow-hidden">
                  <div className={cn("absolute inset-y-0 left-0 w-2 opacity-40 transition-all duration-700 group-hover/col:opacity-80 group-hover/col:w-3", col.color)} />
                  <div className="flex items-center gap-4 pl-4">
                    <div className="space-y-1">
                      <h3 className="text-[12px] font-display font-black uppercase tracking-[0.3em] text-foreground italic">{col.title}</h3>
                      <p className="text-[9px] font-black text-muted-foreground/20 uppercase tracking-[0.3em] italic">{col.description}</p>
                    </div>
                  </div>
