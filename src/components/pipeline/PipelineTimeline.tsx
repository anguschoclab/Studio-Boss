...
const statusColors: Record<ProjectStatus, { bg: string; border: string; text: string }> = {
  ...
  needs_greenlight: { bg: 'bg-amber-500/10', border: 'border-amber-500/30', text: 'text-amber-500' },
...
export const PipelineTimeline: React.FC<PipelineTimelineProps> = ({
  ...
  if (timelineItems.length === 0) {
    return (
      <div className={cn("flex items-center justify-center h-32 text-muted-foreground bg-white/[0.01] border border-white/5", className)}>
        <p className="text-[10px] font-black uppercase tracking-[0.3em] italic opacity-20">NO ACTIVE VENTURES DETECTED</p>
      </div>
    );
  }
  ...
  return (
    <div className={cn("w-full overflow-x-auto custom-scrollbar bg-white/[0.01] border border-white/5 p-6", className)}>
      <div 
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between h-full relative z-10">
                <span className="text-[10px] font-display font-black uppercase italic tracking-tighter truncate leading-none group-hover:text-white transition-colors">{project.title}</span>
