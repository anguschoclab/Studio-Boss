...
export const TelevisionDashboard = () => {
  const activeProjects = useGameStore(
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full py-20 text-center opacity-30">
        ...
        <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mt-2">
          The small screen is currently dark.
        </p>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 bg-white/5 p-5 rounded-none border border-white/5 backdrop-blur-sm">
        <div>
          ...
          <p className="text-[11px] font-black uppercase text-muted-foreground/60 tracking-[0.2em]">
            Episodic Production • Weekly Ratings • Renewal Watch
          </p>
        </div>
      </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                {details.averageRating > 8 && (
                  <Badge
                    variant="outline"
                    className="text-[9px] font-black border-primary/40 text-primary uppercase bg-primary/5"
                  >
                    CRITICAL DARLING
                  </Badge>
                )}
              </div>
              <h3 className="text-xl font-black tracking-tighter uppercase group-hover:text-primary transition-colors">
                {show.title}
              </h3>
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-0.5">
                S{details.currentSeason} • {show.format.toUpperCase()}
              </p>
            </div>
            <div className="text-right">
              <div className="text-[9px] font-black text-muted-foreground uppercase tracking-widest mb-1 leading-none">
                Rating
              </div>
        ...
        <div className="px-6 py-4 grid grid-cols-3 gap-4 border-y border-white/5 bg-white/2">
          <div className="space-y-1">
            ...
            <div className="text-sm font-black tracking-tight">
              {details.episodesAired} / {details.episodesOrdered}
            </div>
          </div>
          <div className="space-y-1">
            <div className="text-[9px] font-black text-muted-foreground uppercase tracking-widest flex items-center gap-1.5">
