...
export const CausalityTooltip: React.FC<CausalityProps> = ({
      ...
      <div className="space-y-2">
        ...
        {causality.map((entry, idx) => (
          <div key={`${entry.factor}-${idx}`} className="flex justify-between items-start text-xs">
            ...
            <span
              className={`font-mono shrink-0 font-bold ${entry.magnitude >= 0 ? "text-emerald-400" : "text-rose-400"}`}
            >
              {entry.magnitude > 0 ? "+" : ""}
              {formatMoney(entry.magnitude)}
            </span>
          </div>
        ))}
      </div>
  ...
  return (
    <TooltipWrapper tooltip={tooltipContent} side="right" align="center">
