...
export const GenreTrendLine: React.FC<GenreTrendLineProps> = ({
  ...
  const getTrendIcon = () => {
    switch (trend) {
      case "rising":
        return <TrendingUp className="h-4 w-4 text-emerald-400" strokeWidth={2.5} />;
      case "cooling":
        return <TrendingDown className="h-4 w-4 text-red-400" strokeWidth={2.5} />;
      default:
        return <Activity className="h-4 w-4 text-muted-foreground/20" strokeWidth={2.5} />;
    }
  };
  ...
  return (
    <div
      className={cn(
        "glass-card p-6 rounded-none group transition-all duration-700 hover:bg-white/[0.03]",
        className
      )}
    >
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <span className="font-display font-black text-lg italic tracking-tighter uppercase leading-none">
            {genre}
          </span>
          {getTrendBadge()}
        </div>
        <div className="flex items-baseline gap-1.5">
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/10 italic">
            / 100
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-[9px] font-black uppercase tracking-[0.3em] italic">
        ...
        >
          {change > 0 ? "+" : ""}
          {change} <span className="text-muted-foreground/20">THIS WEEK</span>
        </span>
