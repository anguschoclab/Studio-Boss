            ...
            {xLabels.map((xLabel) => {
              ...
              return (
                <div
                  key={`${xLabel}-${yLabel}`}
                  className={cn(
                    "w-16 h-10 flex-shrink-0 flex items-center justify-center text-[9px] font-medium cursor-pointer transition-all hover:ring-2 hover:ring-primary focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none",
                    onCellClick && "cursor-pointer"
                  )}
                  role="button"
                  aria-label={`Heatmap cell ${xLabel} ${yLabel}`}
                  tabIndex={0}
                  onKeyDown={(e) => {
