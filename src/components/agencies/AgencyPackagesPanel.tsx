  ...
  if (agencies.length === 0) {
    return (
      ...
      >
        ...
        <p className="text-sm font-black uppercase italic tracking-[0.3em] text-muted-foreground/40 leading-none">
          NO AGENCY DATA DETECTED
        </p>
    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-1000">
            ...
            >
              <div className="flex items-start justify-between mb-6">
                <div className="space-y-1">
                  <span className="font-display font-black text-xs uppercase italic tracking-widest text-foreground group-hover:text-primary transition-colors leading-none block">
                    {agency.name}
                  </span>
                  <span className="text-[8px] font-black uppercase tracking-[0.2em] text-muted-foreground/30 italic block">
                    {agency.archetype || "INSTITUTION"}
                  </span>
                </div>
                ...
                <div className="h-1 bg-white/5 rounded-none overflow-hidden">
                  <div
                    className={cn(
                      ...
                      (agency.relationshipScore ?? 50) > 70
                        ? "bg-primary"
                        : (agency.relationshipScore ?? 50) > 40
                          ? "bg-amber-500"
                          : "bg-rose-500"
                    )}
              ...
              <div className="flex items-center justify-between mt-6 pt-4 border-t border-white/5 text-[8px] font-black uppercase tracking-[0.2em] italic">
                ...
                <span className="text-muted-foreground/20">LEVERAGE: {agency.leverage}%</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="space-y-6">
        ...
        {packages.length === 0 ? (
          <div
            className={cn(
              "text-center py-16 bg-white/[0.01] border border-white/5 border-dashed rounded-none"
            )}
          >
                ...
                <div className="flex items-start justify-between mb-8 relative z-10">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-4 mb-2">
                      <h4 className="font-display font-black text-lg uppercase italic tracking-tight text-foreground group-hover:text-secondary transition-colors leading-none">
                        {pkg.title}
                      </h4>
                  ...
                  <div className="bg-secondary/10 border border-secondary/20 px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-secondary italic shadow-xl flex items-center gap-2">
                    <Star className="h-3.5 w-3.5 fill-current" />+{pkg.qualityBonus || 0} QUALITY
                    MOD
                  </div>
                </div>
