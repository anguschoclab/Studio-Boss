...
export const LiveAuctionDashboard: React.FC<LiveAuctionDashboardProps> = ({
  ...
  const currentMaxBid = useMemo(() => {
    // ⚡ Bolt: Replace Object.values().map() chain with direct for...in loop for O(1) GC overhead max find
        ...
        <div className="p-8 border-b border-white/5 bg-black/40 flex items-center justify-between">
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 rounded-none bg-amber-500/10 border border-amber-500/20 flex items-center justify-center group">
              <Gavel
                className={cn(
                  "w-8 h-8 text-amber-500 transition-transform duration-500",
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <Badge
                  variant="outline"
                  className="text-[10px] font-black uppercase bg-amber-500/10 text-amber-500 border-none"
                >
                  {opportunity.genre}
                </Badge>
                ...
                <div className="p-6 rounded-none bg-black/40 border border-white/5 space-y-1">
                  ...
                  <div
                    className={cn(
                      ...
                      isLeading ? "text-emerald-400" : "text-rose-400"
                    )}
                  >
                    {isLeading ? (
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[9px] font-black text-slate-500 uppercase">
                  Available: {formatMoney(gameState?.finance.cash || 0)}
                </span>
              </div>
              ...
              <div className="p-8 rounded-none bg-slate-800/20 border-2 border-white/5 space-y-8">
                <div className="space-y-4">
                  ...
                  <Progress
                    value={(currentMaxBid / (playerBid || 1)) * 100}
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[0.05, 0.15, 0.25].map((pct) => (
                      <Button
                        ...
                        onClick={() =>
                          setPlayerBid(Math.round((currentMaxBid * (1 + pct)) / 1000) * 1000)
                        }
                      >
                        +{pct * 100}% Aggression
                      </Button>
                ...
                <Button
                  ...
                  onClick={handleBid}
                  disabled={
                    playerBid <= currentMaxBid ||
                    (gameState ? gameState.finance.cash < playerBid : true)
                  }
                >
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {history.map((entry, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-3 rounded-none bg-white/3 border border-white/5 animate-in slide-in-from-right-2"
                    ...
                    <div className="flex flex-col">
                      ...
                      <span
                        className={cn(
                          ...
                          entry.rivalId === "PLAYER" ? "text-primary" : "text-slate-300"
                        )}
                      >
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-none bg-amber-500/5 border border-amber-500/10 space-y-3">
                <p className="text-[11px] font-bold text-amber-200/60 leading-relaxed italic">
                  "Rival studios see high value in this {opportunity.genre} package. We expect
                  aggressive counter-plays. Closing bid estimate is{" "}
                  {formatMoney(opportunity.costToAcquire * 2.5)}."
                </p>
