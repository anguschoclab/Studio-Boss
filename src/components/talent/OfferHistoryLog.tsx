...
export const OfferHistoryLog: React.FC<OfferHistoryLogProps> = ({ offers, onViewOffer }) => {
  ...
  const renderOfferCard = (offer: OfferHistory) => (
    <Card key={offer.offerId} className={cn("p-4", tokens.border.default)}>
      <div className="flex items-center justify-between mb-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <TalentNameLink
              talentId={offer.talentId}
              name={offer.talentName}
      ...
      <div className="space-y-2 mb-3">
        {offer.actions.map((action, idx) => (
          <div key={idx} className="flex items-center gap-2 text-[10px]">
            ...
            {action.note && <span className="italic text-muted-foreground">"{action.note}"</span>}
          </div>
        ))}
      </div>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {pending.length > 0 && (
        <Section
          ...
          icon={Clock}
        >
          <div className="space-y-3">{pending.map(renderOfferCard)}</div>
        </Section>
      )}

      {/* Offer History */}
      <Section title="Offer History" subtitle="Past negotiations and outcomes" icon={History}>
        {resolved.length === 0 ? (
          <div
            className={cn("text-center py-8", tokens.border.default, "border-dashed rounded-none")}
          >
            <History className="h-10 w-10 mx-auto mb-3 opacity-20" />
            <p className={cn("text-sm", tokens.text.caption)}>No offer history yet</p>
          </div>
        ) : (
          <div className="space-y-3">{resolved.map(renderOfferCard)}</div>
        )}
      </Section>
    </div>
  );
