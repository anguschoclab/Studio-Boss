  ...
  return (
    <div className="space-y-4 h-full flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <div className="flex items-center justify-between pb-2 border-b border-border/40">
        <h2 className="text-2xl font-display font-black tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-foreground to-foreground/70 drop-shadow-sm">
          Talent Roster
        </h2>
        <div className="flex gap-2 flex-wrap justify-end">
          {(["all", "actor", "director", "writer", "producer"] as (TalentRole | "all")[]).map(
            (type) => (
              <TooltipWrapper
                key={type}
                tooltip={`Filter by ${type === "all" ? "all professional roles" : `the ${type} category`}`}
                side="bottom"
              >
                <button
                  aria-pressed={filter === type}
                  onClick={() => setFilter(type)}
                  className={`px-3.5 py-1.5 text-[10px] uppercase tracking-wider font-black rounded-none transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 ring-offset-background ${
                    filter === type
