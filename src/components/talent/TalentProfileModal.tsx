...
export const TalentModal = () => {
  ...
  const talent = useMemo(
    () => talentPool.find((t) => t.id === selectedTalentId),
    [talentPool, selectedTalentId]
  );
      ...
      <DialogContent className="max-w-4xl bg-black border-white/5 text-slate-100 shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col h-[90vh] p-0 rounded-none">
        ...
        <div className="flex-1 overflow-y-auto px-10 pt-8 pb-10 custom-scrollbar">
          <Tabs defaultValue="bio" className="w-full">
                    ...
                    <div className="bg-amber-500/5 p-6 border border-amber-500/10 rounded-none shadow-xl">
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-4">
                        <div className="bg-amber-950/20 p-2 rounded-none text-center border border-amber-500/10">
                          <p className="text-[8px] font-black text-amber-500/60 uppercase">
                            Prestige
                          </p>
                          <p className="text-sm font-black text-amber-200">
                            {family.prestigeLegacy}
                          </p>
                        </div>
                        <div className="bg-amber-950/20 p-2 rounded-none text-center border border-amber-500/10">
                          <p className="text-[8px] font-black text-amber-500/60 uppercase">
                      ...
                      <p className="text-[10px] text-amber-200/40 leading-relaxed font-bold uppercase tracking-tight mt-3">
                        A recognized lineage in the Hollywood hierarchy. Transitioning from{" "}
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {talent.knownFor?.map((title, i) => (
                  <div
                    key={i}
                    className="group relative glass-panel p-8 rounded-none hover:border-primary/40 transition-all duration-500 text-center overflow-hidden"
                  >
                    ...
                    <h4 className="text-2xl font-black tracking-tighter text-white uppercase italic truncate mb-2">
                      {title}
                    </h4>
                    <Badge className="bg-slate-800 text-slate-400 border-slate-700 uppercase text-[10px] font-black">
                      LEGACY HIT
                    </Badge>
                  </div>
                ))}
                {(!talent.knownFor || talent.knownFor.length === 0) && (
                  <div className="col-span-3 py-20 text-center border-2 border-dashed border-white/5 rounded-none">
                    <p className="text-slate-500 font-black uppercase tracking-widest italic opacity-40">
                      No Significant Hits Recorded
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-6 mt-4">
                <div className="bg-black/40 p-6 rounded-none border border-white/5">
                  <h5 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">
                    Highest Movie Payday
                  </h5>
                  {talent.highestSalaryMovie ? (
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-black text-white italic">
                          {talent.highestSalaryMovie.project}
                        </p>
                        <p className="text-[10px] text-slate-500 font-bold">
                          {talent.highestSalaryMovie.year}
                        </p>
                      </div>
                      <p className="text-2xl font-black text-emerald-400">
                        {formatMoney(talent.highestSalaryMovie.amount)}
                      </p>
                    </div>
                  ) : (
                ...
                <div className="bg-black/40 p-6 rounded-none border border-white/5">
                  ...
                  {talent.highestSalaryTv ? (
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-black text-white italic">
                          {talent.highestSalaryTv.project}
                        </p>
                        <p className="text-[10px] text-slate-500 font-bold">
                          {talent.highestSalaryTv.year}
                        </p>
                      </div>
                      <p className="text-2xl font-black text-indigo-400">
                        {formatMoney(talent.highestSalaryTv.amount)}
                      </p>
                    </div>
                  ) : (
                    <p className="text-xs italic text-slate-600">No data recorded</p>
            ...
            <TabsContent
              value="filmography"
              className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300 focus-visible:outline-none"
            >
              <div className="bg-black/80 border border-white/5 rounded-none overflow-hidden shadow-2xl">
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="bg-black/40 border-b border-white/5">
                      <th className="px-8 py-5 font-black uppercase tracking-widest text-slate-500 text-[10px]">
                        Year
                      </th>
                      <th className="px-8 py-5 font-black uppercase tracking-widest text-slate-500 text-[10px]">
                        Title
                      ...
                      <th className="px-8 py-5 font-black uppercase tracking-widest text-slate-500 text-[10px] text-right">
                        Performance
                      </th>
                    </tr>
                  </thead>
                    ...
                    {talent.filmography?.map((f, i) => (
                      <tr key={i} className="hover:bg-primary/5 transition-colors group">
                        ...
                        <td className="px-8 py-5">
                          <p className="font-black text-white uppercase italic tracking-tight text-base group-hover:text-primary transition-colors">
                            {f.title}
                          </p>
                        </td>
                        <td className="px-8 py-5">
                          <Badge
                            variant="outline"
                            className="text-[9px] border-white/5 bg-black/60 text-slate-400 font-black uppercase tracking-widest px-2 py-0.5"
                          >
                        ...
                        <td className="px-8 py-5 text-right">
                          <div className="flex flex-col">
                            <span className="font-black text-emerald-500 italic text-sm">
                              {f.type === "movie" ? formatMoney(f.gross) : "N/A"}
                            </span>
                            <span className="text-[9px] text-slate-500 font-black uppercase tracking-widest">
                              Pay: {formatMoney(f.salary)}
                            </span>
                          </div>
                        </td>
                      </tr>
