...
export const StreamingPanel: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    const totalWeeklyGrowth = streamers.reduce((sum, s) => {
      const lastCount = s.subscriberHistory?.length
        ? s.subscriberHistory[s.subscriberHistory.length - 2]?.count || s.subscribers
        : s.subscribers;
  ...
  const activeBuyers = useMemo(
    () =>
      buyers.filter((b) => !b.acquiredBy).sort((a, b) => (b.strength ?? 60) - (a.strength ?? 60)),
    [buyers]
  );
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 bg-white/[0.02] p-10 rounded-none border border-white/5 backdrop-blur-3xl relative overflow-hidden group shadow-2xl">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-6 mb-3">
            ...
            <div>
              <h1 className="text-5xl font-display font-black tracking-tighter uppercase italic leading-none mb-3 drop-shadow-[0_0_30px_rgba(255,255,255,0.1)]">
                GLOBAL DISTRIBUTION INTEL
              </h1>
        ...
        <div className="flex gap-10 relative z-10 shrink-0">
          <TooltipWrapper
            tooltip="NET CHANGE IN TOTAL PLATFORM SUBSCRIBERS ACROSS ALL MONITORED DIGITAL NETWORKS."
            side="top"
          >
            <div className="text-right cursor-help space-y-2">
              <div className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground/40 italic">
                WEEKLY GROWTH
              </div>
              <div
                className={cn(
                  "text-3xl font-display font-black tabular-nums leading-none italic",
                  stats.totalWeeklyGrowth >= 0 ? "text-emerald-400" : "text-red-500"
                )}
              >
                {stats.totalWeeklyGrowth >= 0 ? "+" : ""}
                {formatSubscribers(stats.totalWeeklyGrowth)}
              </div>
            </div>
          </TooltipWrapper>
          <div className="w-[1px] h-12 bg-white/10 self-center" />
          <TooltipWrapper
            tooltip="NUMBER OF INDEPENDENT MEDIA ENTITIES CURRENTLY OPERATING DIGITAL DISTRIBUTION PLATFORMS."
            side="top"
          >
            <div className="text-right cursor-help space-y-2">
              <div className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground/40 italic">
              ...
              <div className="text-3xl font-display font-black text-foreground/90 leading-none italic">
                {stats.activeCount}{" "}
                <span className="text-[12px] text-muted-foreground/20 italic tracking-widest">
                  GLOBAL
                </span>
              </div>
            </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row gap-10">
        ...
        <div className="flex-1 flex flex-col space-y-6">
          <div className="flex items-center justify-between px-2">
            ...
            <div className="text-[9px] font-black uppercase tracking-[0.3em] bg-emerald-400/5 text-emerald-400 border border-emerald-400/20 px-4 h-6 flex items-center justify-center rounded-none italic shadow-[0_0_15px_rgba(52,211,153,0.1)]">
              MARKET OPEN
            </div>
          </div>
        ...
        <div className="w-full lg:w-[400px] flex flex-col shrink-0">
          ...
          <div className="glass-card rounded-none border border-white/5 flex flex-col h-full min-h-[500px] overflow-hidden group/intel bg-white/[0.01] shadow-2xl">
            <div className="p-8 border-b border-white/5 bg-white/[0.02] flex items-center justify-between">
              <TooltipWrapper
                tooltip="CHRONOLOGICAL AUDIT OF PLATFORM ACQUISITIONS, MERGERS, AND MARKET CONSOLIDATION EVENTS."
                side="left"
              >
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div className="p-6 rounded-none bg-black/40 border border-white/5 text-center group-hover/intel:border-red-500/20 transition-all duration-700 shadow-xl">
                        <div className="text-3xl font-display font-black text-red-500 italic leading-none mb-2">
                          {stats.mergers}
                        </div>
                        <div className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground/40 italic">
                          MERGERS
                      ...
                      <div className="p-6 rounded-none bg-black/40 border border-white/5 text-center group-hover/intel:border-violet-500/20 transition-all duration-700 shadow-xl">
                        <div className="text-3xl font-display font-black text-violet-500 italic leading-none mb-2">
                          {acquiredPlatforms.length}
                        </div>
                        <div className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground/40 italic">
                          ABSORBED
                        </div>
                    ...
                    {acquiredPlatforms.length > 0 ? (
                      <div className="space-y-4">
                        {acquiredPlatforms.map((p) => (
                          <div
                            key={p.id}
                            className="flex items-center justify-between p-6 rounded-none bg-white/[0.02] border border-white/5 group/plat hover:border-primary/20 transition-all duration-700 shadow-lg"
                          >
                            <div className="min-w-0">
                              <div className="text-xs font-black uppercase tracking-[0.2em] text-foreground/90 truncate italic mb-1">
                                {p.name}
                              </div>
                              <div className="text-[9px] font-black text-muted-foreground/20 uppercase tracking-[0.3em] italic">
                                ABSORBED BY {p.parentCompany?.toUpperCase()}
                              </div>
                            </div>
                            <Button
                              variant="ghost"
            ...
            <div className="p-6 border-t border-white/5 bg-white/[0.01] text-center">
              <p className="text-[8px] font-black text-muted-foreground/20 uppercase tracking-[0.5em] italic">
