...
const DEAL_TYPE_TOOLTIPS = {
  ...
  deficit:
    "Higher back-end revenue share in exchange for a lower upfront license fee. High risk, high reward.",
  upfront:
    "Higher upfront payment to cover production costs at the expense of back-end participation.",
};
...
export const DealModal: React.FC<DealModalProps> = ({ buyer, open, onClose }) => {
  ...
  const eligibleProjects = useMemo(() => {
    ...
    return Object.values(gameState.studio.internal.projects).filter(
      (p: Project) =>
        p.state === "development" || p.state === "production" || p.state === "needs_greenlight"
    );
  }, [gameState]);
  ...
  const fitScore = useMemo(() => {
    ...
    return calculateFitScore(
      ...
      buyer,
      gameState.week,
      Object.values(gameState.studio.internal.projects)
    );
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-black uppercase tracking-tighter italic">
                Studio Distribution Negotiation
              </h3>
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                {buyer.name} • Executive Review
              </p>
            </div>
          </div>
                ...
                <div className="space-y-2">
                  <h4
                    className={cn(
                      ...
                      dealResult.success ? "text-emerald-500" : "text-rose-500"
                    )}
                  >
                  ...
                  <Button
                    onClick={onClose}
                    className="mt-4 h-9 text-[10px] font-black uppercase"
                    variant="outline"
                  >
                    Back to Dashboard
                  </Button>
            ...
            <>
                  ...
                  <div className="space-y-2 max-h-[240px] overflow-y-auto pr-1">
                    {eligibleProjects.map((p) => (
                      ...
                      >
                        ...
                        <div className="font-black text-xs uppercase truncate group-hover:text-primary transition-colors">
                          {p.title}
                        </div>
                        <div className="text-[9px] font-bold text-muted-foreground/60 uppercase tracking-widest mt-0.5">
                          {p.genre} • {p.budgetTier}
                        </div>
                      </button>
                    ))}
                  </div>
                ...
                <div className="space-y-4">
                  <div className="glass-panel p-5 rounded-none border border-white/5 bg-black/20 space-y-4">
                    <div className="flex items-center justify-between">
                      <TooltipWrapper
                        tooltip="Algorithmic alignment score between your project's genre/talent and the platform's current executive mandate."
                        side="top"
                      >
                        <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground cursor-help">
                          Strategic Fit
                        </span>
                      </TooltipWrapper>
                      <span
                    ...
                    <p className="text-[10px] font-bold text-muted-foreground italic leading-snug">
                      {selectedProjectObj
                        ? fitScore >= 50
                          ? `${buyer.name} is showing positive early interest in this genre.`
                          : `${buyer.name} is hesitant about this package's commercial floor.`
              ...
              <div className="pt-4 border-t border-white/5 flex gap-3">
                <Button
                  variant="ghost"
                  onClick={onClose}
                  className="h-12 px-6 text-[10px] font-black uppercase"
                >
                  Cancel
                </Button>
                <Button
                ...
                >
                  {isPitching ? (
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 border-2 border-black/30 border-t-black rounded-none animate-spin" />{" "}
                      Reviewing Package...
                    </div>
                  ) : (
                    "Send Official Pitch"
