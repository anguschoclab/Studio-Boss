...
export const AchievementsPanel: React.FC = () => {
  ...
  const unlockedIds: string[] = useGameStore(
    (s) => (s.gameState?.studio as unknown as Record<string, unknown>)?.achievements ?? EMPTY_IDS
  );
  ...
  const filtered =
    activeCategory === "all" ? enriched : enriched.filter((a) => a.category === activeCategory);
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="flex items-center justify-between pb-8 border-b border-white/5">
        <div className="flex items-center gap-8">
          ...
          <div className="space-y-2">
            <h3 className="text-3xl font-display font-black tracking-tighter uppercase italic leading-none">
              ACHIEVEMENTS
            </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filtered.map((a) => {
          const colorClass =
            CATEGORY_COLORS[a.category] ?? "text-primary border-primary/30 bg-primary/5";
          return (
            <div
              key={a.id}
              className={cn(
