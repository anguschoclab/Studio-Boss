      ...
      <TooltipWrapper tooltip={`Current Year: ${year}, Week: ${displayWeek}. Business quarters cycle every 13 weeks.`}>
        <div className="flex items-center gap-6 cursor-default group">
          <div className="flex flex-col">
            <span className="text-[9px] text-muted-foreground/30 font-black uppercase tracking-[0.4em] leading-none mb-2 italic group-hover:text-primary transition-colors">FISCAL_PERIOD</span>
            <div className="flex items-center gap-3">
              <span className="font-display font-black text-xl tracking-tighter uppercase italic text-foreground">W{displayWeek}</span>
              <div className="w-1.5 h-1.5 bg-white/10 rotate-45" />
              <span className="font-display font-black text-xl tracking-tighter uppercase italic text-muted-foreground/40">Y{year}</span>
            </div>
          </div>
          <div className="h-10 w-px bg-white/5" />
          <div className="flex flex-col">
             <span className="text-[9px] text-muted-foreground/20 font-black uppercase tracking-[0.4em] leading-none mb-2 italic">QUARTER</span>
             <span className="font-display font-black text-sm text-primary/60 italic">Q{(Math.floor((displayWeek-1)/13) + 1)}</span>
          </div>
        </div>
      </TooltipWrapper>
      ...
      <div className="flex items-center gap-10 ml-auto">
        ...
        <TooltipWrapper tooltip="Total liquid capital available for acquisitions and project funding.">
          <div className="flex flex-col items-end cursor-default group">
            <span className="text-[9px] text-muted-foreground/30 font-black uppercase tracking-[0.4em] leading-none mb-2 italic group-hover:text-primary transition-colors">LIQUID_CAPITAL</span>
            <div className="flex items-center gap-3">
               {cashDelta !== 0 && (
                 <span className={cn("text-[10px] font-display font-black italic", cashDelta > 0 ? "text-emerald-500" : "text-rose-500")}>
                   {cashDelta > 0 ? '+' : ''}{formatMoney(cashDelta).replace('$', '').toUpperCase()}
                 </span>
               )}
               <span className={cn("font-display font-black text-2xl tracking-tighter italic", cash < 0 ? 'text-rose-500' : 'text-primary drop-shadow-[0_0_15px_rgba(var(--primary),0.3)]')}>
        ...
        <TooltipWrapper tooltip="Studio reputation level. High prestige unlocks elite talent and better distribution terms.">
          <div className="flex flex-col items-end cursor-default group">
            <span className="text-[9px] text-muted-foreground/30 font-black uppercase tracking-[0.4em] leading-none mb-2 italic group-hover:text-secondary transition-colors">PRESTIGE</span>
            <div className="flex items-center gap-3">
              <Star className="h-4 w-4 text-secondary/40 group-hover:text-secondary transition-colors" fill="currentColor" />
        ...
        <TooltipWrapper tooltip="Total number of properties currently in active development or production phases.">
          <div className="flex flex-col items-end cursor-default group">
            <span className="text-[9px] text-muted-foreground/30 font-black uppercase tracking-[0.4em] leading-none mb-2 italic group-hover:text-foreground transition-colors">ACTIVE_SLATE</span>
            <div className="flex items-center gap-3">
