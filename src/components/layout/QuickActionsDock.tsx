 ...
 */
export const QuickActionsDock: React.FC<QuickActionsDockProps> = ({ className }) => {
  ...
  return (
    <div className={cn("fixed bottom-12 right-12 z-40 flex flex-col items-end gap-4", className)}>
      {/* Expanded actions */}
      <AnimatePresence>
        {isExpanded && (
          <motion.div
                  ...
                  <div
                    className={cn(
                      ...
                      action.variant === "secondary" &&
                        "bg-secondary text-black shadow-[0_0_20px_rgba(var(--secondary),0.4)]",
                      (!action.variant || action.variant === "default") &&
                        "bg-white/5 text-foreground/40 group-hover:text-primary group-hover:bg-primary/10"
                    )}
                  >
      ...
      >
        <Zap
          className={cn("w-5 h-5 transition-transform duration-700", isExpanded && "rotate-180")}
          strokeWidth={3}
        />
