...
const eventTypeConfig: Record<string, { icon: any, color: string, label: string }> = {
  AWARD: { icon: Icons.Trophy, color: 'text-amber-400', label: 'Awards' },
  CRISIS: { icon: Icons.AlertTriangle, color: 'text-destructive', label: 'Crisis' },
  RELEASE: { icon: Icons.TrendingUp, color: 'text-emerald-400', label: 'Release' },
  STUDIO_EVENT: { icon: Icons.History, color: 'text-blue-400', label: 'Studio' },
  RIVAL: { icon: Icons.Zap, color: 'text-red-400', label: 'Rival' },
  GENERAL: { icon: Icons.Newspaper, color: 'text-primary', label: 'News' },
};
  ...
  if (!headlines || headlines.length === 0) {
    return (
      <div className="flex-1 flex items-center gap-3 px-4 py-1.5 bg-white/5 rounded-full border border-white/5 overflow-hidden">
        <Icons.Newspaper className="h-3.5 w-3.5 text-primary shrink-0" />
    ...
    <>
      <div className="flex-1 max-w-2xl hidden xl:flex items-center gap-3 px-4 py-1.5 bg-white/5 rounded-full border border-white/5 overflow-hidden group relative">
        <div className="flex items-center gap-2 bg-background/80 backdrop-blur-md px-2 z-20 border-r border-white/10 relative -ml-1 h-4 shrink-0">
          <Icons.Newspaper className="h-3 w-3 text-primary shrink-0" />
