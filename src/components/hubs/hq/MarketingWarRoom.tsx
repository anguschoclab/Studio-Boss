...
export const MarketingWarRoom: React.FC<MarketingWarRoomProps> = ({ projectId, onClose }) => {
  ...
  const project = useMemo(() => gameState?.entities.projects[projectId], [gameState, projectId]);

  const [campaignType, setCampaignType] = useState<"marketing" | "awards">("marketing");
  const [selectedTier, setSelectedTier] = useState<string>("Standard");
  const [selectedQuadrant, setSelectedQuadrant] = useState<AudienceQuadrant>(
  ...
  const handleLaunch = () => {
    if (campaignType === "marketing") {
      launchMarketingCampaign(
        projectId,
        selectedTier as "Standard" | "Tentpole" | "Saturation",
        selectedAngle,
        selectedQuadrant
      );
    } else {
          ...
          <div className="flex items-center gap-10">
            ...
            {onClose && (
              <Button
                variant="ghost"
                size="icon"
                aria-label="Close Marketing War Room"
                tooltip="Close"
                onClick={onClose}
                className="h-12 w-12 hover:bg-white/5 border border-transparent hover:border-white/10 rounded-none"
              >
          ...
          <div className="col-span-7 space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-1000">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Button
                  ...
                  className={cn(
                    "h-16 font-black uppercase italic tracking-widest",
                    campaignType === "marketing"
                      ? "bg-primary text-black shadow-[0_0_20px_rgba(var(--primary),0.2)]"
                      : "bg-white/[0.02] border-white/5 hover:border-white/20 text-muted-foreground/40"
                  )}
                  onClick={() => {
            ...
            {campaignType === "marketing" && (
              <div className="space-y-4">
                ...
                <Select
                  value={selectedAngle}
                  onValueChange={(v) => setSelectedAngle(v as MarketingAngle)}
                >
                  <SelectTrigger
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isSelected
                          ? "bg-primary/10 border-primary shadow-[0_0_20px_rgba(var(--primary),0.1)] scale-[1.02] z-10"
                    ...
                    >
                      ...
                      {isSelected && (
                        <Target
                          className="absolute top-4 right-4 w-3 h-3 text-primary animate-pulse"
                          strokeWidth={3}
                        />
                      )}
                    </button>
              ...
              <div className="h-1.5 bg-black/60 rounded-none overflow-hidden border border-white/5">
                <div
                  className={cn(
                    ...
                    currentResonance >= 1.2 ? "bg-primary" : "bg-white/40"
                  )}
