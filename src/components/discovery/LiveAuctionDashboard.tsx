...
export const LiveAuctionDashboard: React.FC<LiveAuctionDashboardProps> = ({
  opportunity: opp,
  onClose,
}) => {
      ...
      <div className="flex items-center justify-between gap-4 p-5 rounded-none bg-gradient-to-br from-primary/10 via-black/40 to-transparent border border-primary/20 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center gap-3">
            <Badge
              variant="outline"
              className="text-[9px] font-black uppercase tracking-widest bg-white/5 border-white/5 h-5"
            >
              {opp.genre}
            </Badge>
            <Badge
        ...
        <div className="text-right relative z-10">
          <div className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground leading-none mb-1.5">
            Highest Bid
          </div>
          <div
            className={cn(
          ...
          <div className="flex items-center justify-end gap-1.5 mt-1">
            ...
            <span
              className={cn(
                ...
                isPlayerWinning ? "text-emerald-400" : "text-primary"
              )}
            >
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    {
                      label: "+1M Match",
                      amount: currentHighest + 1_000_000,
                      color: "bg-white/5 hover:bg-white/10",
                    },
                    {
                      label: "+5M Power",
                      amount: currentHighest + 5_000_000,
                      color: "bg-white/5 hover:bg-white/10",
                    },
                    {
                      label: "10M Strategic",
                      amount: currentHighest + 10_000_000,
                      color: "bg-white/5 hover:bg-white/10",
                    },
                    {
                      label: "Aggressive 20M+",
                      amount: currentHighest + 25_000_000,
                    ...
                    <Button
                      ...
                      className={cn(
                        "h-12 flex flex-col items-center justify-center gap-0.5 border-white/5 group/btn transition-all active:scale-95",
                        btn.color
                      )}
                    >
                      <span className="text-[9px] font-black uppercase tracking-widest leading-none text-muted-foreground group-hover/btn:text-foreground">
                        {btn.label}
                      </span>
                      <span className="text-[11px] font-bold text-foreground">
                        {formatMoney(btn.amount)}
                      </span>
                    </Button>
                  ))}
                </div>
              ...
              <div className="p-4 rounded-none bg-black/20 border border-white/5 space-y-4">
                ...
                <div className="h-1.5 rounded-none bg-white/5 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (currentHighest / (gameState?.finance.cash || 1)) * 100)}%`,
                    }}
                  />
                </div>
              ...
              <div className="mt-8">
                {isPlayerWinning ? (
                  ...
                  <div className="p-4 rounded-none bg-rose-500/10 border border-rose-500/20 text-center">
                    <p className="text-xs font-black uppercase tracking-widest text-rose-500 mb-1">
                      Auction Closed
                    </p>
                    <p className="text-[10px] font-bold text-muted-foreground uppercase leading-tight">
                      Rights acquired by {highestBidder?.name}
                    </p>
                    <Button
                      variant="outline"
                      size="sm"
                      className="mt-3 w-full h-8 text-[9px] font-black uppercase bg-white/5"
                      onClick={onClose}
                    >
                      Dismiss
                    </Button>
        ...
        <div className="flex flex-col space-y-4">
              ...
              {[40, 60, 45, 90, 70, 85, 30, 95, 65, 80].map((h, i) => (
                <div
                  ...
                  className={cn(
                    ...
                    h > 80
                      ? "bg-rose-400/80 shadow-[0_0_10px_rgba(251,113,133,0.4)]"
                      : h > 50
                        ? "bg-amber-400/60"
                        : "bg-white/10"
                  )}
          ...
          <div className="glass-panel rounded-none border border-white/5 overflow-hidden flex flex-col h-full min-h-[300px]">
            <div className="p-4 bg-white/5 border-b border-white/5 flex items-center justify-between">
              ...
              <Badge variant="outline" className="text-[8px] font-black uppercase h-4 px-1.5">
                {opp.bidHistory.length} ENTRIES
              </Badge>
            </div>
              ...
              {[...(opp.bidHistory || [])].reverse().map((bid, i) => (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <div
                      className={cn(
                        "w-6 h-6 rounded-none flex items-center justify-center text-[10px] font-black",
                        bid.rivalId === playerStudioId || bid.rivalId === "PLAYER"
                    ...
                    <div>
                      <div className="text-[10px] font-black uppercase tracking-tight">
                        {bid.rivalId === playerStudioId || bid.rivalId === "PLAYER"
                          ? "YOUR STUDIO"
                          : rivalsMap[bid.rivalId]?.name || "RIVAL"}
                      </div>
