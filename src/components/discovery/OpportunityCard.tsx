...
export const OpportunityCard = ({ opportunity: opp, onEnterAuction }: OpportunityCardProps) => {
  ...
  const highestBid = opp.highestBidderId
    ? opp.highestBidderId === "PLAYER"
      ? "YOU"
      : "CONGLOMERATE"
    : "STARTING";
      ...
      <div className="glass-card p-10 rounded-none group relative overflow-hidden flex flex-col justify-between h-full border border-white/5 hover:border-secondary/40 hover:bg-secondary/[0.02] transition-all duration-700 cursor-pointer active:scale-[0.99] bg-white/[0.01] shadow-2xl">
        ...
        <div className="space-y-8 relative z-10">
          <div className="flex justify-between items-start gap-4">
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-black uppercase text-muted-foreground/30 tracking-[0.3em] italic">
          ...
          <div className="flex items-center gap-4">
            <div className="text-[9px] font-black border border-white/10 bg-white/[0.02] text-muted-foreground/40 uppercase px-3 h-6 flex items-center justify-center tracking-[0.2em] rounded-none italic">
              {opp.format.toUpperCase()}
            </div>
            <div className="text-[9px] font-black border border-primary/20 bg-primary/5 text-primary uppercase px-3 h-6 flex items-center justify-center tracking-[0.2em] rounded-none italic shadow-[0_0_10px_rgba(var(--primary),0.1)]">
              {opp.budgetTier.toUpperCase()} BUDGET
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-8 pt-4">
            <div className="space-y-2">
              ...
              <div className="text-2xl font-display font-black text-foreground italic tabular-nums leading-none drop-shadow-[0_0_10px_rgba(255,255,255,0.1)]">
                {formatMoney(maxBid)}
              </div>
            </div>
            ...
            <div className="space-y-2 text-right">
              ...
              <div
                className={cn(
                  "text-xs font-black uppercase tracking-[0.3em] italic leading-none drop-shadow-[0_0_10px_rgba(var(--color),0.3)]",
                  highestBid === "YOU" ? "text-emerald-400" : "text-red-500"
                )}
              >
                {highestBid}
              </div>
        ...
        <div className="mt-10 pt-10 border-t border-white/5 flex justify-between items-center relative z-10">
          <div className="flex flex-col gap-2">
            <div className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground/20 flex items-center gap-3 italic leading-none">
              <Clock
                className={cn(
                  "h-3.5 w-3.5",
                  opp.weeksUntilExpiry <= 1 ? "text-red-500 animate-pulse" : "text-secondary/20"
                )}
                strokeWidth={3}
              />
              CLOSING SOON
            ...
            <span
              className={cn(
                "text-[11px] font-black uppercase tracking-[0.2em] italic leading-none",
                opp.weeksUntilExpiry <= 1 ? "text-red-500" : "text-foreground/80"
              )}
            >
          ...
          >
            <Gavel
              className="h-4 w-4 mr-3 group-hover/btn:rotate-[-45deg] transition-transform duration-700"
              strokeWidth={3}
            />
            ENTER WAR
          </Button>
