...
export const PiracyImpactMonitor: React.FC<PiracyImpactMonitorProps> = ({
  ...
  const highRiskProjects = projects.filter(
    (p) => p.estimatedRevenueLoss > 100000 || p.totalDownloads > 50000
  );
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {project.byRegion.slice(0, 4).map((region) => (
                    <div key={region.region} className="p-2 bg-muted/30 rounded text-[10px]">
                      ...
                      <Badge className={cn("text-[8px] mt-1", getSeverityColor(region.severity))}>
                        {region.severity}
                      </Badge>
                    </div>
                  ))}
      ...
      {/* Active Protections */}
      <Section title="Active Protections" subtitle="Anti-piracy measures in place" icon={Lock}>
        <div className="flex flex-wrap gap-2">
          ...
          {activeProtections.length === 0 && (
            <p className={cn("text-sm", tokens.text.caption)}>No active anti-piracy protections</p>
          )}
        </div>
      </Section>
      ...
      {projects.length === 0 && (
        <div
          className={cn("text-center py-12", tokens.border.default, "border-dashed rounded-none")}
        >
