...
export const FinancePanel = () => {
  ...
  const projectsMemo = useMemo(() => {
    const arr: (typeof projectsObj)[string][] = [];
    for (const pid in projectsObj) {
      arr.push(projectsObj[pid]);
    }
    return arr;
  }, [projectsObj]);
  ...
  const studioNetWorth = useMemo(
    () => (gameState ? calculateStudioNetWorth(gameState) : 0),
    [gameState]
  );
  const forecast = useMemo(
    () => (gameState ? generateCashflowForecast(gameState, 12) : []),
    [gameState]
  );
  ...
  const chartData = useMemo(() => {
    ...
    const history = financeHistory.map((h) => ({
      ...
      histCosts:
        h.expenses.production +
        h.expenses.marketing +
        h.expenses.burn +
        h.expenses.interest +
        h.expenses.royalties,
      ...h.revenue,
      ...h.expenses,
    }));
    ...
    <div className="space-y-16 animate-in fade-in slide-in-from-bottom-8 duration-1000 pb-32">
      <div className="flex items-center justify-between border-b border-white/5 pb-16">
        <div className="flex items-center gap-10">
          ...
          <div className="space-y-3">
            ...
            <p className="text-[10px] font-black uppercase text-muted-foreground/20 tracking-[0.5em] italic">
              TREASURY MANAGEMENT // PREDICTIVE FISCAL ANALYSIS
            </p>
          </div>
        </div>
            ...
            <div className="mt-16 space-y-10 overflow-y-auto max-h-[calc(100vh-350px)] pr-8 custom-scrollbar">
              {(snapshots || [])
                ...
                .map((s, i) => (
                  <div
                    key={i}
                    className="bg-white/[0.01] border border-white/5 p-10 group hover:bg-white/[0.04] hover:border-primary/40 transition-all duration-700 rounded-none shadow-xl"
                  >
                    <div className="flex justify-between items-start mb-10">
                      <div className="space-y-3">
                        <p className="text-[10px] text-muted-foreground/20 uppercase font-black tracking-[0.4em] leading-none italic">
                          FISCAL YEAR
                        </p>
                        <p className="text-6xl font-display font-black text-foreground italic leading-none drop-shadow-[0_0_10px_rgba(255,255,255,0.1)]">
                          Y{s.year}
                        </p>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-10">
                      <div className="space-y-4">
                        ...
                        <p className="text-2xl font-display font-black text-foreground tracking-tighter italic leading-none">
                          {formatMoney(s.funds).toUpperCase()}
                        </p>
                      </div>
                      <div className="space-y-4">
                        <p className="text-[10px] text-muted-foreground/20 uppercase font-black tracking-[0.4em] flex items-center gap-4 italic">
                        ...
                        <p className="text-2xl font-display font-black tracking-tighter text-foreground italic leading-none">
                          {s.completedProjects} TITLES
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              {snapshots.length === 0 && (
                <div className="py-32 flex flex-col items-center text-center space-y-8 opacity-20">
                  ...
                  <div className="space-y-3">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] max-w-[240px] leading-relaxed">
                      BENCHMARKS ARE GENERATED AT THE CLOSE OF EACH FISCAL YEAR (WEEK 52).
                    </p>
                  </div>
                </div>
              )}
      ...
      <Tabs defaultValue="current" className="space-y-16">
        <TabsList className="bg-white/[0.02] border border-white/5 p-1 h-16 rounded-none w-fit">
          <TabsTrigger
            value="current"
            className="gap-6 font-display font-black uppercase tracking-[0.3em] text-[10px] h-14 px-12 data-[state=active]:bg-primary data-[state=active]:text-black transition-all duration-700 rounded-none italic"
          >
        ...
        <TabsContent
          value="current"
          className="space-y-16 m-0 outline-none animate-in fade-in duration-1000"
        >
          {/* Summary Cards */}
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-10">
            <CausalityTooltip value={cash} causality={causality} label="AVAILABLE LIQUID CASH">
              <KPIStatCard
            ...
            />
            <CausalityTooltip value={netDelta} causality={causality} label="WEEKLY CASH BURN">
              <KPIStatCard
            ...
            <KPIStatCard
              ...
              value={formatMoney(
                forecast.length > 0 ? forecast[forecast.length - 1].projected : 0
              ).toUpperCase()}
              ...
              <div className="glass-card p-12 flex flex-col h-[500px] border border-white/5 rounded-none bg-black/40 shadow-2xl">
                <div className="flex items-center justify-between mb-16 pb-8 border-b border-white/5">
                  <div className="space-y-2">
                    <h3 className="text-3xl font-display font-black uppercase tracking-tighter italic leading-none text-foreground drop-shadow-[0_0_10px_rgba(255,255,255,0.05)]">
                      LIQUIDITY CURVE
                    </h3>
                    <p className="text-[10px] font-black uppercase text-muted-foreground/20 tracking-[0.4em] italic">
                      12-WEEK HISTORICAL VS 12-WEEK PROJECTED VECTOR
                    </p>
                  </div>
                  <div className="flex items-center gap-10 text-[9px] font-black tracking-[0.3em] uppercase text-muted-foreground/30 italic">
                    <div className="flex items-center gap-4">
                      <div className="w-3 h-3 rounded-none bg-primary shadow-[0_0_15px_rgba(var(--primary),0.5)]"></div>{" "}
                      HISTORY
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="w-5 h-0.5 bg-primary/40 border-t border-dashed border-primary"></div>{" "}
                      FORECAST
                    </div>
                  </div>
                </div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <YAxis
                        ...
                        fontSize={9}
                        fontWeight={900}
                        tickFormatter={(v) => formatMoney(v).toUpperCase()}
                        tickLine={false}
                        axisLine={false}
                      />
                      <Tooltip
                        ...
                        formatter={(value: number, name: string) => [
                          formatMoney(value).toUpperCase(),
                          name === "histCash" ? "ACTUAL CASH" : "PROJECTED CASH",
                        ]}
                      />
              ...
              <div className="glass-card flex flex-col h-full overflow-hidden border border-white/5 rounded-none bg-black/40 shadow-2xl">
                <div className="p-10 border-b border-white/5 bg-white/[0.01]">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/30 flex justify-between items-end italic">
                    ...
                    <span className="text-red-400 tracking-tighter text-2xl font-display font-black">
                      -{formatMoney(weeklyCosts).toUpperCase()}/WK
                    </span>
                  </h3>
                </div>
                  ...
                  <div className="flex flex-col divide-y divide-white/5">
                    {activeProjects.length > 0 ? (
                      activeProjects.map((p) => (
                        <div
                          key={p.id}
                          className="p-10 hover:bg-white/[0.03] transition-all duration-700 group"
                        >
                          <div className="flex items-center justify-between mb-6">
                            <span className="text-xl font-display font-black uppercase tracking-tight group-hover:text-primary transition-all duration-700 italic leading-none drop-shadow-[0_0_10px_rgba(255,255,255,0.05)]">
                              {p.title}
                            </span>
                            <span className="text-md font-display font-black text-red-400 tracking-tighter italic leading-none">
                              -{formatMoney(p.weeklyCost).toUpperCase()}
                            </span>
                          </div>
                      ...
                      <div className="py-32 flex flex-col items-center text-center space-y-8 opacity-20">
                        ...
                        <div className="space-y-3 px-12">
                          ...
                          <p className="text-[10px] font-black uppercase tracking-[0.2em] leading-relaxed">
                            YOUR STUDIO CURRENTLY HAS NO ACTIVE PROJECTS INCURRING WEEKLY PRODUCTION
                            COSTS.
                          </p>
                        </div>
                      </div>
          ...
          <div className="glass-card overflow-hidden border border-white/5 rounded-none bg-black/40 shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-white/5">
              {releasedProjects.length > 0 ? (
                releasedProjects.slice(0, 8).map((p) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className="p-10 hover:bg-white/[0.04] transition-all duration-700 group"
                    >
                      <div className="flex items-start justify-between mb-10">
                        <div className="space-y-4">
                          <h4 className="font-display font-black text-2xl tracking-tighter uppercase italic group-hover:text-primary transition-all duration-700 truncate max-w-[180px] leading-none drop-shadow-[0_0_10px_rgba(255,255,255,0.05)]">
                            {p.title}
                          </h4>
                        ...
                        <div
                          className={cn(
                            "text-[9px] font-black tracking-[0.4em] uppercase h-8 px-5 flex items-center border border-transparent rounded-none italic shadow-lg",
                            isProfitable
                      ...
                      <div className="space-y-8">
                        <div className="flex justify-between items-end">
                          <span className="text-[10px] font-black text-muted-foreground/10 uppercase tracking-[0.3em] italic">
                            GROSS YIELD
                          </span>
                          <span className="text-xl font-display font-black text-emerald-400 tracking-tighter italic leading-none drop-shadow-[0_0_10px_rgba(16,185,129,0.2)]">
                            {formatMoney(p.revenue).toUpperCase()}
                          </span>
                        </div>
                        <div className="flex justify-between items-end">
                          <span className="text-[10px] font-black text-muted-foreground/10 uppercase tracking-[0.3em] italic">
                          ...
                          <span className="text-xl font-display font-black text-red-400 tracking-tighter italic leading-none drop-shadow-[0_0_10px_rgba(244,63,94,0.2)]">
                            {formatMoney(p.budget + (p.marketingBudget || 0)).toUpperCase()}
                          </span>
                        </div>
                        <div className="pt-8 border-t border-white/5 flex justify-between items-center">
                          ...
                          <span
                            className={cn(
                              "text-4xl font-display font-black tracking-tighter italic leading-none",
                              isProfitable
                                ? "text-emerald-400 drop-shadow-[0_0_15px_rgba(16,185,129,0.3)]"
                                : "text-red-400 drop-shadow-[0_0_15px_rgba(244,63,94,0.3)]"
                            )}
                          >
                            {roi.toFixed(2)}x
                          </span>
                  ...
                  <div className="py-48 flex flex-col items-center text-center space-y-10 opacity-20">
                    ...
                    <div className="space-y-4 px-12">
                      ...
                      <p className="text-xs font-black uppercase tracking-[0.3em] leading-relaxed max-w-[400px]">
                        PROPERTIES WILL APPEAR HERE FOR ROI AUDITING ONCE THEY HAVE CONCLUDED THEIR
