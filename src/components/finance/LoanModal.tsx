...
const DebtWarningBanner = ({ isDebtWarning, totalWeeklyPayments }: DebtWarningBannerProps) => {
  if (!isDebtWarning) return null;
    ...
    <div className="flex items-start gap-3 rounded-none border border-destructive/40 bg-destructive/10 px-5 py-4">
      ...
      <div>
        ...
        <p className="mt-0.5 text-xs text-muted-foreground">
          Weekly loan payments ({formatMoney(totalWeeklyPayments)}/wk) exceed 10% of your average
          weekly revenue. Consider paying off loans early.
        </p>
      </div>
    </div>
  );
...
interface AvailableLoanOffersProps {
  offers: LoanOffer[];
  onTakeLoan: (amount: number, termWeeks: number) => void;
}

const AvailableLoanOffers = ({ offers, onTakeLoan }: AvailableLoanOffersProps) => {
  return (
          ...
          >
            ...
            <div className="relative z-10 space-y-1.5">
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-muted-foreground">
                <span>
                  <span className="font-bold text-foreground/70">Term:</span> {offer.termWeeks}{" "}
                  weeks
                </span>
                <span>
            ...
            <Button
              ...
              size="sm"
              onClick={() => onTakeLoan(offer.amount, offer.termWeeks)}
            >
              Take Loan
            </Button>
          </div>
...
interface ActiveLoansListProps {
  loans: Loan[];
  onRepayEarly: (id: string) => void;
}

const ActiveLoansList = ({ loans, onRepayEarly }: ActiveLoansListProps) => {
  return (
    <Card className="glass-card border-border/40">
      <CardHeader className="border-b border-border/30 pb-4">
        <CardTitle className="flex items-center gap-2 text-xs font-display font-black uppercase tracking-widest text-foreground/80">
...
export const LoanModal = () => {
  ...
  const isDebtWarning = avgWeeklyRevenue > 0 && totalWeeklyPayments > avgWeeklyRevenue * 0.1;

  if (!gameState) return null;

  return (
    <div className="space-y-6">
      <DebtWarningBanner isDebtWarning={isDebtWarning} totalWeeklyPayments={totalWeeklyPayments} />
      <AvailableLoanOffers offers={offers} onTakeLoan={addLoan} />
      <ActiveLoansList loans={loans} onRepayEarly={repayLoanEarly} />
    </div>
  );
};
