...
export const LoanModal = () => {
  ...
  const addLoan = useGameStore((s) => (s as { addLoan?: (amount: number, termWeeks: number) => void }).addLoan as (amount: number, termWeeks: number) => void);
  const repayLoanEarly = useGameStore((s) => (s as { repayLoanEarly?: (id: string) => void }).repayLoanEarly as (id: string) => void);

  const loanRate = gameState?.finance?.marketState?.loanRate ?? 0.08;
  const loans: Loan[] = useMemo(
    () => ((gameState?.studio as { loans?: Loan[] })?.loans || []) as Loan[],
    [gameState],
  );
