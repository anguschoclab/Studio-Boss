...
export const MarketRatesWidget: React.FC = () => {
  const finance = useGameStore((state) => state.gameState?.finance);
  const market = finance?.marketState;

  if (!market) return null;

  const isRising =
    market.rateHistory.length > 1 &&
    ...
    <div className="glass-card p-8 group relative overflow-hidden transition-all duration-700 hover:bg-white/[0.03] hover:border-primary/20 rounded-none">
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h3 className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-[0.3em] italic leading-none mb-3">
      ...
      <div className="grid grid-cols-2 gap-6 relative z-10">
        <div className="bg-white/[0.01] p-6 border border-white/5 rounded-none group-hover:bg-white/[0.02] transition-all duration-700">
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-[9px] font-black text-muted-foreground/20 uppercase tracking-[0.3em] italic leading-none">
              SAVINGS APY
            </span>
          </div>
          <p className="text-2xl font-display font-black text-emerald-400 italic leading-none tracking-tighter">
            {formatRate(market.savingsYield)}
          </p>
        </div>
        ...
        <div className="bg-white/[0.01] p-6 border border-white/5 rounded-none group-hover:bg-white/[0.02] transition-all duration-700">
          ...
          <p className="text-2xl font-display font-black text-red-400 italic leading-none tracking-tighter">
            {formatRate(market.debtRate)}
          </p>
        </div>
      </div>
