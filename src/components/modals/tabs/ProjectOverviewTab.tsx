    ...
    <div className="bg-black/40 p-5 rounded-none border border-white/5/50 space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {["male_under_25", "female_under_25", "male_over_25", "female_over_25"].map((q) => {
          const score = calculateAudienceIndex(
            project,
            q as import("@/engine/types").AudienceQuadrant
          );
          return (
              ...
              <div className="flex items-center justify-between">
                ...
                <div
                  className={cn(
                    ...
                    score > 1.2 ? "bg-emerald-500" : score > 0.8 ? "bg-amber-500" : "bg-rose-500"
                  )}
                />
              </div>
            </div>
      ...
      <div className="mt-8 p-6 bg-black/60 border border-white/5 rounded-none space-y-6 relative overflow-hidden group">
        <div className="absolute top-0 right-0 p-3">
          {project.reception.isCultPotential && (
            <Badge className="bg-fuchsia-600/20 text-fuchsia-400 border-fuchsia-600/50 animate-pulse font-black uppercase tracking-tighter">
              Cult Potential
            </Badge>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-12">
            ...
            <div className="h-2 bg-slate-800 rounded-none overflow-hidden">
              <div
                className={cn(
                  ...
                  project.reception.metaScore >= 75
                    ? "bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.5)]"
                    : project.reception.metaScore >= 40
                      ? "bg-amber-500"
                      : "bg-rose-500"
                )}
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-end">
              <span className="text-4xl font-black italic tracking-tighter text-white">
                {project.reception.audienceScore}
              </span>
          ...
          <p className="text-xs font-bold text-slate-400">
            ...
            <span
              className={cn(
                ...
                project.reception.status === "Acclaimed"
                  ? "text-emerald-400"
                  : project.reception.status === "Mixed"
                    ? "text-amber-400"
                    : "text-rose-400"
              )}
            >
              {project.reception.status}
            </span>
          </p>
        </div>
      </div>
