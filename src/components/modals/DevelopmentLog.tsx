...
export const DevelopmentLog: React.FC<DevelopmentLogProps> = ({ project }) => {
  const scriptedProject = (
    project.format === "unscripted" ? null : project
  ) as ScriptedProject | null;
    ...
    <div className="flex flex-col h-full space-y-4">
      ...
      <div className="glass-panel p-4 rounded-none border border-white/5 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {(scriptedProject?.activeRoles || []).map((role: string) => (
            <div
              key={role}
              className="px-3 py-1.5 rounded-none bg-white/5 border border-white/5 text-[10px] font-black uppercase tracking-widest text-foreground/80 flex items-center gap-2 group hover:border-primary/40 transition-all"
            >
          ...
          <div className="space-y-4">
            {events.map((event: ScriptEvent, idx: number) => {
              const config =
                ...
                EVENT_CONFIG.DIALOGUE_POLISH;
              const EventIcon = config.icon;
              return (
                <div
                  key={idx}
                  className="glass-panel p-4 rounded-none border border-white/5 animate-in slide-in-from-right-4 duration-500"
                >
                  <div className="flex gap-4">
                    <div
                      className={cn(
                        "w-10 h-10 rounded-none flex items-center justify-center bg-black/40 border border-white/5",
                        config.color
                      )}
                    >
      ...
      <div className="p-4 rounded-none bg-primary/5 border border-primary/10 flex items-center gap-3">
        ...
        <p className="text-[9px] font-bold text-muted-foreground leading-snug">
          Drafting decisions are algorithmic. Script Evolution triggers as "Town Heat" accumulates,
          reflecting the creative dynamism of your writing room.
        </p>
      </div>
    </div>
  );
