      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto bg-card/90 backdrop-blur-2xl border border-white/10 shadow-2xl animate-in fade-in zoom-in-95 duration-300">
        ...
        <DialogHeader className="relative z-10 pb-4 border-b border-border/40">
          ...
          <DialogDescription className="text-muted-foreground font-medium">
            Commission a new project. Talent will be attached after greenlight.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-5 relative z-10 pt-2">
          <div className="space-y-2">
            ...
            <div className="flex gap-2 group relative">
              <Input
                ...
                className="flex-1 font-display font-bold text-lg h-11 bg-background/50 border-border/50 focus-visible:ring-primary/50 transition-all shadow-inner"
              />
              <Button
                type="button"
                variant="outline"
                size="icon"
                onClick={() => setTitle(generateProjectTitle(genre))}
            ...
            <div className="grid grid-cols-2 gap-4">
              {format === "tv" ? (
                ...
                <div className="space-y-2 col-span-2 sm:col-span-1">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setUnscriptedFormat(v as UnscriptedFormatKey)}
                  >
                    <SelectTrigger
                      aria-label="Select Unscripted Format"
                      className="bg-background/50 border-border/50"
                    >
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="space-y-2 col-span-2 sm:col-span-1">
                <Label className="flex justify-between items-center text-[10px] uppercase font-bold tracking-widest text-muted-foreground/80">
                  <span>Episodes</span>
                  <span className="text-foreground font-mono bg-muted/50 px-1.5 py-0.5 rounded shadow-inner">
                    {episodes}
                  </span>
                </Label>
                <div className="pt-2">
                  <Slider
                    ...
                    min={
                      format === "tv"
                        ? TV_FORMATS[tvFormat].minEpisodes
                        : UNSCRIPTED_FORMATS[unscriptedFormat].minEpisodes
                    }
                    max={
                      format === "tv"
                        ? TV_FORMATS[tvFormat].maxEpisodes
                        : UNSCRIPTED_FORMATS[unscriptedFormat].maxEpisodes
                    }
                    step={1}
              ...
              <div className="space-y-2 col-span-2">
                ...
                <Select
                  value={releaseModel}
                  onValueChange={(v) => setReleaseModel(v as ReleaseModelKey)}
                >
                  <SelectTrigger
                    aria-label="Select Release Model"
                    className="bg-background/50 border-border/50"
                  >
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="bg-muted/30 border border-border/40 p-3 rounded-none mt-2 grid grid-cols-2 gap-x-4 gap-y-2 shadow-inner">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs font-semibold text-destructive">
                  {formatMoney(calculatedWeeklyCost)}/wk
                </span>
              </div>
              <div className="flex flex-col">
                <span className="text-[9px] uppercase tracking-widest font-bold text-muted-foreground/70">
                ...
                <span className="text-xs font-medium text-foreground/80">
                  Dev {calculatedDevWeeks}w / Prod {calculatedProdWeeks}w
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2 p-3 rounded-none bg-primary/5 border border-primary/20">
            ...
            <p className="text-[10px] text-muted-foreground leading-relaxed">
              <span className="font-bold text-foreground">
                Talent attachment happens after greenlight.
              </span>{" "}
              First attach producers/writers during development, then cast acting talent and
              directors for production — just like real Hollywood.
            </p>
          </div>
          ...
          <div className="space-y-2">
            <Label
              htmlFor="project-flavor"
              className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground/80"
            >
              Positioning <span className="text-muted-foreground/50 font-normal">(optional)</span>
            </Label>
            <Input
              id="project-flavor"
              ...
              className="bg-background/50 border-border/50 transition-all shadow-inner focus-visible:ring-primary/50"
            />
          </div>
        </div>
