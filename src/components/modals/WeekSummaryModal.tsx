...
export const WeekSummaryModal = () => {
  ...
  const generalChatter =
    narrativeEvents?.filter((e: NarrativeEvent) => e.type === "general" && !e.isPositive) || [];

  const isYearEnd = (toWeek - 1) % 52 === 0 && toWeek > 1;
  const currentSnapshot = isYearEnd ? snapshots[snapshots.length - 1] : null;
  const previousSnapshot =
    isYearEnd && snapshots.length > 1 ? snapshots[snapshots.length - 2] : null;

  const displayCashAfter = isYearEnd && currentSnapshot ? currentSnapshot.funds : cashAfter;
  const displayNetDelta =
    isYearEnd && currentSnapshot
      ? currentSnapshot.funds - (previousSnapshot?.funds || 0)
      : netDelta;

  let yearlyDelta = 0;
  if (isYearEnd && currentSnapshot) {
    yearlyDelta = previousSnapshot
      ? currentSnapshot.funds - previousSnapshot.funds
      : currentSnapshot.funds - (cashAfter - totalRevenue + totalCosts);
  }
          ...
          <div className="p-12 space-y-10">
            ...
            <div className="space-y-10">
                ...
                <div className="p-8 rounded-none bg-amber-500/5 border border-amber-500/20 shadow-2xl relative overflow-hidden group">
                  ...
                  <div className="relative z-10 flex justify-between items-end">
                    <div>
                      ...
                      >
                        {yearlyDelta >= 0 ? "+" : ""}
                        {formatMoney(yearlyDelta).toUpperCase()}
                      </p>
                    </div>
              ...
              {isQuietWeek && (
                <div className="p-8 rounded-none bg-white/[0.01] border border-white/5 shadow-2xl relative overflow-hidden">
                  ...
                  <p className="text-sm text-muted-foreground/60 italic leading-relaxed">
                    Production continues on schedule. No major news. Your CFO notes that cash
                    reserves look healthy.
                  </p>
                </div>
              )}
              ...
              {(positiveUpdates.length > 0 ||
                projectUpdates.some((u: string) => u.includes("(+)")) ||
                netDelta > 0) && (
                  ...
                  <div className="space-y-3">
                    ...
                    {projectUpdates
                      ...
                      .map((u: string, i: number) => (
                        <div
                          key={i}
                          className="p-4 rounded-none border border-emerald-500/20 bg-emerald-500/5 flex items-center gap-4"
                        >
                          <div className="w-1 h-1 bg-emerald-500 shrink-0 rotate-45" />
                          <div className="flex-1 text-sm font-medium text-emerald-500 italic">
                            {u.split("(+")[0]}
                            <span className="ml-2"> (+{u.split("(+")[1]}</span>
                          </div>
                        </div>
                      ))}
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    {crises.map((e: NarrativeEvent, i: number) => (
                      <div
                        key={i}
                        className="p-4 rounded-none border border-amber-500/30 bg-amber-500/10 flex items-center gap-4"
                      >
                        <div className="w-1 h-1 bg-amber-500 shrink-0 rotate-45" />
                        <div className="flex-1 text-sm font-medium text-amber-500 italic">
                          {e.title}
                    ...
                    {projectUpdates
                      ...
                      .map((u: string, i: number) => (
                        <div
                          key={i}
                          className="p-4 rounded-none border border-amber-500/20 bg-amber-500/5 flex items-center gap-4"
                        >
                          <div className="w-1 h-1 bg-amber-500 shrink-0 rotate-45" />
                          <div className="flex-1 text-sm font-medium text-amber-500 italic">
                            {u.split("(-)")[0]}
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    {generalChatter.map((e: NarrativeEvent, i: number) => (
                      <div
                        key={i}
                        className="p-4 rounded-none border border-white/5 bg-white/[0.01] flex items-center gap-4 hover:border-white/10 transition-colors"
                      >
                        <div className="w-1 h-1 bg-secondary shrink-0 rotate-45" />
                        <div className="flex-1 text-sm font-medium text-foreground/70 italic">
                          {e.title}
                        </div>
                      </div>
                    ))}
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-6">
                  ...
                  <div className="p-6 rounded-none border border-white/5 bg-white/[0.02] backdrop-blur-3xl shadow-2xl">
                    ...
                    >
                      {displayNetDelta >= 0 ? "+" : ""}
                      {formatMoney(displayNetDelta).toUpperCase()}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-center gap-8 py-4 bg-black/40 border-y border-white/5">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    >
                      {formatMoney(displayCashAfter).toUpperCase()}
                    </span>
                  </div>
                </div>
              </div>
