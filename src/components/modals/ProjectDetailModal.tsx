...
export const ProjectDetailModal = () => {
  ...
  const projects = useMemo(
    ...
    [gameState?.studio.internal.projects]
  );
  ...
  const roleGroups = useMemo(() => {
    ...
    return groups;
  }, [project, scriptedProject, contracts, talentByRole, gameState]);
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-between items-center text-xs">
                          ...
                          <span className="text-rose-400/60 font-bold">
                            -{formatMoney(project.weeklyCost)}
                          </span>
                        </div>
                      ...
                      <div className="space-y-4">
                        <div className="glass-panel p-5 rounded-none border border-white/5 bg-black/40 space-y-4">
                          ...
                          <p className="text-[10px] font-bold text-muted-foreground/60 leading-relaxed italic">
                            Script is currently in active development. AI drafting system tracks
                            role splitting and Town Heat in real-time.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-6">
                      <div className="glass-panel p-8 rounded-none border border-primary/20 bg-primary/5 flex flex-col items-center text-center space-y-4">
                        ...
                        <p className="text-sm text-muted-foreground max-w-md">
                          ...
                          {formatMoney(project.weeklyCost)} per week. Review casting or wait for
                          wrap.
                        </p>
                            ...
                            <ul className="space-y-2">
                              {greenlightReport.positives.map((p, i) => (
                                <li key={i} className="text-xs text-slate-300 flex gap-2">
                                  <div className="w-1 h-1 bg-emerald-500/40 rounded-none mt-1.5 shrink-0" />{" "}
                                  {p}
                                </li>
                              ))}
                            </ul>
                          ...
                          <div className="space-y-3">
                            ...
                            <ul className="space-y-2">
                              {greenlightReport.negatives.map((n, i) => (
                                <li key={i} className="text-xs text-slate-300 flex gap-2">
                                  <div className="w-1 h-1 bg-rose-500/40 rounded-none mt-1.5 shrink-0" />{" "}
                                  {n}
                                </li>
                              ))}
                            </ul>
                                ...
                                {(project.state === "development" ||
                                  ...
                                  <Select
                                    onValueChange={(val) =>
                                      val &&
                                      gameState &&
                                      gameState.finance.cash >= (talentMap.get(val)?.fee || 0) &&
                    ...
                    {[
                      ...
                      <button
                        ...
                        disabled={
                          ...
                          (gameState ? gameState.finance.cash < tier.cost : false)
                        }
                  ...
                  {!project.marketingLevel ? (
                    <Button
                      ...
                      onClick={() => {
                        ...
                        else if (selectedTier === "blockbuster")
                          launchMarketingCampaign(
                            project.id,
                            "Saturation",
                            "SELL_THE_SPECTACLE",
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-black/40 border border-white/5 p-6 rounded-none space-y-6">
                      ...
                      <div className="space-y-4">
                        <Select
                          onValueChange={(v) => {
                            submitToFestival(project.id, v as AwardBody);
                            selectProject(null);
                          }}
                        >
                          <SelectTrigger
