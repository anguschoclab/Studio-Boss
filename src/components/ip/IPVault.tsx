...
export const IPVault = () => {
  ...
  const { ownedIP, syndicatedIP, marketIP } = React.useMemo(() => {
    ...
    const syndicated = vault.filter(
      (a) => a.syndicationStatus === "SYNDICATED" && a.rightsOwner === "STUDIO"
    );
    ...
    <div className="h-full flex flex-col space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-1000 pb-20">
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-10 bg-white/[0.02] p-10 rounded-none border border-white/5 backdrop-blur-3xl shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-6 mb-6">
            ...
            <div>
              <h2 className="text-6xl font-display font-black tracking-tighter uppercase italic leading-none mb-3 drop-shadow-[0_0_30px_rgba(255,255,255,0.1)]">
                IP VAULT & CATALOG
              </h2>
      ...
      <Tabs defaultValue="franchises" className="flex-1 flex flex-col min-h-0">
        <TabsList className="bg-white/[0.02] border border-white/10 p-1 self-start mb-10 rounded-none h-14">
          <TabsTrigger
            value="franchises"
            className="text-[10px] uppercase font-black tracking-[0.3em] italic px-10 h-12 rounded-none data-[state=active]:bg-primary data-[state=active]:text-black transition-all duration-700"
          >
            FRANCHISE HUB
          </TabsTrigger>
          <TabsTrigger
            value="owned"
            className="text-[10px] uppercase font-black tracking-[0.3em] italic px-10 h-12 rounded-none data-[state=active]:bg-white/10 data-[state=active]:text-white transition-all duration-700"
          >
            OWNED INVENTORY
          </TabsTrigger>
          <TabsTrigger
            value="market"
            className="text-[10px] uppercase font-black tracking-[0.3em] italic px-10 h-12 rounded-none data-[state=active]:bg-secondary data-[state=active]:text-black transition-all duration-700"
          >
            MARKET RIGHTS
          </TabsTrigger>
          <TabsTrigger
        ...
        <ScrollArea className="flex-1 pr-6 custom-scrollbar">
          <TabsContent
            value="franchises"
            className="mt-0 outline-none animate-in fade-in duration-1000"
          >
            <FranchiseHub />
          </TabsContent>
          ...
          <TabsContent
            value="owned"
            className="mt-0 space-y-12 pb-20 outline-none animate-in fade-in duration-1000"
          >
...
const EmptyVault = ({ message }: { message: string }) => (
  <div className="col-span-full py-48 text-center glass-card border-none shadow-none bg-transparent opacity-20 flex flex-col items-center space-y-10">
    ...
    <div className="space-y-4 px-12">
      <p className="text-sm font-black text-muted-foreground uppercase tracking-[0.6em] italic">
        SECURE VAULT EMPTY
      </p>
      <p className="text-xs font-black text-muted-foreground uppercase tracking-[0.3em] max-w-[400px] mx-auto leading-relaxed italic">
        {message}
      </p>
    </div>
  </div>
