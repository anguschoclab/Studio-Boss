...
interface IPAssetTagsProps {
  asset: IPAsset;
  isMarket: boolean;
  tier: (typeof SYNDICATION_TIERS)[keyof typeof SYNDICATION_TIERS];
}

const IPAssetTags = ({ asset, isMarket, tier }: IPAssetTagsProps) => {
  return (
    <div className="flex items-center gap-3 mb-3">
      {asset.syndicationTier !== "NONE" && (
        <TooltipWrapper
          tooltip={`THIS IP IS CURRENTLY IN ${tier.label.toUpperCase()} LEVEL SYNDICATION.`}
        >
          <div
            style={{
...
interface IPAssetHeaderProps {
  asset: IPAsset;
  isMarket: boolean;
  tier: (typeof SYNDICATION_TIERS)[keyof typeof SYNDICATION_TIERS];
}

const IPAssetHeader = ({ asset, isMarket, tier }: IPAssetHeaderProps) => {
  return (
    <div className="flex justify-between items-start gap-6">
      <div className="min-w-0">
        <IPAssetTags asset={asset} isMarket={isMarket} tier={tier} />
      ...
      <div className="w-12 h-12 rounded-none bg-white/[0.03] border border-white/10 flex items-center justify-center shadow-lg group-hover:border-primary/30 transition-all duration-700">
        {isMarket ? (
          <Globe className="h-6 w-6 text-secondary/60" />
        ) : (
...
interface IPAssetRelevanceProps {
  relevancePercent: number;
}

const IPAssetRelevance = ({ relevancePercent }: IPAssetRelevanceProps) => {
  return (
    <TooltipWrapper
      tooltip="CULTURAL RELEVANCE IMPACTS REVENUE AND REBOOT SUCCESS PROBABILITY."
      side="top"
    >
...
interface IPAssetStatsProps {
  asset: IPAsset;
}

const IPAssetStats = ({ asset }: IPAssetStatsProps) => {
  return (
      ...
      <TooltipWrapper tooltip="ESTIMATED PASSIVE INCOME GENERATED PER WEEK." side="top">
        <div className="space-y-2 cursor-help">
          ...
          <div className="text-lg font-display font-black tracking-tighter text-emerald-400 italic leading-none">
            {formatMoney(
              Math.floor(asset.baseValue * asset.merchandisingMultiplier * asset.decayRate)
            ).toUpperCase()}
          </div>
        </div>
      </TooltipWrapper>
...
export const IPAssetCard = ({ asset, isMarket = false }: IPAssetCardProps) => {
  ...
  return (
    <TooltipWrapper
      tooltip={`AUDIT ${asset.title.toUpperCase()} FINANCIALS & LICENSING DETAILS`}
      side="top"
    >
      <Card className="glass-card border border-white/5 rounded-none hover:border-primary/40 group transition-all duration-700 relative overflow-hidden cursor-pointer active:scale-[0.98] bg-white/[0.01] shadow-2xl">
        ...
        <CardContent className="p-8 flex flex-col h-full space-y-8 relative z-10">
          <IPAssetHeader asset={asset} isMarket={isMarket} tier={tier} />
          <IPAssetRelevance relevancePercent={relevancePercent} />
          <IPAssetStats asset={asset} />
          <IPAssetFooter
            asset={asset}
            isMarket={isMarket}
