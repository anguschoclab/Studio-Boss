...
export const MerchandiseRevenuePanel: React.FC<MerchandiseRevenuePanelProps> = ({
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      >
        {byCategory.length === 0 ? (
          ...
          >
            ...
            <p className={cn("text-sm", tokens.text.caption)}>No merchandise data available</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {byCategory.map((category) => (
              <Card key={category.category} className={cn("p-4", tokens.border.default)}>
      ...
      <Section title="Franchise Merchandise" subtitle="Revenue by IP franchise" icon={Star}>
        {byFranchise.length === 0 ? (
          <div
            className={cn("text-center py-8", tokens.border.default, "border-dashed rounded-none")}
          >
            <Star className="h-10 w-10 mx-auto mb-3 opacity-20" />
            <p className={cn("text-sm", tokens.text.caption)}>No franchise merchandise data</p>
          </div>
        ) : (
          <div className="space-y-3">
            {byFranchise.map((franchise) => (
              <Card key={franchise.franchiseId} className={cn("p-4", tokens.border.default)}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={cn("w-3 h-3 rounded-none", getTierColor(franchise.tier))} />
                    <div>
