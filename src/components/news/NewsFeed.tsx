      ...
      <div className="p-8 border-b border-white/5 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h3 className="font-display text-xs font-black uppercase tracking-[0.4em] text-foreground italic leading-none">
              INDUSTRY HISTORY
            </h3>
          </div>
          <Badge
            variant="outline"
            className="text-[9px] font-black border-white/10 bg-white/5 text-muted-foreground/60 rounded-md italic"
          >
        ...
        <div className="flex flex-wrap gap-2 pb-2">
          <button
            ...
            className={`rounded-md text-[9px] font-black uppercase tracking-[0.2em] transition-all duration-700 border italic focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none p-3 ${
              filter === "ALL"
                ? "bg-primary text-black border-primary shadow-[0_0_15px_rgba(var(--primary),0.2)]"
                : "bg-white/[0.02] text-muted-foreground/40 border-white/5 hover:border-white/20"
            }`}
          >
            ALL
          </button>
        ...
        {filteredHistory.map((item, idx) => {
          ...
          return (
            <div key={item.id} className="relative pl-10 group">
              ...
              {/* Timeline Dot/Icon */}
              <div
                className={`absolute left-0 top-1 w-4 h-4 rounded-md border border-white/10 bg-black z-10 flex items-center justify-center transition-all group-hover:scale-110 group-hover:border-primary/40 ${config.color}`}
              >
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-[9px] font-black text-muted-foreground/20 uppercase tracking-[0.2em] italic">
                    WEEK {item.week}
                  </span>
                  {item.impact && (
                    <span className="text-[9px] font-black text-primary tracking-[0.1em] italic">
                      {item.impact.toUpperCase()}
                    </span>
                  )}
                </div>
                <h4 className="text-xs font-black text-foreground group-hover:text-primary transition-all duration-700 leading-none uppercase italic tracking-tight">
                  {item.headline}
                </h4>
                <p className="text-[10px] text-muted-foreground/60 leading-relaxed font-black uppercase tracking-wider italic line-clamp-2 group-hover:line-clamp-none transition-all duration-700">
                  {item.description}
                </p>
              </div>
            </div>
