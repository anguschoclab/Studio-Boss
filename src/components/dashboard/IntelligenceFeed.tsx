...
export const IntelligenceFeed = ({ newsHistory }: IntelligenceFeedProps) => {
  return (
    <Card
      aria-label="Studio Intelligence Feed"
      className={cn(
        "animate-in fade-in duration-1000 relative overflow-hidden text-left group",
        "bg-white/[0.01] backdrop-blur-2xl border border-white/5 rounded-none shadow-2xl"
      )}
    >
          ...
          {newsHistory && newsHistory.length > 0 ? (
            newsHistory.slice(0, 4).map((news, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0 relative z-10 space-y-2">
                  <p className="text-lg font-display font-black text-foreground uppercase italic tracking-tighter leading-none group-hover/item:text-primary transition-colors">
                    {news.headline}
                  </p>
                  <p className="text-[10px] font-black uppercase tracking-[0.1em] text-muted-foreground/40 line-clamp-1">
                    {news.description}
                  </p>
                </div>
            ...
            <div className="text-center py-24 px-8 flex flex-col items-center justify-center gap-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[9px] text-muted-foreground/10 font-black uppercase tracking-widest max-w-[240px]">
                  SIGNAL SILENT // GLOBAL FEED STANDBY
                </p>
              </div>
            </div>
          )}
