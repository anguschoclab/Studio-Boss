...
export const NielsenDashboard = () => {
    ...
    () =>
      projects.filter(
        (p: Project): p is SeriesProject =>
          p.type === "SERIES" &&
          "tvDetails" in p &&
          !!(p as unknown as { nielsenProfile: unknown }).nielsenProfile
      ),
    [projects]
  );
  ...
  const weeklyRankings = React.useMemo(() => {
    return airingShows
      .map((show) => {
        const profile = (show as unknown as { nielsenProfile: NielsenProfile }).nielsenProfile as
          | NielsenProfile
          | undefined;
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full py-40 text-center opacity-20 space-y-8 animate-in fade-in duration-1000">
        ...
        <div className="space-y-2">
          <h3 className="text-2xl font-display font-black uppercase tracking-[0.4em] italic">
            NO NIELSEN DATA
          </h3>
    ...
    <div className="h-full flex flex-col space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-1000 overflow-hidden pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 bg-white/[0.02] p-10 rounded-none border border-white/5 backdrop-blur-3xl shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-6 mb-2">
            ...
            <div>
              <h2 className="text-5xl font-display font-black tracking-tighter uppercase italic leading-none mb-3 drop-shadow-[0_0_30px_rgba(var(--primary),0.1)]">
                NIELSEN RATINGS
              </h2>
        ...
        <div className="flex gap-6 relative z-10">
          <StatChip
            label="AIRING"
            value={airingShows.length}
            icon={<Tv className="h-4 w-4 text-primary" />}
          />
          <StatChip
            label="TRACKED"
            value={allShows.length}
      ...
      <Tabs defaultValue="rankings" className="flex-1 flex flex-col overflow-hidden">
        <TabsList className="bg-white/[0.02] border border-white/5 p-1 h-14 rounded-none w-fit mb-8">
          <TabsTrigger
            value="rankings"
            className="h-12 px-10 text-[10px] font-black uppercase tracking-[0.2em] rounded-none data-[state=active]:bg-white/10 italic transition-all duration-700"
          >
            WEEKLY RANKINGS
          </TabsTrigger>
          <TabsTrigger
...
const StatChip = ({
  ...
  <div className="flex flex-col gap-2 bg-white/[0.02] border border-white/5 rounded-none px-8 py-5 min-w-[160px] shadow-xl relative overflow-hidden group">
    ...
    <span className="text-[8px] font-black uppercase tracking-[0.3em] text-muted-foreground/40 italic leading-none">
      {label}
    </span>
...
const TrendIcon = ({ trend }: { trend: string }) => {
  if (trend === "UP")
    return (
      <ArrowUp className="h-5 w-5 text-emerald-400 drop-shadow-[0_0_10px_rgba(52,211,153,0.3)]" />
    );
  if (trend === "DOWN")
    return (
      <ArrowDown className="h-5 w-5 text-red-500 drop-shadow-[0_0_10px_rgba(239,68,68,0.3)]" />
    );
  if (trend === "PREMIERE")
    return (
      <Trophy className="h-5 w-5 text-amber-500 drop-shadow-[0_0_10px_rgba(245,158,11,0.3)]" />
    );
  if (trend === "FINALE")
    return (
      <Trophy className="h-5 w-5 text-primary drop-shadow-[0_0_10px_rgba(var(--primary),0.3)]" />
    );
        ...
        >
          ...
          <div className="grid grid-cols-12 gap-4 items-center px-10 py-8">
            ...
            <div className="col-span-3">
              <div className="text-sm font-black tracking-[0.15em] uppercase truncate italic mb-1 group-hover:text-primary transition-all duration-700">
                {show.title}
              </div>
              <div className="text-[9px] font-black text-muted-foreground/20 uppercase tracking-[0.2em] italic">
                S{show.tvDetails.currentSeason} E{latest?.episodeNumber || 0} •{" "}
            ...
            <div className="col-span-1 text-right">
              <span className="text-lg font-display font-black italic leading-none">
                {latest?.householdRating.toFixed(1)}
              </span>
            </div>
            <div className="col-span-1 text-right">
              <span className="text-lg font-display font-black italic leading-none text-muted-foreground/40">
                {latest?.householdShare.toFixed(0)}%
              </span>
            </div>
            <div className="col-span-1 text-right">
              <span className="text-lg font-display font-black italic leading-none text-amber-500">
                {latest?.keyDemo.toFixed(1)}
              </span>
            </div>
            <div className="col-span-2 text-right">
              <span className="text-lg font-display font-black italic leading-none">
                {latest?.totalViewers.toFixed(2)}M
              </span>
            </div>
            <div className="col-span-1 text-right">
    ...
    <Card className="glass-card border border-white/5 rounded-none overflow-hidden bg-white/[0.01] shadow-2xl">
      <CardHeader className="p-10 border-b border-white/5 bg-white/[0.02]">
        <div className="flex justify-between items-start">
          <div className="space-y-3">
            <CardTitle className="text-3xl font-display font-black tracking-tighter uppercase italic leading-none">
              {show.title}
            </CardTitle>
            <div className="flex items-center gap-4">
              <p className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-[0.3em] italic">
                S{show.tvDetails.currentSeason} • {show.genre.toUpperCase()} •{" "}
                {TIME_SLOTS[profile.timeSlot]?.label.toUpperCase() ||
                  profile.timeSlot.toUpperCase()}
              </p>
            </div>
          </div>
          <div className="text-right space-y-2 bg-primary/5 border border-primary/20 px-8 py-5 rounded-none shadow-xl">
            <div className="text-[8px] font-black text-muted-foreground/40 uppercase tracking-[0.3em] italic leading-none">
              SEASON AVG
            </div>
            <div className="text-4xl font-display font-black text-primary italic leading-none">
              {profile.seasonAvgKeyDemo.toFixed(1)}
            </div>
            <div className="text-[9px] font-black text-primary/40 italic tracking-widest leading-none">
              A18-49 RTG
            </div>
          </div>
        </div>
      ...
      <CardContent className="p-10 space-y-12">
        ...
        <div className="grid grid-cols-4 gap-8">
          ...
          <MetricBox
            label="RETENTION"
            value={`${profile.audienceRetention}%`}
            color={
              profile.audienceRetention >= 80
                ? "text-emerald-400"
                : profile.audienceRetention >= 60
                  ? "text-amber-400"
                  : "text-red-500"
            }
          />
  ...
  return (
    <Card className="glass-card border border-white/5 rounded-none overflow-hidden bg-white/[0.01] shadow-2xl">
      <CardHeader className="p-8 border-b border-white/5 bg-white/[0.02]">
        <CardTitle className="text-xs font-black tracking-[0.2em] uppercase italic leading-none mb-2">
          {show.title} — DEMOGRAPHICS
        </CardTitle>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={demoData} layout="vertical" margin={{ left: 20 }}>
              ...
              <YAxis
                dataKey="name"
                type="category"
                tick={{ fontSize: 8, fill: "rgba(255,255,255,0.4)", fontWeight: 900 }}
                width={100}
                axisLine={false}
                tickLine={false}
              />
              ...
              <Bar dataKey="rating" name="RTG" radius={0}>
                {demoData.map((entry, idx) => (
                  <Cell
                    key={idx}
                    fill={DEMO_COLORS[entry.demo as NielsenDemographic] || "hsl(var(--primary))"}
                    fillOpacity={0.8}
                  />
                ))}
              </Bar>
...
const MetricBox = ({ label, value, color }: { label: string; value: string; color?: string }) => (
  <div className="bg-white/[0.02] rounded-none p-6 border border-white/5 text-center shadow-xl group hover:border-white/10 transition-all duration-700">
    <div className="text-[8px] font-black text-muted-foreground/40 uppercase tracking-[0.3em] mb-2 italic leading-none">
      {label}
    </div>
    <div
      className={cn(
