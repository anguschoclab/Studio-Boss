...
export const NielsenDashboard = () => {
  ...
  const tvShows = React.useMemo(() => 
    projects.filter((p: Project): p is SeriesProject => 
      p.type === 'SERIES' && 'tvDetails' in p && !!(p as unknown as Record<string, any>).nielsenProfile
    ),
  [projects]);
  ...
  const weeklyRankings = React.useMemo(() => {
    return airingShows
      .map(show => {
        const profile = (show as unknown as Record<string, any>).nielsenProfile as NielsenProfile | undefined;
