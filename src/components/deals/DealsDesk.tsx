      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 bg-white/[0.02] p-10 rounded-none border border-white/5 backdrop-blur-3xl shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-6 mb-2">
            ...
            <div>
              <h2 className="text-5xl font-display font-black tracking-tighter uppercase italic leading-none mb-3 drop-shadow-[0_0_30px_rgba(59,130,246,0.1)]">DEALS DESK & DISTRIBUTION</h2>
        ...
        <div className="flex gap-4 relative z-10">
          <div className="bg-blue-500/5 border border-blue-500/20 text-blue-400 px-8 py-4 rounded-none flex flex-col gap-1 shadow-[0_0_20px_rgba(59,130,246,0.05)] min-w-[200px]">
            <span className="text-[8px] font-black uppercase tracking-[0.3em] opacity-40">PARTNERS</span>
            <span className="text-2xl font-display font-black italic leading-none">{buyers.length} NETWORK</span>
          </div>
        </div>
      </div>
              ...
              <div className="space-y-4 pr-4">
                {pitchingProjects.length === 0 ? (
                  <div className="text-center py-32 opacity-20 flex flex-col items-center gap-6">
                    ...
                    <p className="text-[10px] uppercase font-black tracking-[0.4em] italic">NO ACTIVE PITCH SLATE</p>
                  </div>
                ) : (
                  pitchingProjects.map(p => (
                    <div key={p.id} className="p-6 rounded-none bg-white/[0.02] border border-white/5 hover:border-primary/40 transition-all duration-700 group cursor-pointer shadow-lg">
                      <div className="text-xs font-black uppercase tracking-[0.2em] group-hover:text-primary transition-all duration-700 truncate italic leading-none mb-3">{p.title}</div>
                      <div className="flex justify-between items-center">
                        <span className="text-[9px] uppercase text-muted-foreground/30 font-black tracking-[0.3em] italic">{p.genre}</span>
                        <div className="text-[9px] px-3 h-5 bg-primary/5 text-primary border border-primary/20 font-black uppercase flex items-center justify-center rounded-none italic shadow-[0_0_10px_rgba(var(--primary),0.1)]">{p.budgetTier}</div>
                      </div>
                    </div>
                  ))
                )}
...
const getMandateStyle = (type?: MandateType) => {
  switch (type) {
    case 'prestige': return 'text-amber-400 bg-amber-400/5 border-amber-400/20 shadow-[0_0_15px_rgba(251,191,36,0.1)]';
    case 'sci-fi': return 'text-purple-400 bg-purple-400/5 border-purple-400/20 shadow-[0_0_15px_rgba(192,132,252,0.1)]';
    case 'comedy': return 'text-yellow-400 bg-yellow-400/5 border-yellow-400/20 shadow-[0_0_15px_rgba(250,204,21,0.1)]';
    case 'drama': return 'text-blue-400 bg-blue-400/5 border-blue-400/20 shadow-[0_0_15px_rgba(96,165,250,0.1)]';
    case 'budget_freeze': return 'text-red-500 bg-red-500/5 border-red-500/20 shadow-[0_0_15px_rgba(239,68,68,0.1)]';
    case 'broad_appeal': return 'text-emerald-400 bg-emerald-400/5 border-emerald-400/20 shadow-[0_0_15_rgba(52,211,153,0.1)]';
    default: return 'text-muted-foreground/40 bg-white/5 border-white/10';
  }
};
    ...
    <Card className="glass-card border border-white/5 rounded-none hover:border-blue-500/40 group transition-all duration-700 bg-white/[0.01] shadow-2xl relative overflow-hidden">
      ...
      <CardContent className="p-8 space-y-8 relative z-10">
        ...
        <div className="flex justify-between items-start gap-4">
          <div>
            <h4 className="font-display text-xl font-black tracking-tighter uppercase italic group-hover:text-blue-400 transition-all duration-700 leading-none mb-3 drop-shadow-[0_0_10px_rgba(59,130,246,0.1)]">{buyer.name}</h4>
            <div className="flex items-center gap-3">
              <div className="text-[9px] font-black border border-white/10 bg-white/[0.02] uppercase px-3 h-5 flex items-center justify-center tracking-[0.2em] rounded-none italic text-muted-foreground/40">{buyer.archetype}</div>
            </div>
          </div>
          <div className={cn("px-4 h-6 border text-[9px] font-black uppercase tracking-[0.25em] flex items-center justify-center rounded-none italic", getMandateStyle(buyer.currentMandate?.type))}>
        ...
        {projects.length > 0 && (
          <div className="space-y-5">
            <div className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground/30 flex items-center justify-between italic">
              <span className="flex items-center gap-2"><Target className="h-3 w-3" /> ACQUISITION FIT ANALYSIS</span>
            </div>
            <div className="space-y-5">
                ...
                return (
                  <div key={p.id} className="space-y-3">
                    <div className="flex justify-between text-[10px] font-black uppercase tracking-[0.2em] italic leading-none">
                      <span className="truncate max-w-[200px] text-foreground/60 group-hover:text-foreground/90 transition-all duration-700">{p.title}</span>
                      <span className={cn(
                        "font-mono drop-shadow-[0_0_10px_rgba(var(--color),0.3)]",
                        fit > 70 ? 'text-emerald-400' : fit > 40 ? 'text-amber-400' : 'text-red-500'
                      )}>{fit}%</span>
                    </div>
                    <div className="h-1.5 bg-white/[0.03] rounded-none overflow-hidden border border-white/5">
                      <div 
                        className={cn(
                          ...
                          fit > 70 ? 'bg-emerald-400 shadow-[0_0_15px_rgba(52,211,153,0.4)]' : fit > 40 ? 'bg-amber-400 shadow-[0_0_15px_rgba(251,191,36,0.4)]' : 'bg-red-500 shadow-[0_0_15px_rgba(239,68,68,0.4)]'
                        )}
                        style={{ width: `${fit}%` }}
                      />
                    </div>
        ...
        <div className="pt-4 border-t border-white/5">
           <Button variant="outline" size="sm" className="w-full text-[10px] font-black uppercase tracking-[0.3em] h-12 border-white/10 bg-white/[0.02] hover:bg-blue-500 hover:text-black hover:border-blue-400 transition-all duration-700 rounded-none italic shadow-xl">
             NEGOTIATE TERMS
           </Button>
        </div>
      </CardContent>
