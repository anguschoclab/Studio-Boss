    ...
    >
      ...
      <div className="space-y-4 relative z-10">
        <div className="flex items-start gap-4">
          <div
            className={`p-3 rounded-none flex items-center justify-center shrink-0 transition-colors duration-300 ${selected ? "bg-primary/20 text-primary shadow-[0_0_10px_rgba(var(--primary),0.3)]" : "bg-muted/50 group-hover:bg-primary/10"}`}
          >
            <span className="text-3xl drop-shadow-sm">{archetypeIcons[arch.key]}</span>
          </div>
          <div className="flex-1 min-w-0 pt-1">
            <h3 className="font-display font-black text-xl text-foreground tracking-tight group-hover:text-primary transition-colors drop-shadow-sm">
              {arch.name}
            </h3>
        ...
        <div className="pt-4 border-t border-border/40 flex justify-between items-center text-xs text-muted-foreground">
          <div className="flex flex-col">
            ...
            <span className="text-primary font-bold text-sm drop-shadow-sm">
              {formatMoney(arch.startingCash)}
            </span>
          </div>
          <div className="w-px h-6 bg-border/40" />
          <div className="flex flex-col text-right">
            <span className="text-[9px] uppercase tracking-widest font-bold">Prestige</span>
            <span className="text-secondary font-bold text-sm drop-shadow-sm">
              {arch.startingPrestige}
            </span>
          </div>
        </div>
      </div>
