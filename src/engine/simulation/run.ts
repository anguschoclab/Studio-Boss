...
const archetype = (args.find((_: string, i: number) => args[i - 1] === "--archetype") ||
  "major") as ArchetypeKey;
const persona = args.find((_: string, i: number) => args[i - 1] === "--persona") || "balanced";
const seed = parseInt(
  args.find((_: string, i: number) => args[i - 1] === "--seed") || Date.now().toString().slice(-4)
);

console.log(`🚀 Starting Studio Boss Headless Simulation...`);
console.log(
  `Weeks: ${weeks} | Archetype: ${archetype} | Persona: ${persona} | Seed: ${seed} | Runs: ${runs}\n`
);
