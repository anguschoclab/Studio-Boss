...
export class MetricsCollector {
  ...
  public record(state: GameState, summary: WeekSummary): void {
    ...
    const allStudios = [
      {
        id: state.studio.id,
        projects: playerProjectsList,
        cash: Number(state.finance.cash) || 0,
    ...
    allStudios.forEach((studio) => {
      studio.projects.forEach((p) => {
        ...
        const nProfile = isSeries
          ? (p as import("@/engine/types").SeriesProject).nielsenProfile
          : undefined;
    ...
    const tvAwardEvents = (summary.newsEvents || []).filter(
      (e) => e.type === "AWARD" && e.headline.includes("Best")
    );
