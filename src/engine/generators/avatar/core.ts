...
export function deriveFeatures(talent: Talent, currentWeek: number = 1): AvatarFeatures {
  ...
  if (visualAge > greyOnset) {
    ...
    if (greyProgress >= 0.95) {
      ...
      const blendAmount = targetIsWhite ? (greyProgress - 0.6) / 0.4 : greyProgress / 0.6;
  ...
  const maleStylesByEthnicity: Record<string, string[]> = {
    ...
    Caucasian: [...baseMaleStyles, "pompadour", "middle-part", "undercut"],
    Mixed: [...baseMaleStyles, "thick-wave", "undercut", "middle-part", "pompadour"],
  };
  ...
  const hasEpicanthicFold =
    ["Asian", "South Asian"].includes(ethnicity) || (ethnicity === "Mixed" && rand() < 0.3);
 ...
 */
function tintClothingForPersonality(baseColor: string, talent: Talent, rand: () => number): string {
  const m = talent.motivationProfile;
  const role = (talent.role || "").toLowerCase();
