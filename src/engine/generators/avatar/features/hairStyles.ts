 ...
 */
export function renderHairBack(
  f: AvatarFeatures,
  cx: number,
  cy: number,
  faceW: number,
 ...
 */
export function renderHairFront(
  f: AvatarFeatures,
  cx: number,
  cy: number,
  faceW: number,
  ...
  switch (f.hairStyle) {
    ...
    case "undercut": {
      ...
      const sideShade =
        `<path d="M ${cx - faceW / 2 - 2} ${topY + 12} Q ${cx - faceW / 2} ${topY + 22} ${cx - faceW / 2 + 4} ${topY + 28} L ${cx - faceW / 2 + 4} ${topY + 14} Z" fill="${secondary}" opacity="0.7"/>` +
