...
export function generateAgencies(count: number): Agency[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (i < 2) {
      ...
      actualName =
        pick(POWERHOUSE_PREFIXES) +
        pick([" Partners", " Representation", " Agency", " Group", " Collective"]);
    } else if (i % 3 === 0) {
      archetype = "shark";
      actualName =
        pick(SHARK_PREFIXES) + pick([" Management", " Media", " Brokers", " Associates"]);
    } else if (r < 0.2) {
      archetype = "comedy_specialist";
      actualName =
        pick(["Giggles", "Laugh Track", "Standup", "Joke", "Punchline"]) +
        pick([" Reps", " Management", " Artists"]);
    } else if (r < 0.4) {
      archetype = "lit_agency";
      actualName =
        pick(["Pages", "Story", "Narrative", "Ink", "Typewriter"]) +
        pick([" Guild", " Agency", " Associates"]);
    } else if (r < 0.6) {
      archetype = "mega_corp";
      actualName =
        pick(["Omni", "Global", "Universal", "Infinite", "Massive"]) +
        pick([" Media", " Corp", " Representation"]);
    } else if (r < 0.75) {
      archetype = "streaming_titan";
      actualName =
        pick(STREAMING_TITAN_PREFIXES) + pick([" Representation", " Management", " Artists"]);
    } else if (r < 0.8) {
    ...
    const leverage =
      archetype === "powerhouse" || archetype === "mega_corp" || archetype === "streaming_titan"
        ? Math.floor(randRange(85, 100))
        : archetype === "shark"
          ? Math.floor(randRange(80, 95))
          : Math.floor(randRange(20, 60));
    ...
    agencies.push({
      ...
      prestige:
        tier === "powerhouse"
          ? Math.floor(randRange(80, 100))
          : tier === "major"
            ? Math.floor(randRange(60, 85))
            : Math.floor(randRange(30, 70)),
      leverage,
      traits,
      motivationProfile,
...
export function generateAgents(agencies: Agency[], countPerAgency: number): Agent[] {
  ...
  for (const agency of agencies) {
    const agentCount =
      agency.tier === "powerhouse"
        ? countPerAgency * 2
        : agency.tier === "boutique"
          ? Math.max(1, Math.floor(countPerAgency / 2))
          : countPerAgency;

    for (let i = 0; i < agentCount; i++) {
      const firstName = pick(AGENT_FIRST_NAMES);
      const lastName = pick(AGENT_LAST_NAMES);
      let specialty: AgentSpecialty = pick([
        "film_packaging",
        "tv_packaging",
        "literary",
        "talent",
      ...
      agents.push({
        ...
        leverage:
          agency.culture === "shark"
            ? Math.floor(randRange(70, 100))
            : Math.floor(randRange(30, 80)),
        motivationProfile: agency.motivationProfile
          ? {
              ...agency.motivationProfile,
              aggression: agency.motivationProfile.aggression + randRange(-10, 10),
            }
          : undefined,
      });
    }
