  ...
  processProject(project: Project, state: GameState, context: TickContext): void {
    ...
    if (project.state === "development") {
      ...
      if (project.activeCrisis) {
        ...
        const crisisImpacts = resolveCrisisWithHandlers(
          state,
          project.id,
          context.rng.rangeInt(0, crisis.options.length - 1)
        );
    ...
    if (
      (project.state === "post_release" || project.state === "released") &&
    ...
    if (project.contentFlags?.length && !project.rating) {
      ...
      context.impacts.push({
        ...
        payload: {
          ...
          update: { rating: newRating, regionalRatings: newRegional },
        },
      });
    }
    ...
    if (
      ...
      for (let j = 0; j < project.regionalRatings.length; j++) {
        if (project.regionalRatings[j].isBanned) {
          bannedMarkets.push(
            project.regionalRatings[j].market as import("../../types/project.types").RatingMarket
          );
        }
      }
      if (bannedMarkets.length > 0) {
        const banImpact = generateMarketBanScandal(
          project,
          bannedMarkets,
          context.week,
          state,
