...
export function handleSystemTick(state: GameState, impact: StateImpact): GameState {
  ...
  const { week, tickCount, __studioUpdate, studioIdentity } = payload as unknown as {
    week?: number;
    tickCount?: number;
    __studioUpdate?: Partial<GameState['studio']>;
    studioIdentity?: Partial<GameState['studio']['identity']>;
  };
  ...
  if (studioIdentity) {
    updated = {
      ...
      studio: {
        ...updated.studio,
        identity: { ...(updated.studio as unknown as GameState['studio']).identity, ...studioIdentity }
      }
    };
  }
  ...
  if (newAchievementId) {
    const existing: string[] = (updated.studio as unknown as GameState['studio']).achievements ?? [];
