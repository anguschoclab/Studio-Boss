...
export function initializeGame(
  ...
  const rivals: RivalStudio[] = Array.from({ length: 10 }, () => {
    ...
    const motivations: import("@/engine/types").StudioMotivation[] = [
      "CASH_CRUNCH",
      "AWARD_CHASE",
      "FRANCHISE_BUILDING",
      "MARKET_DISRUPTION",
  ...
  rivals.forEach((rival) => {
    if (rival.archetype !== "indie" && rand() < 0.7) {
      ...
      const rivalStreamer: import("@/engine/types").StreamerPlatform = {
        ...
        marketShare: rival.archetype === "major" ? 0.3 : 0.12,
  ...
  const rivalsRecord = rivals.reduce(
    (acc, r) => {
      acc[r.id] = r;
      return acc;
    },
  ...
  rivals.forEach((rival) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      starterVault.push({
        ...
        rightsOwner: "RIVAL",
        ownerStudioId: rival.id as unknown as StudioId,
      } as unknown as IPAssest);
    }
  });
