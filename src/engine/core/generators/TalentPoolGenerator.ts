...
export function generateTalentPoolWithRelationships(
  ...
  talentAgentRelationships: Record<
    string,
    import("./talentAgentInteractions").TalentAgentRelationship
  >;
} {
  const { talentCount = 500, agencyCount = 5, agentsPerAgency = 4, familyCount = 5 } = options;
  ...
  const talentPool = talentPoolArray.reduce(
    (acc, t) => {
  ...
  const talentAgentRelationships: Record<
    string,
    import("./talentAgentInteractions").TalentAgentRelationship
  > = {};
...
function derivePersonalityFromAgent(
  agent: import("../../types/talent.types").Agent
): import("./talentAgentInteractions").AgentPersonality {
