export const MARKET_HEADLINES = [
  ...
  'Studios panic as an A.I. correctly predicts {pct}% of test audience feedback',
  'Tech-bro CEO insists {projectName} needs "more engagement velocity" before release',
  'Analysts say the mid-budget {genre} film is dead, then back again, then dead',
  'Merchandising revenue for {projectName} drops {pct}% after toys are found to be haunted',
  'Focus group demands protagonist in {projectName} vape on-screen to appeal to Gen-Z',
  'Box office magically saved by a {pct}% surge in ironic hate-watching',
  'Algorithm mistakenly greenlights a 4-hour {genre} epic, studio rebrands it as a "limited series"',
  'Studios announce plan to replace all background actors with holograms to save ${amount}M'
];
...
export const TALENT_HEADLINES = [
  ...
  'Crew on {projectName} complains that the \'revolutionary\' new lighting rig is just an overpriced ring light.',
  '{actorName} demands top billing over the CGI monster in {projectName}',
  'Lead actress goes full method for {projectName}, officially legally changing her name to her character\'s',
  '{directorName} uses A.I. to write their award acceptance speech, accidentally thanks a vacuum cleaner brand'
];
...
export const RIVAL_TEMPLATES = [
  ...
  '{rival} announces their new ${budget}M {genre} cinematic universe will skip theaters entirely'
];
...
export const BOX_OFFICE_MILESTONES = [
  ...
  '{projectName} miraculously breaks even, studio immediately greenlights a ${amount}M sequel',
  '{projectName} crosses ${amount}M, proving audiences will watch literally anything if it has explosions',
  'Despite terrible reviews, {projectName} grosses ${amount}M thanks to a manufactured meme trend'
];
...
export const STREAMING_RECORDS = [
  ...
  '{projectName} breaks {platform} record, mostly because audiences fell asleep with it on',
  '{platform} claims {projectName} was watched for {amount}M hours, refuses to show the math',
  'Viral hate-watching propels {projectName} to the top of {platform}\'s weekly charts'
];
...
export const SCANDAL_TEMPLATES = [
  ...
  'Leaked Slack messages reveal {directorName} begging studio to cancel {projectName}',
  '{actorName} goes on unhinged podcast rant, PR team for {projectName} goes into hiding',
  'Studio accidentally tweets the password to their main corporate account while promoting {projectName}'
];
...
export const DISPUTE_TEMPLATES = [
  ...
  'Writers threaten strike after {directorName} attempts to credit an LLM on {projectName}',
  '{actorName} sues studio after realizing their face was deepfaked onto a stunt double for {projectName}',
  'Merchandising war: {rival} sues over confusingly similar toys for {projectName}'
];
