 ...
 */
export function editForRating(
  project: Project,
  state: GameState,
  targetRemoval: ContentFlag
): EditRatingResult {
...
export function requestStudioEdit(
            ...
            options: [
              {
                ...
                effectDescription:
                  "Pay the director to relinquish final cut on this specific edit.",
  ...
  if (request.directorId) {
    ...
    if (director) {
      const moodDelta =
        request.directorArchetype === "commercial_hack"
          ? 2
          : request.directorArchetype === "journeyman"
            ? -5
            : request.directorArchetype === "visionary"
              ? -10
              : request.directorArchetype === "auteur"
                ? -15
                : -5;
