...
export function checkDirectorsCutEligibility(
  ...
  const weeksSinceRelease =
    project.releaseWeek !== null ? currentWeek - (project.releaseWeek ?? 0) : 0;
  const minWeeks = 4;
...
export function releaseDirectorsCut(
  ...
  const revenueShare = 0.2 + rng.next() * 0.2; // 20-40%
...
export function releaseUnrated(
  ...
  const updatedCuts = [
    ...(project.availableCuts || []).filter((c) => c.type !== "unrated"),
    unratedCut,
  ];
  ...
  const hasExtremeStack =
    flags.includes("gore") && flags.includes("sexual_content") && flags.includes("nudity");
  if (hasExtremeStack && rng.next() < 0.4) {
    impacts.push({
      newsEvents: [
        {
          id: rng.uuid("NWS"),
