...
export class ExpenseProcessor {
  ...
  static calculateStudioBurn(level: number, activeProjectsCount: number): number {
    // The Studio Comptroller: Increased base rent to 2M and levelScale base to 1.85 to further penalize passive empires.
    const baseRent = 2000000;
    const levelScale = Math.pow(1.85, Math.max(0, level - 1));
