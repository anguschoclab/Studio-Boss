 ...
 */
export function tickFinance(state: GameState): StateImpact[] {
  ...
  const productionBurn = activeProjects
    ...
    .reduce((sum, p) => sum + (p.budget / p.productionWeeks), 0);
  ...
  const revenue = activeProjects
    ...
    .reduce((sum, p) => sum + (p.revenue / 10), 0); // Placeholder weekly revenue
