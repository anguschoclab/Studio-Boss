...
export class RevenueProcessor {
    ...
    projects.forEach(p => {
      ...
      if (p.state === 'released') {
        ...
        if (p.distributionStatus === 'theatrical') {
          // The Studio Comptroller: Base theatrical decay rate lowered from 0.18 to 0.15 representing an incredibly ruthless front-loaded drop.
  ...
  static calculateTheatricalDecay(currentRevenue: number, decayRate: number, isCultClassic: boolean = false): number {
    ...
    if (isCultClassic) {
      ...
      revenue = Math.max(revenue * 2.0, 50000);
    }
    return revenue;
  }
