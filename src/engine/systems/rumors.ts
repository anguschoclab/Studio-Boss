...
export function advanceRumors(state: GameState, rng: RandomGenerator): StateImpact {
  ...
  if (rng.next() < 0.05 && currentRumors.filter(r => !r.resolved).length < 3) {
    ...
    if (category === 'talent' && Object.keys(state.industry.talentPool).length > 0) {
      ...
      text = rng.pick(rumors);
    } else if (category === 'rival' && state.industry.rivals.length > 0) {
      const rival = rng.pick(state.industry.rivals);
      const rumors = [
      ...
      const project = rng.pick(Object.values(state.studio.internal.projects));
