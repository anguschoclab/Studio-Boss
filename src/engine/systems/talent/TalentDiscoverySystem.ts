...
export function tickTalentDiscoverySystem(state: GameState, rng: RandomGenerator): StateImpact[] {
  ...
  // ⚡ Bolt Optimization: Pre-group contracts by projectId to avoid O(N*M) lookups inside the projects loop
  ...
  // ⚡ Bolt Optimization: Replaced Object.values().filter() array allocation with direct for...in loop
  ...
  // ⚡ Bolt Optimization: Replaced Object.values().filter() with direct for...in loops
  ...
  if (rng.next() < DISCOVERY_CHANCE) {
    // ⚡ Bolt Optimization: Replaced Object.values().filter() with direct for...in loop
  ...
  // ⚡ Bolt Optimization: Replaced Object.values() with direct for...in loop
