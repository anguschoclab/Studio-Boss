...
// Addition bonuses
const ADDITION_BONUSES: Record<
  ProductionAdditionType,
  { quality: number; risk: number; marketing: number }
> = {
...
function generateCreditScenes(
  ...
  const unlockConditions: Array<
    "franchise_member" | "boxoffice_threshold" | "sequels" | "standalone"
  > = ["franchise_member", "boxoffice_threshold", "sequels", "standalone"];
...
function generateCreditSceneDescription(type: CreditSceneType, rng: RandomGenerator): string {
  const descriptions: Record<CreditSceneType, string[]> = {
    ...
    joke: ["Comedic button on the story", "Funny outtake moment", "Meta-commentary on the film"],
    emotional_button: [
