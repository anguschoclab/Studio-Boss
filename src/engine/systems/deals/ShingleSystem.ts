...
function rankBidders(state: GameState): { id: string; archetype: 'major' | 'mid-tier' | 'indie'; cash: number; prestige: number }[] {
  const bidders: { id: string; archetype: 'major' | 'mid-tier' | 'indie'; cash: number; prestige: number }[] = [];
...
function createShingle(
  ...
  for (const id in contracts) {
    const c = contracts[id] as import('../../types').Contract;
  ...
  impacts.push({ type: 'SHINGLE_CREATED', payload: { shingle } } as unknown as import("../../types").StateImpact);
...
function decrementTerms(state: GameState, rng: RandomGenerator, impacts: StateImpact[]) {
  ...
  for (const id in shingles) {
    ...
    if (next > 0) {
      impacts.push({ type: 'SHINGLE_UPDATED', payload: { shingleId: s.id, update: { termWeeksRemaining: next } } } as unknown as import("../../types").StateImpact);
      continue;
    }
...
function handleExpiry(state: GameState, s: ProducerShingle, rng: RandomGenerator, impacts: StateImpact[]) {
  ...
  if (!owner) {
    impacts.push({ type: 'SHINGLE_DISSOLVED', payload: { shingleId: s.id } } as unknown as import("../../types").StateImpact);
    shingleEventLog.push({
  ...
  if (offers.length === 0) {
    ...
    impacts.push({ type: 'SHINGLE_UPDATED', payload: { shingleId: s.id, update: { baseStudioId: null, termWeeksRemaining: 52, dealType: 'HOUSEKEEPING', overheadPerYear: 0 } } } as unknown as import("../../types").StateImpact);
    shingleEventLog.push({
