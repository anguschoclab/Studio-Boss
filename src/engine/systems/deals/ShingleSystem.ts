...
function makeShingleName(ownerName: string, rng: RandomGenerator): string {
  ...
  if (style < 0.7)
    return `${rng.pick(prefixes)}${rng.pick(["", "ed", "er", "star", "rock", "wave"])} ${rng.pick(suffixes)}`;
...
function rankBidders(
  state: GameState
): { id: string; archetype: string; cash: number; prestige: number }[] {
  ...
  bidders.push({
    ...
    prestige: state.studio?.prestige || 0,
  });
...
function getStudioDealCounts(state: GameState): Record<string, number> {
  const counts: Record<string, number> = {};
  const shingles = state.entities.shingles || {};
  for (const id in shingles) {
    const sId = shingles[id].baseStudioId;
...
function createShingle(
  ...
  for (const id in contracts) {
    ...
    if (
      c.talentId === owner.id &&
      (isPlayerOwner(state, c.ownerId) || !c.ownerId) &&
      ((c.signedWeek || 0) >= twoYearWeek || (c.weeksRemaining || 0) > 0)
    ) {
  ...
  const studioDealCounts = getStudioDealCounts(state);
  for (const b of bidders) {
    if (medium === "TV" && b.archetype === "indie") continue;
    const existing = studioDealCounts[b.id] || 0;
    let dealType = proposeDealType(
      b.archetype as "major" | "mid-tier" | "indie",
      b.cash,
      b.prestige,
      owner.prestige || 0,
      existing,
    ...
    const streamerBias =
      medium === "TV" && streamerOwners.has(b.id)
        ? streamingEraWeek(state.week)
          ? 20_000_000
          : 5_000_000
        : 0;
    const existingContractBias =
      isPlayerOwner(state, b.id) && playerHasRecentContractWithOwner ? 20_000_000 : 0;
    const score =
      overhead * archetypeWeight +
      b.prestige * 50_000 +
      streamerBias +
      existingContractBias +
      rng.next() * 500_000;
  ...
  const topTierOfferExists = offers.some(
    (o) => o.dealType === "OVERALL" || o.dealType === "FIRST_LOOK"
  );
  const wantsFallback =
    medium === "FILM" && (owner.prestige || 0) < 60 && (offers.length === 0 || !topTierOfferExists);
  if (wantsFallback) {
    for (const b of bidders) {
      if (b.cash < 20_000_000) continue;
      if ((studioDealCounts[b.id] || 0) >= 4) continue;
      ...
      const archetypeWeight =
        b.archetype === "major" ? 1.2 : b.archetype === "mid-tier" ? 1.0 : 0.8;
      offers.push({
        id: b.id,
        dealType: "HOUSEKEEPING",
        overhead,
        score: overhead * archetypeWeight + rng.next() * 500_000,
      });
    }
  }
  ...
  shingleEventLog.push({
    ...
    kind: "formed",
    shingleId: shingle.id,
    shingleName: shingle.name,
    ownerName: owner.name,
    studioId: winner.id,
    studioName: getStudioName(state, winner.id),
    dealType: winner.dealType,
    overheadPerYear: winner.overhead,
    termYears: Math.round(termWeeks / 52),
    medium,
  });
  return shingle;
}
  ...
  for (const id in shingles) {
    ...
    if (next > 0) {
      impacts.push({
        ...
        payload: { shingleId: s.id, update: { termWeeksRemaining: next } },
      });
      continue;
    }
...
function handleExpiry(
  ...
  for (const b of bidders) {
    ...
    if (!dealType && b.id === s.baseStudioId && b.cash > s.overheadPerYear * 3)
      dealType = s.dealType;
    ...
    if (b.id === s.baseStudioId && dealType === s.dealType) {
      const escalation = hasHits ? 1 + 0.05 + rng.next() * 0.1 : 1 + rng.next() * 0.03;
      overhead = Math.max(
        overhead,
        Math.round((s.overheadPerYear * escalation) / 100_000) * 100_000
      );
    }
    if (overhead * 3 > b.cash) continue;
    const archetypeWeight = b.archetype === "major" ? 1.4 : b.archetype === "mid-tier" ? 1.0 : 0.6;
  ...
  if (offers.length === 0) {
    ...
    shingleEventLog.push({
      ...
      shingleName: s.name,
      ownerName: owner.name,
      studioName: getStudioName(state, s.baseStudioId),
      note: "No suitor, became free agent",
    });
    return;
  }
  offers.sort((a, b) => b.score - a.score);
  ...
  const trail = [
    ...(s.historyTrail || []),
    { week: state.week, studioId: winner.id, dealType: winner.dealType, overhead: winner.overhead },
  ];
  impacts.push({
    type: "SHINGLE_UPDATED",
    payload: {
  ...
  impacts.push({
    ...
    payload: {
      ...
      description: churned
        ? `${s.name} has moved its banner from ${prior} to ${now}.`
        : `${s.name} extends with ${now}.`,
      category: "market",
    },
  });
  shingleEventLog.push({
...
function spawnShingles(state: GameState, rng: RandomGenerator, impacts: StateImpact[]) {
  ...
  for (const id in talents) {
    ...
    if (p >= 30 && p < 50) {
      const roles = t.roles || [t.role];
      if (
        roles.some((r) => r === "director" || r === "actor" || r === "producer" || r === "writer")
      ) {
        lowProspect.push(t);
      }
    }
...
function createPodShingle(
  ...
  for (const b of bidders) {
    ...
    if ((studioDealCounts[b.id] || 0) >= 5) continue;
  ...
  const shingle: ProducerShingle = {
    ...
    medium: "FILM",
    historyTrail: [
      { week: state.week, studioId: winner.id, dealType: "POD", overhead: winner.overhead },
    ],
  };
  impacts.push({ type: "SHINGLE_CREATED", payload: { shingle } });
  impacts.push({
 ...
 */
function createHousekeepingShingle(
  state: GameState,
  owner: Talent,
  rng: RandomGenerator,
  impacts: StateImpact[]
 ...
 */
export function cancelHighestOverheadDeal(
  state: GameState,
  studioId: string
): StateImpact[] | null {
  ...
  shingleEventLog.push({
    ...
    kind: "cancelled",
    shingleId: target.id,
    shingleName: target.name,
    ownerName: owner?.name || "unknown",
    studioId,
    studioName: getStudioName(state, studioId),
    overheadPerYear: target.overheadPerYear,
    note: "Distress stage-2 cancellation",
  });
  return impacts;
}
