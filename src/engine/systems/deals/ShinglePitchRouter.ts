...
export function tickShinglePitchRouter(state: GameState, rng: RandomGenerator): StateImpact[] {
  ...
  for (const shingleId in shingles) {
    ...
    const entry: ShinglePitchOutcome = {
      week: state.week,
    ...
    const existingLog = getSimMemory(state).eventLogs.pitch;
    impacts.push(I.industryUpdate({ "simMemory.eventLogs.pitch": [...existingLog, entry] }));
