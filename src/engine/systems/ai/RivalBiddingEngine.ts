...
export function generateFestivalBid(
  ...
  const behaviorId = rival.archetypeId || rival.behaviorId || "major";
  ...
  let interest =
    (reviewScore * (archetype.awardObsession / 100) + buzz * (1 - archetype.awardObsession / 100)) /
    100;
  ...
  if (
    ...
    ["Sci-Fi", "Action", "Fantasy"].includes(project.genre)
  ) {
    interest *= 2.0;
  }
  ...
  if (
    rival.currentMotivation === "CASH_CRUNCH" &&
    buzz > 70 &&
    project.budget < rival.cash * 0.15
  ) {
    interest *= 1.8;
  }
...
export function assignRivalTimeSlot(rival: RivalStudio, project: Project): TimeSlot | null {
