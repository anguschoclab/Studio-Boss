...
export function calculateFranchiseEquity(
  ...
  if (
    assets.length >= 6 &&
    genres.some((g) => g === "Superhero" || g === "Video Game Adaptation")
  ) {
    crossoverBonus += 0.4;
  }
  ...
  if (
    assets.length >= 10 &&
    franchise.relevanceScore < 50 &&
    franchise.activeProjectIds.length >= 3
  ) {
    crossoverBonus -= 0.25;
  }
  ...
  const overSaturationPenalty =
    franchise.activeProjectIds && franchise.activeProjectIds.length >= 4 ? 0.8 : 1.0;
