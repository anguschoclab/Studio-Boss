...
const ANGLE_MULTIPLIERS: Record<string, (p: Project) => number> = {
  ...
  SELL_THE_ROMANCE: (p) => {
    const genre = (p.genre || "").toLowerCase();
    return genre.includes("romance") || genre.includes("drama") ? 1.2 : 1.0;
  },
  SELL_THE_WORLD_MYTHOLOGY: (p) => (p.franchiseId ? 1.3 : 1.05),
 ...
 */
export function computeAngleMultiplier(angle: ExtendedMarketingAngle, project: Project): number {
...
export function computeCampaignMultiplier(
  ...
  if (secondaryAngle && secondaryAngle !== primaryAngle) {
    ...
    secondaryContribution = (secondaryMult - 1.0) * 0.3;
  }
...
export function handleMarketingPhase(
  p: Project,
  ...
  if (p.marketingCampaign) {
    const campaignWithSecondary = p.marketingCampaign as MarketingCampaign & {
      secondaryAngle?: ExtendedMarketingAngle;
    };
