...
export function handleUnscriptedReleasedPhase(
  ...
  const impacts: StateImpact[] = [];
  const formatData =
    UNSCRIPTED_FORMATS[p.unscriptedFormat as keyof typeof UNSCRIPTED_FORMATS] ||
    UNSCRIPTED_FORMATS["competition"];
  const eps = formatData.defaultEpisodes;
  const currentRevenue = p.revenue + p.weeklyRevenue;
  const newWeeklyRevenue =
    p.weeklyRevenue *
    rng.range(formatData.revenueDecayWeekly - 0.05, formatData.revenueDecayWeekly + 0.05) *
    franchiseSynergy;
