...
export function tickPostProduction(
  ...
  for (const projectId in state.entities.projects) {
    ...
    const currentWeeksRemaining: number =
      (project as unknown as Record<string, number>).postProductionWeeksRemaining ??
    ...
    if (
      ...
      !(project as unknown as Record<string, boolean>).directorsCutNotified
    ) {
      impacts.push({
    ...
    if (weeksRemaining === 0) {
      impacts.push({
        ...
        payload: {
          projectId: project.id,
          update: {
            state: 'marketing' as unknown as import("@/engine/types").ProjectStatus,
