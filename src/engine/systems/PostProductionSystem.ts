...
export function tickPostProduction(
  ...
  for (const projectId in state.entities.projects) {
    ...
    const currentWeeksRemaining: number =
      (project as Project & { postProductionWeeksRemaining?: number }).postProductionWeeksRemaining ??
    ...
    if (
      ...
      !(project as Project & { directorsCutNotified?: boolean }).directorsCutNotified
    ) {
      impacts.push({
        type: 'PROJECT_UPDATED',
        payload: {
          projectId: project.id,
          update: { directorsCutNotified: true } as unknown as Partial<Project>,
        },
      });
    ...
    if (weeksRemaining === 0) {
      impacts.push({
        ...
        payload: {
          projectId: project.id,
          update: {
            state: 'marketing' as unknown as Project['state'],
