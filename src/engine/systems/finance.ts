  ...
  projects.forEach(p => {
    if (p.state === 'released') {
      if (p.distributionStatus === 'theatrical') {
        // The Studio Comptroller: Synchronized theatrical decay rate down to 0.24 to simulate even more ruthless modern front-loaded box office drops.
        boxOffice += RevenueProcessor.calculateTheatricalDecay(p.weeklyRevenue || 0, 0.24, p.isCultClassic);
