...
export function generateWeeklyFinancialReport(
  ...
  projects.forEach((p) => {
    if (p.state === "released") {
      ...
      if (genreTrend) {
        ...
        if (trendMultiplier !== 1.0) {
          causality.push({
            ...
            description:
              genreTrend.heat >= 60
                ? `${p.genre} is trending hot this season`
                : `${p.genre} has gone stale in the market`,
          });
        }
      }
      ...
      if (p.distributionStatus === "theatrical") {
        weeklyGross =
          RevenueProcessor.calculateTheatricalDecay(p.weeklyRevenue || 0, 0.35) * trendMultiplier; // The Studio Comptroller: Severely increased theatrical decay to 0.35 to accurately model modern front-loaded box office drops.
      ...
      const weeklyMerch = RevenueProcessor.calculateMerchRevenue(
        p.buzz,
        franchise?.relevanceScore || 0
      );
      merch += weeklyMerch;
      ...
      totalRoyalties += RevenueProcessor.calculateNetPointsRoyalty(
        p,
  ...
  const overhead = ExpenseProcessor.calculateStudioBurn(
    studioLevel,
    projects.filter((p) => p.state !== "released").length
  );
