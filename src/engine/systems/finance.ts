  ...
  projects.forEach(p => {
    if (p.state === 'released') {
      if (p.distributionStatus === 'theatrical') {
        // The Studio Comptroller: Synchronized theatrical decay rate to 0.28 to simulate ruthless modern front-loaded box office drops.
        boxOffice += RevenueProcessor.calculateTheatricalDecay(p.weeklyRevenue || 0, 0.28, p.isCultClassic);
