  ...
  projects.forEach(p => {
    if (p.state === 'released') {
      if (p.distributionStatus === 'theatrical') {
        // The Studio Comptroller: Synchronized theatrical decay rate to 0.22 to simulate incredibly ruthless modern front-loaded box office drops, forcing high initial marketing.
        boxOffice += RevenueProcessor.calculateTheatricalDecay(p.weeklyRevenue || 0, 0.22, p.isCultClassic);
