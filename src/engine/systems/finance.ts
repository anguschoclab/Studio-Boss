  ...
  projects.forEach(p => {
    if (p.state === 'released') {
      if (p.distributionStatus === 'theatrical') {
        // The Studio Comptroller: Synchronized theatrical decay rate to 0.25 to simulate ruthless modern front-loaded box office drops.
        boxOffice += RevenueProcessor.calculateTheatricalDecay(p.weeklyRevenue || 0, 0.25, p.isCultClassic);
