...
export function generateAwardsProfile(project: Project): AwardsProfile {
  const basePrestige = (rand() * 50) + (project.budget / 1000000) * 0.5;
  const baseCritic = rand() * 100;

  return {
    criticScore: Math.min(100, Math.max(0, baseCritic)),
    audienceScore: Math.min(100, Math.max(0, rand() * 100)),
    prestigeScore: Math.min(100, Math.max(0, basePrestige)),
    craftScore: Math.min(100, Math.max(0, rand() * 100)),
    culturalHeat: Math.min(100, Math.max(0, rand() * 100)),
    campaignStrength: 10,
    controversyRisk: Math.min(100, Math.max(0, rand() * 30)),
    festivalBuzz: Math.min(100, Math.max(0, rand() * 100)),
    academyAppeal: Math.min(100, Math.max(0, basePrestige * 0.8 + rand() * 40)),
    guildAppeal: Math.min(100, Math.max(0, baseCritic * 0.7 + rand() * 40)),
    populistAppeal: Math.min(100, Math.max(0, rand() * 100)),
    indieCredibility: Math.min(100, Math.max(0, project.budgetTier === 'low' ? rand() * 80 + 20 : rand() * 30)),
    industryNarrativeScore: Math.min(100, Math.max(0, rand() * 100))
  };
}
