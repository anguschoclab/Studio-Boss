...
export const FESTIVAL_BY_BODY = new Map<AwardBody, Festival>(FESTIVALS.map((f) => [f.body, f]));
...
export function resolveFestivals(state: GameState): StateImpact {
  if (!state.industry.festivalSubmissions || state.industry.festivalSubmissions.length === 0)
    return {};
  ...
  impact.newFestivalSubmissions = updatedSubmissions.filter(
    (s) => s.status !== "rejected" || state.week - s.week < 12
  );
  return impact;
}
