...
function alreadyUnlocked(state: GameState, id: string): boolean {
  const unlocked: string[] = (state.studio as unknown as Record<string, string[]>).achievements ?? [];
...
export function checkAchievements(state: GameState): StateImpact[] {
  ...
  for (const id in state.entities.talents) {
    ...
    if (t.tier === 'A_LIST' && (t as unknown as Record<string, boolean>).wasNewcomerWhenSigned === true) {
      hasStarMaker = true;
      break;
    }
  }
  ...
  const resolvedCrises: number = (state.studio as unknown as Record<string, number>).resolvedCrisesCount ?? 0;
