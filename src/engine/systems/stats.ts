...
export function getFilmStats(tier: typeof BUDGET_TIERS[keyof typeof BUDGET_TIERS]) {
  // The Studio Comptroller: Increased extreme risk multiplier for $200M+ films (to 4.0), $100M+ films (to 2.0), and $50M+ films (to 1.2).
  const riskMultiplier = tier.budget >= 200_000_000 ? 4.0 : tier.budget >= 100_000_000 ? 2.0 : tier.budget >= 50_000_000 ? 1.2 : 1.0;
...
export function getTvStats(tier: typeof BUDGET_TIERS[keyof typeof BUDGET_TIERS], tvFormatData: typeof TV_FORMATS[keyof typeof TV_FORMATS], episodes: number) {
  // The Studio Comptroller: Steeper scaling for mega-budget TV risks (e.g., 3.0x for >=150M, 2.5x for >=100M, and 1.8x for >50M).
  const scaleMultiplier = tier.budget >= 150_000_000 ? 3.0 : tier.budget >= 100_000_000 ? 2.5 : tier.budget > 50_000_000 ? 1.8 : 1.0;
...
export function getUnscriptedStats(tier: typeof BUDGET_TIERS[keyof typeof BUDGET_TIERS], unscriptedFormatData: typeof UNSCRIPTED_FORMATS[keyof typeof UNSCRIPTED_FORMATS], episodes: number) {
  // The Studio Comptroller: Increased scaling for massive unscripted logistics overhead (e.g., 2.5x for >=100M, 1.8x for >50M).
  const scaleMultiplier = tier.budget >= 100_000_000 ? 2.5 : tier.budget > 50_000_000 ? 1.8 : 1.0;
