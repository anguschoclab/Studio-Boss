...
export function getFilmStats(tier: typeof BUDGET_TIERS[keyof typeof BUDGET_TIERS]) {
  ...
  const riskMultiplier = tier.budget >= 200_000_000 ? 12.0 : tier.budget >= 100_000_000 ? 6.0 : tier.budget >= 50_000_000 ? 2.5 : 1.0; // The Studio Comptroller: Geometrically penalized large budgets to enforce steep risk scaling.
...
export function getTvStats(tier: typeof BUDGET_TIERS[keyof typeof BUDGET_TIERS], tvFormatData: typeof TV_FORMATS[keyof typeof TV_FORMATS], episodes: number) {
  ...
  const scaleMultiplier = tier.budget >= 150_000_000 ? 7.5 : tier.budget >= 100_000_000 ? 5.0 : tier.budget > 50_000_000 ? 3.0 : 1.0; // The Studio Comptroller: Geometrically penalized large TV budgets.
...
export function getUnscriptedStats(tier: typeof BUDGET_TIERS[keyof typeof BUDGET_TIERS], unscriptedFormatData: typeof UNSCRIPTED_FORMATS[keyof typeof UNSCRIPTED_FORMATS], episodes: number) {
  ...
  const scaleMultiplier = tier.budget >= 100_000_000 ? 6.0 : tier.budget > 50_000_000 ? 3.5 : 1.0; // The Studio Comptroller: Severely increased unscripted overhead to penalize sprawling productions.
