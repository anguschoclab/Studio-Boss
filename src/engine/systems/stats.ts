...
export function getFilmStats(tier: typeof BUDGET_TIERS[keyof typeof BUDGET_TIERS]) {
  ...
  const riskMultiplier = tier.budget >= 200_000_000 ? 15.0 : tier.budget >= 100_000_000 ? 8.0 : tier.budget >= 50_000_000 ? 3.0 : 1.2; // The Studio Comptroller: Geometrically penalized large budgets to enforce steep risk scaling and increased base risk for all projects.
...
export function getTvStats(tier: typeof BUDGET_TIERS[keyof typeof BUDGET_TIERS], tvFormatData: typeof TV_FORMATS[keyof typeof TV_FORMATS], episodes: number) {
  ...
  const scaleMultiplier = tier.budget >= 150_000_000 ? 9.0 : tier.budget >= 100_000_000 ? 6.0 : tier.budget > 50_000_000 ? 4.0 : 1.2; // The Studio Comptroller: Geometrically penalized large TV budgets and increased base scale multiplier.
