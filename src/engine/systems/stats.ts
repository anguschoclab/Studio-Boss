...
export function getFilmStats(tier: typeof BUDGET_TIERS[keyof typeof BUDGET_TIERS]) {
  // The Studio Comptroller: Increased risk multipliers to penalize high budgets and increase stakes.
  const riskMultiplier = tier.budget >= 200_000_000 ? 6.0 : tier.budget >= 100_000_000 ? 2.5 : tier.budget >= 50_000_000 ? 1.4 : 1.0;
...
export function getTvStats(tier: typeof BUDGET_TIERS[keyof typeof BUDGET_TIERS], tvFormatData: typeof TV_FORMATS[keyof typeof TV_FORMATS], episodes: number) {
  // The Studio Comptroller: Increased scale multiplier to make high budget TV riskier.
  const scaleMultiplier = tier.budget >= 150_000_000 ? 4.0 : tier.budget >= 100_000_000 ? 1.8 : tier.budget > 50_000_000 ? 1.3 : 1.0;
...
export function getUnscriptedStats(tier: typeof BUDGET_TIERS[keyof typeof BUDGET_TIERS], unscriptedFormatData: typeof UNSCRIPTED_FORMATS[keyof typeof UNSCRIPTED_FORMATS], episodes: number) {
  // The Studio Comptroller: Increased scale multiplier to ensure unscripted high budgets are suitably risky.
  const scaleMultiplier = tier.budget >= 100_000_000 ? 2.5 : tier.budget > 50_000_000 ? 1.4 : 1.0;
