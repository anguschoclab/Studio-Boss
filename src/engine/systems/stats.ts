...
export function getFilmStats(tier: typeof BUDGET_TIERS[keyof typeof BUDGET_TIERS]) {
  // The Studio Comptroller: Further increased risk multipliers for high-budget films to make tentpoles genuinely risky while maintaining stakes. Increased blockbuster to 8.0 and high tier to 3.0.
  const riskMultiplier = tier.budget >= 200_000_000 ? 8.0 : tier.budget >= 100_000_000 ? 3.0 : tier.budget >= 50_000_000 ? 1.5 : 1.0;
...
export function getTvStats(tier: typeof BUDGET_TIERS[keyof typeof BUDGET_TIERS], tvFormatData: typeof TV_FORMATS[keyof typeof TV_FORMATS], episodes: number) {
  // The Studio Comptroller: Increased TV scaling to add more risk to prestige and blockbuster television. Blockbuster TV scaling increased to 5.0 and high tier TV to 3.0.
  const scaleMultiplier = tier.budget >= 150_000_000 ? 5.0 : tier.budget >= 100_000_000 ? 3.0 : tier.budget >= 50_000_000 ? 1.5 : 1.0;
...
export function getUnscriptedStats(tier: typeof BUDGET_TIERS[keyof typeof BUDGET_TIERS], unscriptedFormatData: typeof UNSCRIPTED_FORMATS[keyof typeof UNSCRIPTED_FORMATS], episodes: number) {
  // The Studio Comptroller: Increased scale multiplier for large unscripted formats to reflect bloated modern reality TV costs. High tier increased to 3.0.
