...
export function stage1IPFireSale(
  ...
  // ⚡ Bolt: Replaced chained array creation with single-pass for/in loop
  ...
  // ⚡ Bolt: Replaced Object.values(...).filter(...) with O(N) single-pass iteration
...
function stage2AssetLiquidation(state: GameState, seller: RivalStudio): StateImpact[] {
  ...
  if (roll < 0.78) {
    ...
    // ⚡ Bolt: Replaced Object.values(...).filter(...) with direct single-pass Array loop
    const vaultArr = state.ip?.vault || [];
    const ownedAssets: any[] = [];
    for (let i = 0; i < vaultArr.length; i++) {
      if (vaultArr[i].ownerStudioId === (seller.id as any)) {
    ...
    if (ownedAssets.length >= 2) {
      ...
      for (const id in rivalsDict) {
        if (Object.prototype.hasOwnProperty.call(rivalsDict, id)) {
          const r = rivalsDict[id];
          if (r.id !== seller.id && (r.cash || 0) > 300_000_000) {
            buyers.push(r);
          }
        }
      }
      ...
      // ⚡ Bolt: Replaced O(N^2) .map().find() with O(N) Set lookup
