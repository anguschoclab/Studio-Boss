      ...
      <Dialog open={showLoad} onOpenChange={setShowLoad}>
        <DialogContent className="max-w-4xl bg-black/95 backdrop-blur-3xl border border-white/5 rounded-none p-0 overflow-hidden shadow-2xl">
          <div className="p-10 border-b border-white/5 bg-white/[0.02]">
            <DialogHeader>
              <DialogTitle className="text-4xl font-display font-black tracking-tighter uppercase italic leading-none drop-shadow-[0_0_20px_rgba(255,255,255,0.1)]">LOAD ARCHIVES</DialogTitle>
                ...
                {slot.exists ? (
                  <div className="space-y-6 relative z-10">
                    <div className="space-y-2">
                       <span className="text-[9px] font-black text-primary/40 uppercase tracking-[0.3em] italic">SLOT {String(slot.slot + 1).padStart(2, '0')}</span>
                       <h3 className="text-xl font-display font-black text-foreground uppercase tracking-tight italic group-hover:text-primary transition-colors">{slot.studioName}</h3>
                    </div>
                    
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <p className="text-[8px] font-black text-muted-foreground/30 uppercase tracking-[0.2em] italic">TIMELINE</p>
                        <p className="text-[10px] font-black text-foreground/60 uppercase tracking-widest italic">
                          W{getWeekDisplay(slot.week).displayWeek} • Y{getWeekDisplay(slot.week).year}
                        </p>
                      </div>
                      <div className="space-y-1 text-right">
                        <p className="text-[8px] font-black text-muted-foreground/30 uppercase tracking-[0.2em] italic">REVENUE</p>
                        <p className="text-sm font-display font-black text-primary italic leading-none">{formatMoney(slot.cash)}</p>
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="flex flex-col items-center justify-center h-32 gap-4 opacity-20 group-hover:opacity-40 transition-opacity">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-[0.4em] italic">VACANT MODULE {slot.slot + 1}</p>
                  </div>
                )}
              </button>
            ))}
