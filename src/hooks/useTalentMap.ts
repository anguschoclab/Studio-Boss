 ...
 */
export function useAgencyMap(): Map<string, import('@/engine/types').Agency> {
  const gameState = useGameStore(s => s.gameState);
  
  return useMemo(() => {
    const agencies = gameState?.industry?.agencies || [];
    const agenciesArray = Array.isArray(agencies) ? agencies : Object.values(agencies);
    return new Map(agenciesArray.map((a: import('@/engine/types').Agency) => [a.id, a]));
