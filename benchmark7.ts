...
for (let i = 0; i < 100; i++) { original(); optimized7(); }
...
for (let i = 0; i < ITERATIONS; i++) { optimized7(); }
const endOptimized = performance.now();

console.log(`Original: ${(endOriginal - startOriginal).toFixed(2)} ms`);
console.log(`Optimized7: ${(endOptimized - startOptimized).toFixed(2)} ms`);
console.log(`Improvement: ${(((endOriginal - startOriginal) - (endOptimized - startOriginal)) / (endOriginal - startOriginal) * 100).toFixed(2)}% faster`);
