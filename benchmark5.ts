...
function optimized5() {
  ...
  for (const t of talentPool) {
    ...
    for (const r of t.roles) {
      ...
      if (group) {
        if (!attachedChecked) {
            isAttached = projectTalentIds.has(t.id);
            attachedChecked = true;
        }
        if (isAttached) {
          group.attached.push(t);
        } else {
...
for (let i = 0; i < 100; i++) { original(); optimized5(); }
...
for (let i = 0; i < ITERATIONS; i++) { optimized5(); }
const endOptimized = performance.now();

console.log(`Original: ${(endOriginal - startOriginal).toFixed(2)} ms`);
console.log(`Optimized5: ${(endOptimized - startOptimized).toFixed(2)} ms`);
