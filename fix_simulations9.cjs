...
code = code.replace(/const \{ newState: steppedState, summary, impacts: engineImpacts \} = WeekCoordinator\.execute\(state, rng\);/g, 'const { newState: steppedState, summary } = WeekCoordinator.execute(state, rng);');
...
code = code.replace(/const projectsList = Object\.values\(state\.entities\.projects \|\| \{\}\);\n/g, '');
