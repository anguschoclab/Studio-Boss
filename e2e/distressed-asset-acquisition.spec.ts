...
test("distressed asset acquisition: modal appears, acquire works, decline works", async ({
  ...
  const storeAccess = await page.evaluate(async () => {
    ...
    const gameStore = (window as Record<string, unknown>).__GAME_STORE__ || (window as Record<string, unknown>).useGameStore;
    const uiStore = (window as Record<string, unknown>).__UI_STORE__ || (window as Record<string, unknown>).useUIStore;
    ...
    try {
      const gameMod = await (window as Record<string, unknown>).import("/src/store/gameStore.ts");
      const uiMod = await (window as Record<string, unknown>).import("/src/store/uiStore.ts");
  ...
  await page.evaluate(() => {
    const store = (window as Record<string, unknown>).__GAME_STORE__ || (window as Record<string, unknown>).useGameStore?.getState?.();
  ...
  await page.evaluate(() => {
    const uiStore = (window as Record<string, unknown>).__UI_STORE__ || (window as Record<string, unknown>).useUIStore?.getState?.();
    if (uiStore) {
