...
code = code.replace(/const archetype = this\.getRivalArchetype\(rival\);/g, '');
...
code = code.replace(/let persona = this\._aiPersonas\.get\(rivalId\);/g, '');
code = code.replace(/const persona = this\._aiPersonas\.get\(rivalId\);/g, '');
code = code.replace(/let engineImpacts = engine\.tick\(nextState\);/g, 'engine.tick(nextState);');
...
code = code.replace(/let projectsList = Object\.values\(result\.state\.entities\.projects\);/g, '');
