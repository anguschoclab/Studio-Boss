...
code = code.replace(/const persona = this\._aiPersonas\.get\(rivalId\);\n/g, '');
code = code.replace(/const engineImpacts = engine\.tick\(nextState\);\n/g, 'engine.tick(nextState);\n');
...
code = code.replace(/const projectsList = Object\.values\(result\.state\.entities\.projects\);\n/g, '');
