...
function optimized9() {
  ...
  for (const t of talentPool) {
    for (const r of t.roles) {
      if (r === 'director' || r === 'actor' || r === 'writer' || r === 'producer') {
        const group = groups.get(r)!;
        if (projectTalentIds.has(t.id)) {
            group.attached.push(t);
        } else {
...
for (let i = 0; i < 100; i++) { original(); optimized9(); }
...
for (let i = 0; i < ITERATIONS; i++) { optimized9(); }
const endOptimized = performance.now();

console.log(`Original: ${(endOriginal - startOriginal).toFixed(2)} ms`);
console.log(`Optimized9: ${(endOptimized - startOptimized).toFixed(2)} ms`);
