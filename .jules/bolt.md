...
## 2026-06-20 - Replace Object.values().map() chain with direct for...in loop for max bid calculation
**Learning:** Using `Math.max(...Object.values(obj).map(fn))` on dictionaries inside hot components (like `LiveAuctionDashboard` or `OpportunityCard`) or tick loops (`biddingEngine`) creates unnecessary intermediate arrays and puts garbage collection pressure on the JS engine.
**Action:** Replaced `Object.values().map()` chains with direct `for...in` loops, keeping a running max. This reduced the time complexity from multiple passes to a single pass with O(1) allocation overhead, optimizing the React render cycle and AI evaluation loops.
