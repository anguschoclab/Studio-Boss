...
## 2026-07-25 - Avoid Object.values().filter().flatMap() chains for dependent entities
**Learning:** Object.values().filter().flatMap() in `generateCashflowForecast` creates intermediate arrays and causes GC overhead, particularly when finding contracts for a filtered subset of projects.
**Action:** Use single-pass for...in loops instead.
