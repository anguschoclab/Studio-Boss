...
## 2026-05-30 - Replace Object.values with for...in loops in CliqueSystem.ts
**Learning:** Functions like `findPotentialCliques`, `tickCliqueSystem`, and `getCliqueFameBonus` in `CliqueSystem.ts` were calling `Object.values()` coupled with `.some()`, `for` loop, or `.filter()` to process GameState relationships records. This caused redundant intermediate array allocations that compound GC spikes in hot paths.
**Action:** Replaced these chains with direct `for...in` loops in `CliqueSystem.ts` to process record entities directly, eliminating O(N) array allocation overhead.
