...
## 2024-05-24 - Prevent massive array allocation in TalentSystem.applyProjectResults
**Learning:** Calling `Object.values()` or `Array.from()` to convert a `Record` or `Map` into an array, just to immediately convert it back into a `Map` inside a helper function, causes severe garbage collection overhead, especially when iterating thousands of entities.
**Action:** Update helper functions like `applyProjectResults` to accept `Record<string, T>` or `Map<string, T>` directly instead of forcing array conversions.
