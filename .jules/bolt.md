...
## 2024-05-18 - Replacing O(N*M) Relationship Lookups with O(1) Pre-grouped Hash Maps
**Learning:** When scanning a large undirected graph of entity edges (like 20,000 talent relationships) to find mutual connections inside an outer O(N) loop (checking 1,000 talents), relying on helper functions like `getTalentRelationships` that iterate `Object.values(state.relationships.relationships)` creates a massive O(N*M) bottleneck. In benchmarks, checking potential cliques took >11 seconds because it repeatedly scanned the entire relationships dictionary for every talent.
**Action:** Before running the outer loop, perform a single O(M) pass over the relationships dictionary to pre-group edges into adjacency lists (`friendsMap = new Map<string, string[]>`). Inside the O(N) loop, fetch the pre-grouped array in O(1) time. This optimization dropped the clique detection execution time from ~11.5s down to ~26ms, representing an enormous >99% performance boost for mid/late game simulations, without sacrificing accuracy or encapsulation.
