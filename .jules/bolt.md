...
## 2026-05-18 - Replace Object.values with for...in for State Records in engine files
**Learning:** Functions like `checkAchievements` and `advanceRivals` frequently called `Object.values` on `state.entities.projects`, `talents`, and `rivals`. Because these functions run every game tick, this resulted in the repeated allocation and garbage collection of large intermediate arrays, causing noticeable performance degradation. Combining multiple sequential loops and avoiding repeated array creation is a big algorithmic win.
**Action:** When iterating through dictionaries in hot paths (like game tick functions), always prefer `for...in` loops over `Object.values().map/.filter` to eliminate memory allocation overhead. Also, try to hoist collection mapping out of nested loops where possible to reduce complexity from O(N*M) to O(N+M).
