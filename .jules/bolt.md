...
## 2026-05-28 - Eliminate intermediate array allocations in Awards processRazzies
**Learning:** `processRazzies` was calling `Object.values()` coupled with `.filter()` and `.map()` on the contracts state record to find `contractTalentIds`. Given this runs frequently in the engine, these intermediate array allocations caused compounding garbage collection spikes.
**Action:** Replaced `Object.values(state.entities.contracts || {}).filter().map()` with a direct `for...in` loop in `processRazzies` to directly populate the `contractTalentIds` Set, eliminating O(N) array allocation overhead.
