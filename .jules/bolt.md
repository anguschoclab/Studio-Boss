...
## 2024-05-18 - ⚡ Pre-computing Shingle Counts per Studio
**Learning:** Calling an O(N) lookup function like `countDealsByStudio` inside an O(M) loop over bidders yields O(N*M) complexity, creating a performance bottleneck when there are many entities.
**Action:** Removed `countDealsByStudio` and introduced a single-pass `dealsByStudio` frequency map computed in O(N) time before bidder loops, changing overall complexity to O(N + M).
