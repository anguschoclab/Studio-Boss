...
## 2023-10-27 - Object.values() and forEach overhead in hot loops
**Learning:** Using `Object.values().forEach()` in high-frequency game loops (like `StudioAutomation.tick`) creates unnecessary intermediate array allocations and closure instantiations, leading to significant GC pressure and frame stuttering. Furthermore, to satisfy strict automated code reviews and prevent prototype pollution, any replacement `for...in` loop must wrap its inner logic with `Object.prototype.hasOwnProperty.call(obj, key)`.
**Action:** Replace `Object.values().forEach()` with a direct `for...in` loop accompanied by a `.hasOwnProperty.call` check when iterating over dictionaries in frequently executed simulation ticks.
