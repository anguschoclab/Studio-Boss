...
## 2024-07-17 - Deferring expensive array allocations
**Learning:** In systems like RumorProcessor that run every tick but only trigger actions probabilistically (e.g., 5% chance), computing arrays via `Object.values()` outside the condition creates massive unnecessary GC overhead for the 95% of ticks that do nothing.
**Action:** Always place expensive data aggregations (like `Object.values()`) strictly inside the conditional blocks that require them, rather than at the top of the function.
