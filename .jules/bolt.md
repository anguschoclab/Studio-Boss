...
## 2024-07-06 - Eliminate O(N*M) array allocations in weekly Biography Generation
**Learning:** Found a major O(N*M) performance bottleneck specific to this architecture during weekly entity iterations (`tickBiographyGenerator`). Because it iteratively calls `shouldUpdateBio` for every talent (O(N)), which in turn loops through game state dictionaries (e.g., relationships, cliques, scandals) doing `Object.values().filter()` and `.some()` calls repeatedly (O(M)), it created a massive number of short-lived array allocations and O(N*M) nested complexity during the hot path.
**Action:** Replace nested loops querying dictionary objects with a single O(M) pre-pass to construct a `Set` of target IDs before entering the main O(N) talent loop. Use `Set.has()` (O(1)) instead of array mapping to significantly improve scaling as the entity pool grows.
