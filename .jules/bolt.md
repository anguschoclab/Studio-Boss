...
## 2024-05-15 - Replace Math.max(...array.map()) with for loop for aggregates
**Learning:** Using `Math.max(...array.map(fn))` in calculation loops (like `calculateNominationWeight`) creates unnecessary intermediate arrays and risks exceeding the call stack limit for large arrays.
**Action:** Replace `Math.max(...array.map())` with a direct loop tracking the maximum manually.
