...
## 2025-02-27 - Optimized applyProjectResults to prevent severe GC overhead
**Learning:** To prevent severe garbage collection overhead in ECS game loops, avoid passing `Object.values()` or `Array.from()` to helper functions (like `applyProjectResults`) that internally reconstruct a `Map`.
**Action:** Type the helper function signature to accept `Record<string, T>` or `Map<string, T>` directly instead of forcing array allocation.
