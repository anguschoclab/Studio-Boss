...
## 2025-05-18 - Replace Object.values().some() with for...in loop in relationshipFormation

**Learning:** `checkNaturalFormation` in `relationshipFormation.ts` used `Object.values(existingRelationships).some()` to check for existing romantic relationships. Calling `Object.values` allocates an intermediate array of all relationships, causing severe garbage collection pressure and O(N) penalties during high-frequency relationship formation checks.
**Action:** Iterate directly using a `for...in` loop and explicitly access properties (with `hasOwnProperty` check) to drastically reduce overhead and prevent array allocation.
