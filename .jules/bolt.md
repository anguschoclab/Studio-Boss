...
## 2026-07-01 - Merge branch optimizations and lint cleanup

**Learning:** During the merge of multiple feature branches, several performance patterns and lint issues were consolidated across the codebase. The `for...in` loop pattern was applied consistently to `DeathSystem.ts`, `FranchiseEvolution.ts`, `StudioAutomation.ts`, and `TVRecommendationSystem.ts` to avoid `Object.values()` intermediate array allocations. Deterministic ID generation in `handleNewsAdded` was verified to use `state.week` and `state.tickCount` instead of `Date.now()`, ensuring reproducible simulations. Accessibility improvements (`aria-label`, `aria-hidden`, `aria-pressed`, `focus-visible:ring-*`) were verified across `FilterBar`, `TalentHub`, `ProjectCard`, and `TalentCard` components. Security hardening of Electron's `will-navigate` handler was confirmed to block non-localhost URLs in production.

**Action:** Replaced `as any` casts with `as unknown as` in `FranchiseEvolution.ts` and `state.types.ts` for safer type narrowing. Replaced `any` payload type with `unknown` in `ModalTriggeredImpact`. Fixed `no-prototype-builtins` by replacing `.hasOwnProperty()` calls with `Object.prototype.hasOwnProperty.call()` in `saveSchema.test.ts` and `saveWorker.test.ts`. Fixed `no-non-null-asserted-optional-chain` by replacing `?.prop!` with `?.prop ?? fallback` in `talentMarketplaceSlice.test.ts` and `talentSlice.test.ts`. Fixed `no-useless-assignment` in `efficiencyEvaluator.ts` by removing unused initial variable assignment. Created 9 new test files covering Electron security, DeathSystem, FranchiseEvolution, StudioAutomation, TVRecommendationSystem, Accessibility, BookmarkTooltips, TalentHubFocus, and handleNewsAdded determinism.

## 2024-07-01 - Avoid Object.values() combined with .filter() in BiographyGenerator.ts
**Learning:** Checking for breakout stars and active scandals in the BiographyGenerator.ts file was creating a new array per property per loop by using `Object.values(state).filter()`. This caused large performance spikes inside the high-frequency tick generator `tickBiographyGenerator` which calls `generateBiography()`.
**Action:** Replace `Object.values().filter()` chained methods with in-place `for...in` loops to evaluate state without creating intermediate arrays, reducing garbage collection spikes. Added `some()` array method for scandal check for performance.
