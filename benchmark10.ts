...
for (let i = 0; i < 100; i++) { original(); optimized10(); }
...
for (let i = 0; i < ITERATIONS; i++) { optimized10(); }
const endOptimized = performance.now();

console.log(`Original: ${(endOriginal - startOriginal).toFixed(2)} ms`);
console.log(`Optimized10: ${(endOptimized - startOptimized).toFixed(2)} ms`);
